#!/usr/bin/env python3
"""Golden reference builder — run the AIDLC workflow N times, evaluate each run,
then synthesise the best golden document set.

Usage:
    python scripts/build_golden_reference.py \
...
def write_summary(
    ...
    print(f"\n{'=' * 60}")
    print("  Golden Reference Build Complete")
    print(f"{'=' * 60}")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="build_golden_reference",
        description="Run AIDLC N times and synthesise a golden-reference document set.",
    )
...
def main() -> None:
    ...
    if not all_scores:
        log("No successful runs — cannot build golden reference")
        sys.exit(1)
