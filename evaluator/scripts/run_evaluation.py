...
def _find_aidlc_docs(run_folder: Path) -> Path:
    """Locate the aidlc-docs directory within a run folder.

    Handles three layouts:
    1. Flat:   run_folder/aidlc-docs/                          (v1 / CLI adapter)
    2. Intent: run_folder/aidlc-docs/intent-*/stages/          (v2 CLI adapter, already flattened)
    3. Raw v2: run_folder/workspace/org-ai-kb/aidlc-docs/      (Strands runner before flattening)

    Returns the first aidlc-docs directory found, preferring the flattened layout.
    Falls back to run_folder/aidlc-docs (may be empty) if nothing else is found.
    """
    # 1. Flat layout at root
    flat = run_folder / "aidlc-docs"
    if flat.is_dir() and any(flat.rglob("*.md")):
        return flat

    # 2. Raw v2: workspace/org-ai-kb/aidlc-docs/
    raw_v2 = run_folder / "workspace" / "org-ai-kb" / "aidlc-docs"
    if raw_v2.is_dir() and any(raw_v2.rglob("*.md")):
        return raw_v2

    # 3. One level deep (e.g. workspace/aidlc-docs/)
    ...
    return flat  # default even if empty
...
def _read_test_status(test_results_path: Path) -> dict | None:
    ...
    if status == "install_failed":
        ...
        if total is not None:
            summary = (
                f"{passed or 0} passed, {failed or 0} failed, {errors or 0} errors (of {total})"
            )
        else:
...
def stage_execute(args: argparse.Namespace) -> Path | None:
    ...
    docs_dir = _find_aidlc_docs(run_folder)
...
def stage_quantitative(
    workspace: Path, output_path: Path, pmd_path: str | None = None
) -> dict | None:
...
def stage_contract(
    ...
    hurl_dir: Path | None = None,
) -> dict | None:
    """Stage 4: API contract tests — spin up app and validate endpoints.

    Greenfield / bugfix: score against an OpenAPI spec (``spec_path``).
    Brownfield feature: score against a Hurl suite (``hurl_dir``) instead.
    """
    import os
    ...
    cmd = [sys.executable, "-m", "contracttest", "run", str(workspace)]
    if hurl_dir is not None:
        cmd += ["--hurl-dir", str(hurl_dir)]
    else:
        cmd += ["--openapi", str(spec_path)]
    cmd += ["--output", str(output_path)]
    if use_sandbox:
        cmd.append("--sandbox")
    else:
        cmd.append("--no-sandbox")
    _run_cmd(cmd, "Stage 4: API Contract Tests", env=env)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Brownfield feature: dir of .hurl files to score against instead of "
        "--openapi. Boots the app via framework auto-detection.",
    )
...
def main() -> None:
    ...
    if args.evaluate_only:
        if not args.evaluate_only.is_dir():
            ...
            sys.exit(1)
        # Brownfield: no golden doc set — the contract (openapi/hurl) is the
        # oracle. Skip the golden requirement + the qualitative-vs-golden stage.
        brownfield_eval = args.golden is None or not Path(args.golden).is_dir()
        if brownfield_eval:
            args.golden = None
        ...
        # Run contract tests if a contract exists and workspace is present.
        # Brownfield feature: --hurl-dir; greenfield/bugfix: --openapi.
        _has_hurl = bool(args.hurl_dir and args.hurl_dir.is_dir())
        _has_openapi = bool(args.openapi and args.openapi.is_file())
        if workspace.is_dir() and (_has_hurl or _has_openapi):
            contract_report_path = run_folder / "contract-test-results.yaml"
            contract_data = stage_contract(
                workspace,
                args.openapi if _has_openapi else None,
                contract_report_path,
                use_sandbox=args.sandbox,
                hurl_dir=args.hurl_dir if _has_hurl else None,
            )
        ...
        if args.golden is not None:
            ...
            print("\n  Qualitative stage skipped (brownfield: contract is the oracle)")
    ...
    candidate_docs = _find_aidlc_docs(run_folder)
