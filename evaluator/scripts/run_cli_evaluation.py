...
def main() -> None:
    ...
    parser.add_argument(
        "--tech-env", type=Path, default=REPO_ROOT / "test_cases" / "sci-calc-v2" / "tech-env.md"
    )
    parser.add_argument(
        "--golden",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to the claude-code .claude/ distribution directory "
            "(e.g. claude-code/dist/claude/.claude). When set, the claude-code "
            "adapter copies it into the workspace and drives the /aidlc skill. "
            "Defaults to claude-code/dist/claude/.claude relative to the git root. "
    ...
    if args.kiro_dist:
        ...
        candidate = git_root / "kiro" / "dist" / "kiro-ide" / ".kiro"
        if candidate.is_dir():
    ...
    if args.claude_dist:
        ...
        candidate = git_root / "claude-code" / "dist" / "claude" / ".claude"
        if candidate.is_dir():
