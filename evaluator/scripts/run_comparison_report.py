...
from reporting.baseline import (  # noqa: E402
    BaselineMetrics,
    extract_baseline,  # noqa: E402
    load_baseline,
)
...
def generate_comparison_markdown(
    ...
    if golden:
        golden_label = (
            f"Golden ({golden.executor_model.split('.')[-1]})"
            if golden.executor_model
            else "Golden"
        )
    ...
    for display_name, category, attr, higher_is_better in metric_rows:
        ...
        for col_name, metrics in columns:
            ...
            if (
                golden
                and col_name != "Golden (Opus 4.6)"
                and val is not None
                and golden_val is not None
            ):
