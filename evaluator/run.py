#!/usr/bin/env python3
"""Primary run script for AIDLC evaluation framework.
...
Available modes:
  ...
  - cli        Evaluation through a CLI AI assistant (kiro-cli, claude-cli, codex-cli)
...
def main() -> None:
    ...
    subparsers.add_parser(
        "cli",
        help="Evaluation through CLI AI assistants (kiro-cli, claude-cli, codex-cli)",
        add_help=False,
    )
    ...
    # Cross-version batch mode (v1 vs v1.5 vs v2 via the claude CLI)
    subparsers.add_parser(
        "version-batch",
        help="Run v1/v1.5/v2 x N through the claude CLI (concurrency-capped) + version report",
        add_help=False,
    )

    # Version comparison report (from completed version-batch runs)
    subparsers.add_parser(
        "version-report",
        help="Generate cross-version comparison report from version-batch runs",
        add_help=False,
    )

    # Combined greenfield + brownfield report (graded + deterministic sections)
    subparsers.add_parser(
        "combined-report",
        help="Roll up greenfield (graded) + brownfield (deterministic pass/fail) into one report",
        add_help=False,
    )
    ...
    mode_to_script = {
        ...
        "version-batch": SCRIPTS_DIR / "run_version_batch.py",
        "version-report": SCRIPTS_DIR / "run_version_report.py",
        "combined-report": SCRIPTS_DIR / "run_combined_report.py",
