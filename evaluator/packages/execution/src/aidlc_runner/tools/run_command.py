...
def make_run_command(run_folder: Path, timeout: int = 120) -> object:
    ...
    def run_command(command: str, working_directory: str = "workspace") -> str:
        ...
        try:
            # shell=True so that compound commands (&&, ||, ;, pipes) work correctly.
            # The working directory is validated above via _resolve_safe; the env is
            # restricted to prevent host credential leakage.
            # nosemgrep: dangerous-subprocess-use-audit, subprocess-shell-true
            result = subprocess.run(  # nosec B602,B603,B604 - intentional; cwd sandboxed to run_folder
                command,
                # nosemgrep: subprocess-shell-true
                shell=True,  # nosec B602 - intentional; cwd sandboxed, env restricted
                cwd=str(cwd),
                capture_output=True,
                text=True,
                timeout=timeout,
