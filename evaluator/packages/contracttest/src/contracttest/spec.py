            ...
            for tc in operation.get("x-test-cases", []):
                cases.append(
                    TestCase(
                        ...
                        method=method.upper(),
                        # Per-case `path` override lets one operation carry cases
                        # with concrete path-param values (the runner uses the
                        # path literally; there is no {param} substitution).
                        # Falls back to the OpenAPI path key.
                        path=tc.get("path", path_str),
