...
from contracttest.runner import (
    print_results,
    run_contract_tests,
    run_hurl_contract,
    write_results,
...
def main() -> None:
    ...
    run_cmd.add_argument(
        ...
        "(greenfield / bugfix). Mutually exclusive with --hurl-dir.",
    )
    run_cmd.add_argument(
        "--hurl-dir",
        type=Path,
        default=None,
        help="Directory of .hurl files to score a brownfield run against "
        "(RealWorld feature task). Boots the app via framework auto-detection "
        "and runs the suite. Mutually exclusive with --openapi.",
    )
    run_cmd.add_argument(
        "--output",
        "-o",
    ...
    if not args.workspace.is_dir():
        ...
        sys.exit(1)
    if bool(args.openapi) == bool(args.hurl_dir):
        print("Error: pass exactly one of --openapi or --hurl-dir", file=sys.stderr)
        sys.exit(1)

    # Brownfield Hurl-suite scoring path (RealWorld feature task).
    if args.hurl_dir:
        if not args.hurl_dir.is_dir():
            print(f"Error: hurl dir not found: {args.hurl_dir}", file=sys.stderr)
            sys.exit(1)
        results = run_hurl_contract(
            workspace=args.workspace,
            hurl_dir=args.hurl_dir,
