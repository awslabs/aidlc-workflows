...
def _run_case(client: httpx.Client, base_url: str, case: TestCase) -> CaseResult:
    ...
    if resp.status_code != case.expected_status:
        failures.append(f"status: expected {case.expected_status}, got {resp.status_code}")
        ...
        with server:
            ...
            with httpx.Client(follow_redirects=True) as client:
                for case in spec.test_cases:
                    ...
                    # nosemgrep: is-function-without-parentheses - is_running is a @property, not a method  # noqa: E501
...
def run_hurl_contract(
    ...
    """Boot the brownfield app and score it against a Hurl suite.

    Emits the SAME ContractTestResults shape as run_contract_tests (so the
    existing run_evaluation stage_contract / _read_contract_status / reporting
    all work unchanged) — each .hurl file becomes one CaseResult. Framework is
    auto-detected by ServerProcess (Flask/Django); no module/spec needed.
    """
    from contracttest.hurl_runner import run_hurl_suite

    hurl_files = sorted(hurl_dir.glob("*.hurl"))
    results = ContractTestResults(total=len(hurl_files))
    if not hurl_files:
        results.server_error = f"no .hurl files in {hurl_dir}"
        results.errors = 0
        return results
    ...
    try:
        server = ServerProcess(
            workspace=workspace,
            module="",  # unused for the auto-detected Flask/Django boot
            use_sandbox=use_sandbox,
            sandbox_image=sandbox_image,
            sandbox_memory=sandbox_memory,
            sandbox_cpus=sandbox_cpus,
    ...
    try:
        with server:
            results.server_started = True
            hres = run_hurl_suite(hurl_files, server.base_url, timeout=300)
            if not hres.hurl_available or hres.error:
                results.server_error = hres.error or "hurl unavailable"
                results.errors = results.total
                return results
            # Map per-file Hurl outcomes to CaseResults.
            by_file = {e.file: e for e in hres.entries}
            for f in hurl_files:
                ent = next(
                    (v for k, v in by_file.items() if k.endswith(f.name)), None
                )
                passed = bool(ent and ent.passed)
                results.cases.append(
                    CaseResult(
                        name=f.name, path=f"hurl:{f.name}", method="SUITE",
                        passed=passed, expected_status=200,
                        failures=[] if passed else [ent.message if ent else "suite failed"],
                    )
                )
                results.passed += int(passed)
                results.failed += int(not passed)
            # Trust the summary counts if the per-file mapping came up short.
            if not hres.entries:
                results.passed = hres.passed
                results.failed = hres.failed
    except (RuntimeError, TimeoutError) as e:
        results.server_error = str(e)
        results.errors = results.total - results.passed - results.failed
    return results
