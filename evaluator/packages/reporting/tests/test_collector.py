...
import yaml
from reporting.collector import _parse_coverage, collect
...
def _minimal_run(tmp_path: Path) -> Path:
    ...
    _write(
        run / "contract-test-results.yaml",
        {
            "total": 10,
            "passed": 9,
            "failed": 1,
    ...
    _write(
        run / "qualitative-comparison.yaml",
        {
            "overall_score": 0.89,
            "phases": [
                {
...
def test_parse_coverage_formats():
    # Explicit summary line (v1/v1.5 harness format)
    assert _parse_coverage("Total coverage: 91.3%") == 91.3
    # pytest-cov TOTAL without branch coverage: Stmts Miss Cover
    assert _parse_coverage("TOTAL     547     22    96%") == 96.0
    # pytest-cov TOTAL with --cov-branch (v2/v2-odyssey): Stmts Miss Branch BrPart Cover
    assert _parse_coverage("TOTAL     547     22     84      2    96%") == 96.0
    assert _parse_coverage("TOTAL     486      5     82      1    99%") == 99.0
    # No coverage info at all
    assert _parse_coverage("160 passed in 0.38s") is None
