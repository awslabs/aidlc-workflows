...
)  # packages/cli-harness/src/cli_harness -> evaluator root
...
def run_cli_evaluation(
    ...
    contract_hurl_dir: Path | None = None,
    ...
    config = AdapterConfig(
        ...
        capture_tokens_otel=capture_tokens_otel,
    ...
    # 5. Run evaluation pipeline (stages 2-6). Brownfield has no golden doc set
    # (the contract IS the oracle), so --golden is omitted there and the
    # qualitative-vs-golden stage is skipped by run_evaluation.
    eval_cmd = [
        sys.executable,
    ...
    if golden_docs and Path(golden_docs).is_dir():
        eval_cmd += ["--golden", str(golden_docs)]
    if profile:
        eval_cmd += ["--profile", profile]
    if region:
        eval_cmd += ["--region", region]
    # Brownfield feature scores against a Hurl suite; else the OpenAPI spec.
    if contract_hurl_dir and Path(contract_hurl_dir).is_dir():
        eval_cmd += ["--hurl-dir", str(contract_hurl_dir)]
    elif openapi_path and openapi_path.is_file():
