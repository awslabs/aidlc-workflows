...
# Strip a leading v1.5/v2 intent-mode prefix so docs bucket by stage folder.
_INTENT_PREFIX_RE = re.compile(r"^intent-\d+-[^/]+/")
...
def normalize_output(
    ...
    # Add cost if available (not in the reference schema but useful). The OTEL
    # capture path emits `cost_usd` (from claude_code.cost.usage) — the robust
    # cross-version consumption axis; older paths used `total_cost_usd`.
    cost = tu.get("cost_usd", tu.get("total_cost_usd"))
    if cost:
...
# Directories that hold DEPENDENCIES / build caches / VCS — never first-party
# generated code. Excluded from artifact counts, else a single `.venv` inflates
# "source files" ~40x (1,122 dep .py files vs 27 real ones on the sci-calc run).
_NON_FIRST_PARTY_DIRS = {
    ".venv",
    "venv",
    "site-packages",
    "node_modules",
...
def _count_workspace_files(workspace: Path) -> dict:
    """Count first-party files in the workspace by category.

    Skips dependency/cache/VCS trees (see _NON_FIRST_PARTY_DIRS) so the counts
    reflect GENERATED code, not the installed venv or tooling caches.
    """
    ...
    for f in workspace.rglob("*"):
        if not f.is_file():
            continue
        # Skip anything inside a dependency/cache/VCS dir.
        if any(part in _NON_FIRST_PARTY_DIRS for part in f.parts):
            continue
        ext = f.suffix.lower()
        name = f.name.lower()

        # A file under a tests/ tree is a test regardless of its name
        # (conftest.py, tests/__init__.py, fixtures), matching how the
        # generated projects organize their suites.
        rel_parts = {p.lower() for p in f.relative_to(workspace).parts[:-1]}
        in_test_dir = bool(rel_parts & {"tests", "test", "__tests__"})

        is_test = in_test_dir or any(p in name for p in test_patterns)
...
def _count_doc_files(docs_dir: Path) -> dict:
    ...
    Recognizes v1/v2 phases (inception, construction) and v1.5's four-stage
    layout (inception, design, construction, operations). A leading
    ``intent-NNN-<slug>/`` prefix (v1.5/v2 intent mode) is stripped first so
    docs bucket by their stage folder, not into ``other``.
    """
    inception = design = construction = operations = other = 0
    for f in docs_dir.rglob("*.md"):
        rel = str(f.relative_to(docs_dir))
        rel = _INTENT_PREFIX_RE.sub("", rel)
    ...
    return {
        ...
        "operations_files": operations,
        "other_files": other,
        "total_files": inception + design + construction + operations + other,
    }
