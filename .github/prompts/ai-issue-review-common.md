...
When a candidate relies on a trusted repository file, name its exact
`path:line` or `path:start-end` location in backticks in the specialist report.
The final no-tool judge receives an immutable base copy of tracked paths cited
...
The judge discards claims whose supporting repository source was not supplied.
