...
- `mode: "incremental"`: AIDA already reviewed this pull request at the head
  ...
  changed since that review, plus the listed files for file-level or deleted-line
  evidence. Read whatever base and head context you need to judge those lines,
...
candidates are never deferred. If you notice such a defect outside your scope,
name it in one line as a pointer for the security lens instead of a candidate.
