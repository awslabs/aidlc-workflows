...
- `mode: "incremental"`: AIDA already reviewed this pull request at the head
  ...
  changed since that review; a file with `deletions: true` also admits `LEFT`
  evidence (lines or a whole file deleted since, cited by `previousPath` for a
  rename); a listed file admits file-level evidence. Read whatever base and head context you need to judge those lines,
