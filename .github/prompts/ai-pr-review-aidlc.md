...
needed to review it. The runner separately verifies that the read-only sandbox
starts and that each Codex process exits successfully, so do not add a second
subjective execution verdict. Return `"status": "complete"` and every manifest
path exactly once in `inspection.changedFiles`. Record any non-blocking
validation limitation in `residualRisk`.
