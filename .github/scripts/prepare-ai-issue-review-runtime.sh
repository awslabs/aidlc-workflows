...
context_file="$GITHUB_WORKSPACE/.ai-issue-review-context/issue.json"
if ! sudo -u "$review_user" test -r "$context_file"; then
  echo "$review_user cannot read the immutable issue-review context" >&2
  exit 1
fi
if sudo -u "$review_user" test -w "$context_file"; then
...
sudo sh -c \
  'printf "%s\n" \
    ...
    > /etc/sudoers.d/ai-issue-review-env'
sudo chmod 440 /etc/sudoers.d/ai-issue-review-env
sudo visudo -cf /etc/sudoers.d/ai-issue-review-env
