...
| t66 | `tests/feature/t66-graph-library.sh` | aidlc-graph.ts 8-function library + CLI — compile, traversal, parity, the MR 12 `AIDLC_GRAPH_RESOLVE=1` cutover-parity net (frontmatter-derived grid == legacy plan ×9), env-seam + designer-export `--check` drift guard + `rules_in_context` resolution (MR 7a) + `sensors_applicable` resolution (MR 7b) + `withAuditLock` reentrancy guards + compile passes `ctx.agents` (unknown `lead_agent`/`support_agent` fails the build; reserved `orchestrator` exempt) (99 tests) |
