...
// drift sweeps. Migrated from tests/feature/t55-test-suite-drift.sh (TAP plan 7).
...
// `.sh` rows — all 30 surviving `.sh`, zero `.test.ts` rows. The Smoke / Unit /
// Worktree tier tables are EMPTY because those tiers are now entirely `.test.ts`.
...
//           guardrail, feature/t32-stage-graph-consistency) are now `.test.ts`
...
// import.meta.dir is tests/feature/; TESTS_DIR is its parent (tests/).
...
// The seven tier directories the .sh walked.
const TIERS = [
  "smoke",
  "unit",
  "feature",
...
interface ReadmeRow {
  /** Column-2 backticked path, e.g. "feature/t55-test-suite-drift.sh". */
  path: string;
 ...
 * Column 2 holds a backticked `tier/file` path; the description's LAST
 * parenthesised count is canonical (a row like "31 stages ... (93 tests)" takes
 * the last). Keyed by full `tier/file` — bare IDs collide across tiers.
 */
...
/** Lookup a README row's count by full `tier/file` path (null = not listed). */
...
/** Every `.sh` test file on disk, as `tier/file` relative paths. */
...
/** Every test file on disk (`.sh` AND `.test.ts`) as `tier/file` paths. */
...
describe("t55 — test-suite metadata drift (migrated from t55-test-suite-drift.sh, plan 7)", () => {
  // ───────────────────────────────────────────────────────────────────────────
  ...
  // README tracks only `.sh` rows today (verified: zero `.test.ts` rows; Smoke/
  // Unit/Worktree tier tables empty). A `.test.ts` lacking a row is CORRECT and
  ...
  test("4: dynamic-plan allowlist is obsolete — no surviving .sh has a computed plan [.sh ok 4]", () => {
    ...
    const formerEntries = [
      ...
      { tier: "feature", stem: "t32-stage-graph-consistency" },
    ];
  ...
  // that names a `tests/<tier>/<file>.sh` path AND carries a literal count, that
  // count must equal the README row for the same `tier/file`. (09-testing.md
  ...
  test("5: docs/reference/09-testing.md (N tests) matches tests/README.md [.sh ok 5]", () => {
    ...
    for (const line of testingDoc.split("\n")) {
      if (!/^\|\s*t[0-9]/.test(line)) continue;
      const pathM = line.match(/tests\/[a-z]+\/t[0-9]+[a-zA-Z-]*\.sh/);
      if (!pathM) continue;
...
function walk(
  ...
  for (const ent of entries) {
    ...
    const relPath = relFromRepo(child);
