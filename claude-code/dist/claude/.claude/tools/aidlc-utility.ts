...
function handleDoctor(projectDir: string): void {
  ...
  // 2. Hook presence — all 7 hooks are TypeScript, run via bun (no executable bit needed)
  const tsHooks = [
    "aidlc-audit-logger",
  ...
  for (const h of tsHooks) {
    const hookPath = join(projectDir, ".claude", "hooks", `${h}.ts`);
    results.push({
  ...
  // self-creates the audit file/dir, so the only throw is a real write
...
function handleScopeTable(
  ...
  console.error(
    "SKILL.md scope-table region is out of date. Run `bun .claude/tools/aidlc-utility.ts scope-table` and paste the output between the BEGIN/END markers."
  );
  process.exit(1);
}
