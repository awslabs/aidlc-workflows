...
prior in its frontmatter — `ars: {targets: [ve, r], cost: 4}` on the integration
stage, `ars: {targets: [ve], cost: 5}` on the full suite — so `aidlc-graph ars`
screens them like core stages instead of listing them as `no-prior` (doc 15
§ `ars`); the shipped `tools/data/ars-priors.json` names core stages only.
