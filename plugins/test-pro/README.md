...
| **`aidlc-quality-agent`** (persona) | One Collaboration bullet naming `test-pro-metrics-agent` as the coverage read-out partner — `contributions/agents/`, prose fragments only. |
...
`sensors` / `required_sections`) and **prose** additions; a contribution under
`contributions/agents/` enriches a core persona with prose fragments only and **prose** additions (`fragments` at anchors
