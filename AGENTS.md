...
- `scripts/install.ts` — **The interactive installer.** `bun scripts/install.ts` prompts for a harness and copies the correct `dist/<harness>/` tree into a target project. Accepts `--harness`, `--target`, `--force`, `--all`, `--doctor` for non-interactive use.
...
- **Hooks** (`hooks/`) — 17 framework hooks, all prefixed `aidlc-*.ts`, covering audit emission, sensor dispatch, runtime-graph compile, session lifecycle, state validation, subagent tracking, statusline rendering, human-presence mint, exact dispatch-rule delivery, forwarding-loop enforcement, reviewer read-scope enforcement, review-receipt write-freeze enforcement, code-generation plan-approval enforcement, direct state-transition enforcement, and token-usage folding (the Claude-only usage-ledger producer)
