...
- `plugins/<name>/` — **Optional, owned AIDLC plugins** (the plugin mechanism; design set in `plugins/docs/`). Each holds `.aidlc-plugin/plugin.json` (the declarative manifest) + core-shaped subtrees (`stages/`, `contributions/`, `sensors/`, …). Not yet wired into the build — design only, implementation pending. `plugins/ops-min/` and `plugins/test-pro/` are the fixtures.
