...
- `core/tools/vendor/markdown-parser.js` — **Committed, generated parser dependency.** Bun-bundled `micromark@4.0.2` + `micromark-extension-gfm@3.0.0`; upstream MIT notices are in the bundle header and `core/tools/vendor/LICENSES.md`. The packager copies this directory into every harness's tools.
...
- **Markdown block structure has one interpreter:** `markdownBlocks` in `core/tools/aidlc-lib.ts` supplies block visibility, containers, link reference definitions, and claim-splitting structure; `visibleMarkdownLines` projects it. Do not add another block scanner or definition grammar. The security-sensitive `Bun.markdown.render` review-authority renderer is deliberately separate.
- **Regenerate the vendored parser only with `bun scripts/vendor-markdown-parser.ts` under Bun 1.3.14**, matching the `package.json` `packageManager` and CI pins. The script refuses another Bun version; never hand-edit the bundle. `tests/unit/t340-vendored-markdown-parser.test.ts` guards byte-identical rebuilds and MIT notices (parity explicitly skips on a different Bun). When upgrading parser dependencies or Bun, update the pins together, regenerate the bundle, and reconcile `core/tools/vendor/LICENSES.md`.
