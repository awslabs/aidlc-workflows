...
- **Model provider**: The shipped \`.claude/settings.json\` does not select a provider, model, region, or reasoning effort. Claude Code keeps your existing subscription, API, Bedrock, or enterprise-gateway configuration. Amazon Bedrock remains optional; setup instructions are in \`docs/guide/01-getting-started.md\` § "AWS Bedrock Setup (optional)".
