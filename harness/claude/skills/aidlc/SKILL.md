...
| `ask` | Render `directive.question` via `AskUserQuestion` (this harness's binding for the protocol's structured questions — see `question-rendering.md` beside this file), then feed the human's answer back on the next `report` via `--user-input "<answer>"`. For "Resume from last checkpoint", call `report --result resumed --user-input "<answer>"`, then act on its run-then-continue `print`. The engine never calls `AskUserQuestion` itself — it defers the human turn to you. |
