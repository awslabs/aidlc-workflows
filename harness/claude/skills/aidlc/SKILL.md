...
- **`gate: true`** — after the stage body produces its artifacts:
  0. **Draft step (inline artifact-generating stages):** Once answers are collected and ALL `[Answer]:` tags are filled, draft the `produces` artifacts via a single `Task(directive.lead_agent)` — passing the answered questions file + resolved `consumes` paths — per `aidlc-common/protocols/stage-protocol.md` § "Drafting delegation for inline stages". The subagent writes the artifacts, appends to `memory_path`, and returns ONLY the §11 summary; read it, then continue. This offloads only the draft reasoning — it is DISTINCT from `mode: subagent` (whole-stage delegation incl. the gate). The subagent treats any `*-questions.md` in the stage dir as read-only and is never spawned while a tag is blank.
  ...
  4. Present the approval gate via `AskUserQuestion` (Approve / Request Changes). On approval, `report --stage "<directive.stage>" --result approved` — the engine's `report` owns the full transition (it opens a missing gate if needed, dispatches the right `aidlc-state.ts` subcommand, and advances; never call those tools yourself, and never re-report the same directive). On a Request-Changes / reject, run the Keep/Modify/Redo loop within this stage (below) and re-present; the reject path stays conductor-side and is not a `report` outcome.
...
- **Self-learning guardrails**: Human corrections can become persistent behavioural rules in `.claude/rules/` via the §13 learnings ritual.
