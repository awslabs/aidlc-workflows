...
// where <target> ∈ record-human-turn | verb-intercept | enforce-approval-gate |
//                  session-start | audit-and-sensors | rebuild-stage-graph |
...
interface IdeHookContext {
  ...
  toolResult?: string;
  toolSuccess?: boolean;
  // UserPromptSubmit only: the text the human typed, verbatim. Both supported
  // surfaces deliver the RAW slash literal here (see the verb-intercept seam).
  prompt?: string;
...
// The targets whose forward depends on the hook payload. Every other target
// builds a fixed input (or reads only the filesystem), so it skips payload
// acquisition entirely and keeps its zero-latency path. verb-intercept is the
// one member that reads `prompt` rather than a tool payload: it must see the
// raw slash text the human typed, so it pays the same acquisition.
// review-freeze and plan-approval-guard read the tool name + arguments to decide
// whether to refuse, so they must acquire the payload — omitting them here leaves
// `ide` empty and the guards silently no-op.
const PAYLOAD_TARGETS = new Set([
  ...
  "review-freeze",
  "plan-approval-guard",
  // state-transition-guard reads the shell command out of the payload, so it
  // must acquire it too - omitting it here leaves `ide` empty and the guard
  // silently no-ops (the same failure review-freeze and plan-approval-guard hit).
...
//
// Wired by aidlc-verb-intercept.json. A `/aidlc` command that leads with a
// workspace navigation verb (space / space-create / intent) or carries a
// read-only utility flag (--status / --doctor / --help / --version) is TERMINAL:
// it maps 1:1 to an aidlc-utility.ts subcommand and carries no workflow work.
// Over an ACTIVE workflow the live conductor is unreliable at honouring these -
// under accumulated session context it runs a bare `next` and advances the
// active intent (the "roll-forward" bug). This seam runs the utility
// DETERMINISTICALLY before the conductor decides anything, hands back the
// verbatim output with an explicit do-NOT-advance instruction, and stamps the
// turn-scoped latch that the engine done-guard reads (aidlc-orchestrate.ts
// Branch 0). Advisory throughout: every failure path exits 0 and leaves the turn
// to the conductor.
//
// WHY THE LATCH MATTERS HERE: Branch 0 of the engine's `next` already reads
// aidlc/.aidlc-readonly-latch and aidlc/.aidlc-turn-counter, but nothing in this
// tree wrote either file, so that guard was permanently fail-open. This seam is
// the writer it was waiting for.
//
// WHY A RAW PARSER, and not the Kiro CLI adapter's extractNextInvocation(): that
// function recovers the argv from the `aidlc-orchestrate.ts next <ARGS>` anchor
// inside the EXPANDED skill body. MEASURED 2026-08-16 on both supported surfaces
// (Kiro IDE 1.0.309 and Kiro CLI 2.18.1 --v3): UserPromptSubmit delivers `prompt`
// as the RAW slash text - the 34-character probe `/probe-echo hello --scope
// tutorial` arrived verbatim on both - with no expanded body and no anchor line.
// So the anchor never matches, args comes back empty, and the dispatch is a
// silent no-op while the turn clock still ticks (half-dead and quiet). The raw
// text is exactly what the user typed, so parse that instead. The anchor parser
// is not a fallback: on these surfaces it can only ever return nothing.
//
// DELIBERATELY NOT PORTED from that adapter, pending a live run:
//   - the forwarding latch and its exit-2 argument-fidelity gate. They hard-block
//     a tool call, have never executed on either supported surface, and on a
//     mismatch the latch is not consumed - so a conductor that normalizes the
//     argv differently is blocked repeatedly within the same turn.
//   - the pre-dispatch of `next` for explicit flags. It mutates workflow state
//     off-band, which no failure path in this seam does.
...
// The argv of a raw `/aidlc …` invocation. The negative lookahead keeps a
// different command that merely starts with the same letters (`/aidlc-foo`) from
// matching. Anything that is not this slash command yields no args, so the seam
// falls through to the conductor untouched.
function extractSlashArgs(prompt: string): string[] {
  const m = prompt.match(/^\s*\/aidlc(?![\w-])\s*([^\n]*)/);
  if (m === null) return [];
  return shellWords((m[1] ?? "").trim());
}

if (target === "verb-intercept") {
  const args = extractSlashArgs(ide.prompt ?? "");
  const cmd = classifyTerminalCommand(args);
  // Turn-clock: bump EVERY time this seam fires, and BEFORE the cmd === null
  // exit, so a turn that carried no terminal command still advances the clock
  // and a prior turn's latch goes stale. The latch below stamps THIS value; the
  // engine done-guard fires only when latch.turn === the current counter (the
  // same turn), so the neutralization is turn-scoped - no time window, no wedge.
  // Best-effort: a counter failure fails open.
  let turn = 0;
  try {
    mkdirSync(join(projectDir, "aidlc"), { recursive: true });
    const counterPath = join(projectDir, "aidlc", ".aidlc-turn-counter");
    turn = existsSync(counterPath)
      ? (Number.parseInt(readFileSync(counterPath, "utf-8").trim(), 10) || 0) + 1
      : 1;
    writeFileSync(counterPath, `${turn}\n`, "utf-8");
  } catch {
    /* turn-clock best-effort */
  }
  // Human presence is NOT recorded here: aidlc-record-human-turn.json owns the
  // HUMAN_TURN event and the .aidlc-human-turn marker on this harness. The Kiro
  // CLI adapter rides both on this one seam because that tree has no separate
  // registration for them; duplicating them here would double-count every turn.
  if (cmd === null) return 0; // not terminal - the conductor owns this turn
  ...
  if (cmd.error !== undefined) {
    ...
    out = ((spawned.stdout?.toString() ?? "") + (spawned.stderr?.toString() ?? "")).trim();
  }
  // Turn-scoped latch: a terminal command was handled OFF-BAND this turn, so a
  // bare advancing `next` in the SAME turn is the spurious roll-forward and the
  // engine done-guard neutralizes it. Every classified family arms it, including
  // the error path (the command still did not advance the workflow).
  try {
    mkdirSync(join(projectDir, "aidlc"), { recursive: true });
  ...
  // Kiro's context channel is plain stdout at exit 0, and UserPromptSubmit CANNOT
  // block on either supported surface (MEASURED 2026-08-16: exit 2 does not stop
  // the turn, contradicting the "Can block: Yes" in the official table). So the
  // conductor RELAYS this output rather than being bypassed. Echo the command the
  // way the user typed it so the short-circuit message is legible.
...
// --- plan-approval-guard: code-generation cannot dispatch pre-approval -------
//
// Wired by aidlc-plan-approval-guard.json (PreToolUse). Its matcher is kept
// EQUAL to the isDispatch predicate below - `^(subagent|invoke_sub_agent|
// subagent_.+)$` - because a matcher narrower than the predicate silently drops
// dispatches the adapter would have handled. An unanchored `subagent` matcher
// does NOT reach `invoke_sub_agent` - the underscore in `sub_agent` breaks the
// substring - and that is the built-in-subagent path, so top-level built-in
// dispatches would never arrive here. aidlc-log-subagent's matcher hit the same
// asymmetry and carries the same anchored form.
// This forwards the dispatch prompt (which carries the `AIDLC-UNIT:` target
// marker the core body reads) as the Task tool input.
//
// MEASURED (2026-08-14 live runs, IDE 1.0.309 and CLI 2.18.1 --v3). PreToolUse
// DOES deliver tool arguments on both surfaces: a subagent dispatch arrives with
// keys ["prompt", "explanation"]. The "a pre-tool payload carries no tool
// arguments on Kiro" reading came from IDE 1.0.89/1.0.165, which neither of these
// surfaces is; several core hook bodies still carry prose from that vintage.
//
// A `.aidlc-hooks-health/*.last` file proves the hook FIRED, never that the
// payload was populated - read the debug log's argument keys for that.
if (target === "plan-approval-guard") {
  const tool = ide.toolName ?? "";
  const isDispatch =
    tool === "subagent" ||
    tool === "invoke_sub_agent" ||
    (tool.startsWith("subagent_") && tool !== "subagent_response");
  if (!isDispatch) return 0;
  const ti = ide.toolArgs ?? {};
  // The dispatched agent's identity is in the TOOL NAME, not in the arguments:
  // `subagent_aidlc-developer-agent`. Measured 2026-08-14 (IDE 1.0.309, CLI
  // --v3): the argument keys are ["prompt", "explanation"] - neither
  // `agent_type` nor `name` is present. Deriving from the arguments alone left
  // subagent_type "" and the core body returned early at its
  // `subagent_type !== "aidlc-developer-agent"` check, so the guard never
  // reached evaluation. Keep both keys as fallback: `subagent` and
  // `invoke_sub_agent` carry no agent suffix to strip.
...
// NO fold-usage verb here, deliberately. aidlc-fold-usage.ts ships in this tree
// (packaged core) but is a CLAUDE-CODE-ONLY producer: its reader in
// aidlc-usage.ts parses Claude Code's `.jsonl` transcript format, which Kiro
// does not write. Its own header says so - "this hook is wired only in the
// Claude harness's settings.json. Kiro / Codex / opencode wire no producer".
// Leaving it unregistered here is therefore intent, not an oversight. Measured
// on a 2026-08-06 live run: 200 invocations, ledger writes ZERO. Do not add it
// by symmetry with the Claude harness.
