...
- TTSR rules: omp's regex-triggered stream aborts are *enabled* but no AIDLC rule ships today. If you author one, drop it in `.omp/rules/` or `.omp/ttsr/`. Test with `ttsr.condition`.
