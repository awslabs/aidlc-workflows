...
| `invoke-swarm` | Load `aidlc-common/protocols/stage-protocol-swarm.md` and `aidlc-common/protocols/stage-protocol-construction.md` before acting, then follow the swarm module's subsection for this harness. The engine lists both in `directive.protocol_modules`; the directive kind is the fallback trigger. |
