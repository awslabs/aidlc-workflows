...
Cursor has no structured-question widget, so every structured question renders
...
- **Always append an "Other" escape** as the final number — the spec's
  options never include one; on this harness you render it.
...
- **Long prompts**: the question body renders at full terminal width in Cursor
  chat and wraps gracefully — see
