...
| `ask` | Every engine ask carries `ask_type` and `response_route`. Render `directive.question` as numbered prose per `question-rendering.md`, then follow that typed contract. When `directive.ask_type === "new-work-routing"` and `response_route === "next"`, never call `report`: part of the active work = re-run bare `next`; separate work = `next --new-intent --scope <directive.proposed_scope, or the human's correction> "<directive.new_work_description>"`; reshape = `next compose "<the human's words>"`. When `directive.ask_type === "unit-claim"` and `response_route === "claim"`, present the listed claimable, claimed, and waiting Units; on the human choice run `bun {{HARNESS_DIR}}/tools/aidlc-utility.ts claim <unit> --team "<label>"`, print its output, and STOP. For any other `"next"` ask, run the exact command field named for the chosen answer and never call `report`. For `"command"`, run the named command and then re-run `next`. `"report"` is reserved for the resume menu only: for any answer (resume / redo / jump / start fresh), call `report --result resumed --user-input "<answer>"`, then act on the returned per-choice `print`. Do not call `ask_user` or `vscode/askQuestions`: while the selected workflow is running, the PreToolUse guard denies those picker calls and directs numbered prose plus an end-turn because picker results do not record the trusted human action required by the approval guard. The tool never asks the user itself; it defers the human turn to you. |
