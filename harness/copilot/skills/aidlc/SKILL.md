...
- Retiring an intent is a first-class command, not a manual edit: `/aidlc intent abandon <name>` moves it to the terminal `abandoned` status (record dir and audit shard PRESERVED) and hides it from the default listing; `/aidlc intent restore <name>` reverses it, and `/aidlc intent list --all` shows abandoned ones. Both take a `<name>` — forward it verbatim and never hand-edit `intents.json`.
