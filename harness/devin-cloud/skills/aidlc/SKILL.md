...
# AI-DLC Orchestrator (Devin Cloud harness)
...
All stages follow `aidlc-common/protocols/stage-protocol.md` for approval gates, question format, and completion messages. Structured questions render in chat per `question-rendering.md` beside this file.
...
## Cooperative enforcement (READ FIRST — this harness has no hooks)

Devin Cloud sessions have no repo-local lifecycle-hook transport: nothing in this repository can intercept a tool call before it runs. On this harness the engine's authority lives in **persisted state + the audit trail + explicit engine calls**, not in interception. Your obligations:

- **Call the engine at every transition.** `next`, `report`, `decision`, `answer`, `approve`, `intent`, `space`, `compose`/`recompose` — each transition is an engine call, and the engine REFUSES the transition when the state or receipts do not satisfy the precondition. Never edit `aidlc-state.md`, audit shards, or receipt files by hand; a hand-edit is exactly what the doctor's checkpoint scan flags.
- **Session identity is yours to mint once.** On first invocation in a session, run `bun .aidlc/hooks/aidlc-session-start.ts` with a stdin payload of `{"session_id": "<a fresh UUID>", "hook_event_name": "SessionStart"}` (generate the UUID once and reuse it all session — for example `cat /proc/sys/kernel/random/uuid`). That call writes the session record the rest of the engine binds to; every later engine call that accepts `--session` MUST pass the same id. Do not re-mint per call and do not substitute user text or timestamps for the id.
- **Human turns are blocking chat questions.** There is no `ask_user_question` tool here; render the structured question in chat per `question-rendering.md` and END YOUR TURN — the session waits for the human's reply natively. Record the human's answer through the engine commands the stage protocol names (`aidlc-log.ts answer`, `report --user-input`, plan-approval `answer`), passing `--session <your session id>`.
- **Plan Approval rides the same binding.** When a Plan Approval decision is pending, `answer`/`decision` calls carry `--session <your session id>`; a response recorded under a different session cannot consume the challenge. Do not fall back to any ambient marker.
- **Subagent-shaped topologies are advisory.** Cloud's parallel surface is managed Devin sessions/dynamic workflows — separate VMs that cannot share this repo's engine session. Run `subagent`/`pipeline`/`mob` directives with inline personas instead (adopt the named agent's persona in this session), and say so plainly when the directive's mode is not inline.
- **Reviewer isolation and plan-approval enforcement are cooperative, not intercepted.** The engine verifies state at each call; nothing blocks a stray write between calls. The doctor reports the guarantee level honestly.

---

## The Forwarding Loop

This is the orchestrator's whole control structure. Run it from the moment the skill is invoked.
...
Run the engine via the shell tool. If a directive looks malformed or names a move you cannot make, say so plainly and stop ("something in the workflow's setup is off", plus the specific detail), never a cue to improvise the routing in prose.

**Validity advisories.** If a directive carries `stage_validity`, show `stage_validity.warning` to the user, then act on `directive.kind` normally. The field is detection-only: never turn it into an error, stop, jump, or alternate route. Untracked-only histories do not attach an advisory per turn and remain visible in `--status`, which provides the detailed stale, revalidation, and untracked lists.
...
| `parked` | The workflow was parked at a clean inter-stage boundary (`directive.stage`) for a later session. Tell the user it is parked and how to resume (invoke this skill with `--resume`), then STOP the loop. No stage was advanced and nothing was marked complete. |
...
**Per-unit batch waves (optional).** On the default stage-major walk, `directive.wave` is the engine-owned parallel surface for functional-design, nfr-requirements, nfr-design, and infrastructure-design. Branch on it before the ordinary per-unit/gate path; parent Unit fields are only a projection of the first entry. Give every builder the parent `stage_file`, every `inline_context_paths` file, `context_warnings`, and the complete steering bundle verbatim, plus its entry paths, including `entry.unit_memory_path`. On this harness builders run serially or as inline personas — there is no same-VM worker primitive; independent managed sessions would not share this engine's session state, so do not fan a wave out to separate Cloud sessions. Builders do not call serial `unit start/pause/resume`; a blocked builder returns its question and withholds a path from `entry.required_produces`. After build work, run or resume the review named by `review_state`: `outstanding` uses `review_iteration`, `retry-required` repeats that request with `--retry-pending`, `repair-required` runs lead repair then the next iteration, and `recovery-required` runs the one stale-receipt recovery at `review_iteration`. `escalation-required` means recovery was already spent: do not request another review or complete the Unit; halt for a human Request Changes decision. Reviews are advisory passes here — nothing intercepts the paths. When build and review are settled and `completion_required` is true, run `aidlc-state.ts unit complete --wave --stage "<directive.stage>" --unit "<entry.unit>"`; that tool verifies the live entry, deduplicates its Unit diary into the parent diary, and emits `UNIT_COMPLETED`. Then re-run `next` without report-approve. Code Generation and unit-major iteration never carry a wave.

`directive.mode` selects the communication topology. Load `aidlc-common/protocols/stage-protocol-ensemble.md` when the engine lists `ensemble` in `directive.protocol_modules` (or, as a fallback, when mode is `subagent`, `pipeline`, or `mob`, or support agents are present), then follow only this harness's topology subsection — on Cloud every dispatched topology resolves to its inline form (personas adopted in-session), since same-VM subagents do not exist here.

**Dispatched topologies run inline on this harness.** When `directive.mode` is `subagent`, `pipeline`, or `mob`, adopt the lead persona (and each support persona in turn per the topology) INSIDE this session — never claim a `run_subagent` dispatch occurred, and never write contribution files pretending a dispatch ran. If a stage's contract structurally requires separate subagent evidence (contribution files the engine verifies), stop and tell the user that stage's dispatched mode is unavailable on Devin Cloud rather than fabricating it. `mode: inline` stages are unaffected.
...
- **On genuine new-work, OFFER - never auto-create.** Surface a structured question showing the active intent and the proposed new one, including the **scope** you would give the new intent (infer it from the new-work description the way the engine resolves a fresh invocation - keyword/precedence - and name it so the human can correct it). Phrase it as a Yes/No confirmation and **lead the affirmative option with the word "Yes"** (e.g. "Yes - start a second intent"), with a decline option alongside. Starting a workflow is a mutation gated on a human yes (judgement→human) - never create without an explicit confirmation.
- **On CONFIRM:** re-run `next` with `--new-intent` and the confirmed scope + new-work text: `bun .aidlc/tools/aidlc-orchestrate.ts next --new-intent --scope <the confirmed scope> "<the new-work description>"`. The engine returns a `print` directive naming the `intent-create` command, including the `--label "<2-3 word kebab essence>"` placeholder. Replace `--label` with a short 2-3 word essence of the description and run the command. Then **STOP and hand off to a fresh session** rather than re-running `next`: tell the user to start a new Devin session and invoke the aidlc skill there to begin the new intent with a clean slate. The full description and intent are already saved on disk. The offer itself is conductor prose, not a new directive kind.
...
**On approve (front/report), the write and the creation run in the SAME turn - no second skill invocation:**

1. If the proposal MATCHED a stock scope, skip the write entirely.
2. For a CUSTOM grid, author the two files at the paths the `detect --json` output printed: `scopes/aidlc-<name>.md` (frontmatter `name`, `depth`, and `keywords: []` - composed scopes are NOT inferable unless the human explicitly granted keywords at the gate) plus the `"<name>": { "stages": {...} }` entry in `scope-grid.json`. BOTH files are required - a `.md` without a grid entry resolves as all-SKIP.
...
**In-flight recompose (a workflow is RUNNING):** the dispatch print carries the marker discipline - write `aidlc/.aidlc-compose-pending` BEFORE presenting the gate (it lets the turn end at the gate cleanly), and DELETE it the moment the gate resolves (approve, edit-then-resolve, or reject). On approve run the named `recompose --skip <slugs> --add <slugs>` command; it validates strictly (a starved required input rejects), flips only PENDING ahead-of-cursor stages, rebuilds the derived state fields, and audits RECOMPOSED - never edit the state file's suffixes by hand. A leftover marker after the gate resolves would masquerade as a pending gate on the next read; deleting it is part of acting on the directive.

**Reshape requests arrive in plain chat too, not just as the literal verb.** Mid-workflow, "can we skip market research? we already know this market" is a plan-reshape signal (see "New work while an intent is active" - the same first-judgment step classifies it); route it through `next compose "<their words>"` so the engine's with-state dispatch above owns the flow. **The fast path:** when the request NAMES specific stages imperatively ("drop market-research and team-formation"), you may skip the composer pass (skip the `next compose` call too; if you already ran it, its dispatch print stands - follow it as it says): write the pending marker, present the same approve/edit/reject gate yourself as a structured question (listing the named flips and what the plan becomes), and on approve run `{{INVOKE}} engine recompose --skip <slugs> --add <slugs>` directly, then delete the marker. This is sound because the recompose verb IS the guard: it deterministically rejects starved, frozen, behind-cursor, and skeleton-gate flips no matter who calls it. Open-ended judgment-shaped requests ("what can we cut?") still run the composer pass. The gate is NEVER skipped on either path - fast means skipping the composer pass, never the human approval - the marker discipline is unchanged, and neither path runs under autonomous Construction.
...
The orchestration engine resolves scope-level stage routing internally (it reads the compiled scope grid the table below summarises). The summary table is kept here as human-readable data — not dispatch logic — and is regenerated, never hand-edited. (One carve-out: the composer pass APPENDS approved composed scopes to the runtime scope registry (`scopes/aidlc-<name>.md` + a `scope-grid.json` entry) - that is the sanctioned write path for composed scopes, not a hand-edit; this summary table itself stays generated.)

Source of truth: one file per scope under `.aidlc/scopes/aidlc-<name>.md` (identity + keywords + description + skeleton default) plus each stage's `scopes:` frontmatter (membership), transposed into the compiled grid at `bun .aidlc/tools/aidlc-graph.ts compile`. Adding a scope is the same muscle memory as authoring a sensor or agent — drop `.aidlc/scopes/aidlc-<name>.md`, tag the member stages' `scopes:` lists, recompile, then `{{INVOKE}} engine gen scope-table` to regenerate the table below + commit. CI runs `scope-table --check` to prevent drift.
...
## Harness notes (Devin Cloud)

- **No hook transport**: nothing in this tree intercepts tool calls. Enforcement is cooperative — engine calls verify state at every transition; `--doctor` scans for transitions that bypassed a gate checkpoint.
- **Session identity**: minted once per session by invoking `bun .aidlc/hooks/aidlc-session-start.ts` with a synthesized `session_id`; every later `--session`-accepting call reuses it.
- **Gates/questions**: structured questions render as blocking chat questions per `question-rendering.md`; a question ends the turn and the session natively waits for the human.
- **Tool names**: engine commands run via the shell (`bun .aidlc/tools/...`). There is no `ask_user_question`, `run_subagent`, or `read_subagent` on Cloud; dispatched topologies run inline.
- **No custom statusline**: run `--status` on demand for the phase/stage/progress/cost readout.
- **Method ambient context**: AGENTS.md carries the method pointer; AIDLC's stage resolver reads `aidlc/spaces/<space>/memory/` directly so stage correctness is unaffected.
- **MCP**: optional servers are templated in `.aidlc/mcp_config.json` (all disabled); on Cloud, MCP servers are connected through Customize → MCPs, not repo config.
...
- **Tri-mode interaction**: The user chooses guided, self-guided, or chat mode for answering questions; on Cloud all three render as blocking chat questions.
- **Audit trail**: All transitions are tool-owned and logged automatically via the engine's `report` step and the stage tools — never from prose. On this harness the audit trail is also the evidence the doctor checks for gate bypasses.
