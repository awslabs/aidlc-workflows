...
carried once in `intent_text`; the question echoes at most 240 characters plus
an ellipsis when truncated. With existing intents but no selected cursor,
...
question-supplied token commands must survive selection and composition.
...
verbatim, then re-run `next`. `claim` follows the Unit claim contract and
