...
// Claude Code harness).
//
// Kiro Crew note: this file is shared verbatim with the Kiro CLI harness. Kiro
// Crew drives `kiro-cli chat` under its gateway, so the hook payloads it
// delivers ARE Kiro CLI-shaped and this shim normalizes them unchanged. The
// difference is registration, not payload: Kiro Crew keeps its hooks in a
// GATEWAY-WATCHED store (~/.kiro/crew/hooks.json), so `aidlc config` wires
// these targets through Kiro Crew's hook API / a gateway reload rather than a
// passive per-session file read (the running gateway owns that store and
// resets a hand-added entry). The agent JSONs invoke this shim through the
// recognized `engine adapter kiro` route — core/tools/aidlc.ts resolves that
// route to aidlc-kiro-adapter.ts for every kiro-family harness, so the same
// dispatch serves Kiro Crew with no core route-table change.
//
...
// --- register-hooks: install the *.sh autoimport shims into ~/.kiro/hooks ---
//
// Kiro Crew's gateway rebuilds the agent config from its own sources and does
// NOT read a project's .kiro.hook manifests or agent-JSON hooks block, but it
// DOES autoimport executable *.sh from ~/.kiro/hooks (the default
// _DEFAULT_KIRO_HOOKS_DIR) into the agent-config hooks block the underlying
// kiro-cli fires. `aidlc config` lands the emitted shims in the PROJECT's
// .kiro/hooks/; this verb copies them into ~/.kiro/hooks so the gateway picks
// them up on its next (re)load. Explicit and idempotent: it only ever writes
// aidlc-*.sh shims, reports what it did, and never touches non-aidlc files or
// the gateway's own ~/.kiro/crew/hooks.json API store (which the gateway
// reconciles). The user restarts the gateway once after running it.
if (target === "register-hooks") {
  const srcDir = join(projectDir, ".kiro", "hooks");
  const destDir = join(homedir(), ".kiro", "hooks");
  let shims: string[];
  try {
    shims = readdirSync(srcDir).filter(
      (name) => name.startsWith("aidlc-") && name.endsWith(".sh"),
    );
  } catch {
    process.stderr.write(
      `register-hooks: no .kiro/hooks in ${projectDir} — run \`aidlc config --harness kirocrew\` first.\n`,
    );
    return 1;
  }
  if (shims.length === 0) {
    process.stderr.write(
      `register-hooks: no aidlc-*.sh shims in ${srcDir} — nothing to install.\n`,
    );
    return 1;
  }
  ...
  for (const name of shims) {
    ...
    chmodSync(to, 0o755); // autoimport skips non-executable files
    installed.push(name);
  }
  process.stdout.write(
    `register-hooks: installed ${installed.length} AI-DLC hook shim(s) into ${destDir}:\n` +
      installed.map((n) => `  - ${n}`).join("\n") +
      "\n\nRestart Kiro Crew once so the gateway autoimports them " +
      "(the gates fire on the next gateway load).\n",
  );
  return 0;
}
