...
description: >
  ...
  `.pi/scopes/`; run
  `bun .pi/tools/aidlc-utility.ts help` for the authoritative list
...
```
Loop:
  1. directive = `bun .pi/tools/aidlc-orchestrate.ts next $ARGUMENTS`
  2. act on directive.kind (see "Acting on a directive" below)
  3. `bun .pi/tools/aidlc-orchestrate.ts report --result <outcome> [--user-input "<text>"]`
...
| `ask` | Render `directive.question` via `ask_user_question`, then feed the human's answer back on the next `report` via `--user-input "<answer>"`. The engine never calls `ask_user_question` itself — it defers the human turn to you. |
...
Under **test-run mode** (the engine threads `--test-run` through; `report` rides it to the committing tool), gates auto-approve and the learnings ritual is skipped — there is no human in the loop. Pass `--test-run` through on `report` so the `GATE_APPROVED` row is stamped `Test-Run: true`.
...
Source of truth: one file per scope under `.pi/scopes/aidlc-<name>.md` (identity + keywords + description) plus each stage's `scopes:` frontmatter (membership), transposed into the compiled grid at `bun .pi/tools/aidlc-graph.ts compile`. Adding a scope is the same muscle memory as authoring a sensor or agent — drop `.pi/scopes/aidlc-<name>.md`, tag the member stages' `scopes:` lists, recompile, then `bun .pi/tools/aidlc-utility.ts scope-table` to regenerate the table below + commit. No prose edit required. CI runs `scope-table --check` to prevent drift.
...
- **Self-learning guardrails**: Human corrections can become persistent behavioural rules in `.pi/rules/` via the §13 learnings ritual.
