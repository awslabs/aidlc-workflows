...
- **Permissions**: the \`aidlc\` agent pre-approves project-relative \`bun .kiro/tools/aidlc-*\` tool calls, the dispatcher's engine routes (\`bun .kiro/tools/aidlc.ts engine …\`), \`date -u\`, writes under \`aidlc/spaces/\` and \`.kiro/sensors/\`, and spawning the framework's own personas; \`rm -rf\` and \`git push\` are denied outright, and everything else prompts — the dispatcher's other routes (\`config\`, \`compose\`, \`intent\`, \`space\`) included, deliberately. There is no blanket shell trust, and each delegated persona carries its own narrower allowlist. On the IDE, **Agent Autonomy** applies before any of this: Supervised mode prompts regardless of what is pre-approved here. Start Kiro from the project root so those relative tool paths resolve correctly. In \`--no-interactive\` runs, a command that would prompt is refused because no approver is present. \`--trust-all-tools\` bypasses the deny list too; use it only in a disposable sandbox.`,
