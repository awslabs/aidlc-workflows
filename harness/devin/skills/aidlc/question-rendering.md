...
Render every structured question as **numbered prose options in chat**. Devin
does expose a native picker (`ask_user_question`), but a picker selection returns
as a tool result rather than a submitted message, so it does not fire the trusted
`UserPromptSubmit` hook that records `HUMAN_TURN`. Calling the picker would
therefore make answer or approval logging refuse the selection and cause the
question to be asked again. While the session-selected workflow has valid
`Status: Running` state, the matcher-free PreToolUse guard denies the picker and
directs the model to render numbered prose in chat and end the turn. With no
running workflow, including completed or unusable state, the guard fails open so
the native picker remains available. Never treat a picker tool result as a human
turn.

Two Devin-specific reasons this is a correctness rule and not a presentation
preference. Both are properties of the picker itself, and both mean a
completion-triggered presence mint would be unsound here:

- **A picker question can be skipped without blocking progress.** Devin's CLI
  changelog (v3000.3.22) records that skipping some questions in an
  `ask_user_question` no longer blocks progress. The tool therefore *finishes*
  with no human choice made — so a completion-triggered presence mint would
  record a human turn that never happened.
- **Completed picker prompts are revertable and forkable.** Devin's `/steps`
  history (v3000.4.16) lets a user rewind past an answered prompt, while the
  AI-DLC audit ledger is append-only. Presence minted from a picker could
  therefore outlive the answer that justified it, and nothing can retract it.
