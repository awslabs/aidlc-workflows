...
- **Personal overrides**: Settings in \`~/.codex/config.toml\` merge over the project \`.codex/config.toml\`. Put machine-specific overrides (model, AWS profile/region, environment variables) there to avoid changing the shared project config.`,
    ...
    agents_note: `On Codex all 14 agent personas are transposed into \`.codex/agents/\` TOMLs (the conductor reads the persona \`.md\` bodies as prose); dispatched stage workers, ensemble collaborators, reviewers, and the composer run through Codex subagent roles.`,
