...
3. Continue into the normal birth: run `bun .codex/tools/aidlc-orchestrate.ts next --scope <name> "<original task text, verbatim>"` and act on its birth print exactly as "Acting on a directive" describes (the `--label` essence and the duplicate-intent guard ride the same path as any birth). Omit the quoted task argument only when the composition carried no task text (for example, a bare `--report`); never drop task text that the front composition received.
