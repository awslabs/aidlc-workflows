...
- **`for_each` names the iteration artifact.** The six Construction stages that run once per Unit declare `for_each: unit-of-work`; PR Integration is dormant until affirmed, while the other five follow their scope grid. Aggregation is inferred from the graph, not declared.
