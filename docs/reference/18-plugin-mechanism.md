...
**Activation (`when:`).** A stage may carry a structured `when:` predicate. `{producer-in-plan: X}` is schema-validated, parsed, compiled into `stage-graph.json`, and evaluated by the scope-grid pass: the stage is EXECUTE in a scope only when some stage that is itself EXECUTE in that same scope produces `X` (`produces` or `optional_produces`). Demotion iterates to a fixpoint, so a stage starved because its own producer was demoted is demoted too. A stage whose predicate is unsatisfied is plain `SKIP` in the compiled grid. A plugin's own stages exist only when the plugin is in the chosen set, so "is this plugin active" is already a compose-time fact.
