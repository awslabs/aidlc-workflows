...
> **Security note — Kiro's folder-drop has no install-time trust gate.** Claude and Codex mediate a plugin through their own trust prompts (managed marketplace / hash-pinned approval) *before* its hooks can run. Kiro has no plugin store, so the folder-drop path copies the plugin's files — its `compose.ts` and the compose manifest emitted for the target tree (a legacy `.kiro.hook` on `dist/kiro` and `dist/kiro-ide`, a standalone `hooks/*.json` manifest on `dist/kiro-unified`, whose runtime reads only that schema) — with **no equivalent gate**: dropping the tree *is* the trust decision. On Kiro the composer is invoked **explicitly** (the `bun compose.ts` call the harness chapters lead with); the documented drop lands the manifest at the project root rather than in `.kiro/hooks/`, so the drop alone does not arm a hook that runs on the next prompt. Treat a Kiro plugin drop like `git clone && run` all the same: only install plugins from a source you would run code from, review the diff the drop introduces, and pin the plugin repo to a reviewed tag rather than tracking a moving branch. The composer is additive and never edits `core/`, but you run it with your shell's privileges.
