...
- **Committed evidence** (authoritative for resolution): `<record>/construction/units/<unit>/<stage>/reviewed-source-<hash12>.tsv` inside the intent record, where `<hash12>` is the first 12 hex digits of the fingerprint. Because the record is committed, the evidence travels with every clone.
