...
2. Creates the record dir tree (idempotent -- skips existing directories/files): knowledge directories with READMEs, the `audit/` shard dir, stage artifact directories (empty), and verification directory.
