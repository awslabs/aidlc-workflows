...
| 3.5 Code Generation | subagent | `aidlc-developer-agent` twice per Unit: a planning dispatch (`AIDLC-PLANNING` marker) that writes the plan and unit-test instructions before Plan Approval, then the generation dispatch (`AIDLC-UNIT`/`AIDLC-STAGE` + `AIDLC-TESTING-CONTRACT` markers) that implements the approved plan and writes `code-summary.md`, `source-manifest.json`, and `traceability.json`; a Request Changes revision or a reviewer repair is another planning dispatch | aidlc-developer-agent | The developer owns the plan the human approves and the code that plan binds; the conductor presents the gate, records the receipts, and verifies the evidence, and the plan-approval guard refuses a conductor write to the plan once it is fingerprinted |
