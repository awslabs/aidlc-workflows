...
4. On a clear keyword match, emits a typed `scope-confirm` ask naming the effective ceremony from the compiled grid and workspace scan: `Starting a "[scope]" workflow for: "[text]" - [N] of [T] stages, [G] approval gates. Confirm to proceed, name a different scope, or say "compose" for a tailored plan.` The directive carries `response_route: "next"`, the proposed scope and original intent text, plus exact `confirm_command`, `compose_command`, and `scope_command_template` fields. Greenfield previews apply the same reverse-engineering skip as intent creation. A per-unit clause is appended only when the scope executes `units-generation` and its Construction stages fan out over the resulting Unit DAG.
5. On no match / rich prose, emits a typed `compose-offer` ask. Its `compose_command` routes the original intent through `next compose`, while `scope_command_template` routes a human-selected ready-made scope through `next --scope`; both preserve the intent after a literal `--` as one shell-safe argument. The offer's example scope list carries counts too (`express = 10 of 33 stages, classic = 26, feature = all 33`) so the magnitude difference is visible before choosing.
6. On confirmation, the conductor runs the ask's named `next` command instead of calling `report`. The explicit-scope path then stores the original freeform text as `Initial Intent` in `aidlc-state.md`.
