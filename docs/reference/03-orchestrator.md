...
The engine-driven per-unit loop for the design stages (3.1–3.4) and non-autonomous code-generation hands the conductor one uncovered unit per `next` (`directive.unit`, `gate: false` until every unit is built). On the default stage-major walk the conductor MAY process that directive as a **batch wave** instead of one unit at a time: it reads `bolt_dag.batches` from the intent's `runtime-graph.json`, takes the uncovered units of the batch containing `directive.unit`, and dispatches the stage body for all of them concurrently — one `Task` per unit in a single message, each running the stage's lead persona for that one unit, confined to `construction/<unit>/<stage>/` (no stage-level diary; the conductor appends one consolidated entry per wave). §12a reviewers run per wave unit at wave end — parallel foreground dispatches only where no reviewer-scope dispatch record is active; on enforcement harnesses the single-file record serializes them — and every verdict (including NOT-READY revision loops) settles before the single stage gate. This is safe because per-unit coverage is a stateless disk scan: the engine skips covered units on the next `next` regardless of how many landed in between. Waves never apply under `Construction Iteration: unit-major`, and a harness without a parallel dispatch primitive runs the serial loop unchanged. See stage-protocol.md §3 "Per-unit batch waves" for the contract.
