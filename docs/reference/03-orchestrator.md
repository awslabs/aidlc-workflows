...
3. Disambiguation rule: if the text contains BOTH a scope keyword AND a longer project description (more than 5 words), the match is treated as incidental and the COMPOSE OFFER fires instead of a silent default. Exception: an explicit mention — a scope name or keyword bound to `scope`/`plan`/`workflow` (`using the express scope`, `scope: poc`) — is flagged `explicit` (the audit `Source` stays `keyword`): it outranks any incidental keyword in the same text and takes Branch 7b's creation move (the `intent-create` print, whole sentence as `--arguments`) with no confirm, exactly like `--scope <name>`. The keyword confirm is for inference only.
