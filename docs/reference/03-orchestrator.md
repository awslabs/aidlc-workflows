...
The engine-driven per-unit loop for the design stages (3.1–3.4) and non-autonomous code-generation hands the conductor one uncovered unit per `next` (`directive.unit`, `gate: false` until every unit is built). On the default stage-major walk the conductor MAY widen that directive into a **batch wave** — but ONLY for the four inline design stages (3.1–3.4); code-generation (3.5, `workspace_requires: true`) is NEVER wave-eligible, because concurrent builders would collide writing into the shared workspace (the swarm path's per-unit worktrees exist for exactly this isolation) and because its Plan Approval step is a mandatory hard stop in every execution mode that cannot fold into a builder's return message. For an eligible stage, the conductor reads `bolt_dag.batches` from the intent's `runtime-graph.json`, takes the uncovered units of the batch containing `directive.unit` whose *kind* actually requires this stage's produces (via the stage's `produces_kinds` map against each unit's `bolt_dag.units[].kind`), resolves each sibling unit's own paths and produces from its own kind, and dispatches the stage body for all of them concurrently — one `Task` per unit in a single message, each running the stage's lead persona for that one unit, confined to `construction/<unit>/<stage>/` (no stage-level diary; the conductor appends one consolidated entry per wave, carrying every unit's memory notes verbatim). §12a reviewers run per wave unit at wave end — parallel foreground dispatches only where no reviewer-scope dispatch record is active; on enforcement harnesses the single-file record serializes them — and every verdict (including NOT-READY revision loops) settles before the single stage gate. This is safe because per-unit coverage is a stateless disk scan: the engine skips covered units on the next `next` regardless of how many landed in between. That scan proves artifact existence only, not review, so on crash re-entry the conductor runs the §12a reviewer step for any wave unit that reads as covered but carries no verdict yet, before presenting the gate. Waves never apply under `Construction Iteration: unit-major`, and a harness without a parallel dispatch primitive runs the serial loop unchanged. See stage-protocol.md §3 "Per-unit batch waves" for the contract.
