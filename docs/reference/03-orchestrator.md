...
5. On no match / rich prose, emits a typed `compose-offer` ask with `response_route: "next"`, `intent_text`, `compose_command`, and `scope_commands`. The composer estimates the task's implementation entropy and proposes the minimum viable EXECUTE/SKIP grid, human-gated (see below). The offer's example scope list carries counts too (`express = 10 of 33 stages, classic = 18, feature = all 33`) so the magnitude difference is visible before choosing.
6. On confirmation, the conductor runs the ask's complete shell-safe `next` command instead of calling `report`. Scope and compose commands carry `--pending-request <8hex id>`, never the request text. For a different scope, it runs the `scope_commands` entry whose `scope` matches the chosen plan; a name with no entry is not a valid scope. The engine resolves the token to the stored full description, which intent creation records as `Project` in `aidlc-state.md`.
7. If the user overrides the detected scope, uses the user's chosen scope instead.

The engine stores each request as `{id, description, proposedScope, createdAt}` in its own gitignored file under `aidlc/.aidlc-sessions/pending-requests/`, so concurrent sessions in one clone never share or invalidate each other's asks; re-running `next` for the same prose returns an equivalent ask with a fresh id, and the earlier id stays valid. A request that arrives with a live id keeps it across confirmation, routing questions, composer dispatch, and creation prints until intent creation claims it inside its workspace transaction, so one request creates at most one intent. Unanswered requests expire after seven days. A used id names the intent it created (run `next` to continue it); an expired or unknown id errors with a restate-the-request message. The claim journals the record about to be minted, and the registry identity (uuid) it will carry, before anything is exposed, and only a terminal receipt at the end of the creation transaction completes the request; a pending record whose journaled names are not a minted record name and a valid space is refused. A retry after an interrupted creation recovers it under the same lock: it undoes exactly the journaled record when that record holds nothing but creation and engine artifacts and its registry row, if any, carries the journaled identity (a record another request created under the planned name is left alone and the retry mints under the next free name; a record holding more is refused and left untouched), then mints again, so exactly one complete intent results. Every recovery read and write of the record, the space, and `intents.json` is reached through no symlinked path component, and the mutations run from inside the verified intents directory (the tool moves into it and proves it is the directory the checks inspected, so a parent swapped for a link afterwards aborts the rollback); a record that cannot be read is never treated as absent. An interrupted creation's journal does not expire until it is recovered or its record is archived, which settles it. The rollback drops the registry row before the directory, so a rollback that is itself interrupted is finished by the next retry. While a selected record's creation is interrupted, and that record still carries the journaled identity (or, for a route-free bare `next`, when a rollback left no record selected), `next` names the command that sets it up again, replaying the scope, label, and every explicit creation option (depth, test strategy, review, Guard Policy, ceremony switches, repos, space) in `--flag=value` form so a flag-like value stays one option. When the record cannot be set up again automatically, `next` names `intent archive <record>` instead, and a selected record left with only the creation stub and no journal gets the same per-record exit rather than workspace-wide archive advice. On POSIX the directory is owner-only (0700) and each record 0600, because records hold request text. On a legacy flat-layout (pre-workspace) project, a token-backed creation refuses before anything moves and keeps the request; the refusal names the one-time `intent create --scope <scope>` that moves the flat workflow into its own intent, after which the same command creates the request. A token-bearing command that meets a workflow which became active after the question was asked is routed as new work (the `new-work-routing` ask, proposing the token's scope), never as a scope change of that workflow. Conductors do not read or rewrite this runtime transport and must not substitute the preview for the full request.

If existing intents have no selected cursor and at least one record is selectable, confirming a scope with pending work emits `new-work-routing` on every harness rather than dropping the request into a picker. `new_work_description` contains the request's directions once, `proposed_scope` preserves the confirmed scope, `available_intents` lists exact selectors, `response_route` is `next`, and `numbered_prose_question` provides the engine-authored numbered rendering. The routes travel as fields, never inside the human-facing question: `new_intent_command` (separate new work with the proposed scope), `scope_commands` (the same for each valid scope, for a human-corrected scope), `compose_command` (reshape), and `select_commands` (one complete `next intent` command per selector), all carrying the same pending token. Continuation runs the chosen record's select command and follows its `print`; reshape runs it, then `compose_command`. Without pending work, the engine emits `intent-pick`: choose the `select_commands` entry whose `selector` matches the selected `available_intents` value and execute its `command` verbatim, never interpolate the selector into a command.
