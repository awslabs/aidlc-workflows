...
Pending requests live in `aidlc/.aidlc-sessions/pending-request.json`, alongside the per-person runtime cursor, as `{id, description, proposedScope, createdAt}`. A new request ask overwrites this slot. The engine preserves it across confirmation, routing questions, composer dispatch, and creation prints; only successful intent creation consumes it. Conductors do not read or rewrite this runtime transport and must not substitute the preview for the full request.

If existing intents have no selected cursor, confirming a scope with pending work emits `new-work-routing` on every harness rather than dropping the request into a picker. This ask retains its existing fields: `new_work_description` contains the full request once, `proposed_scope` preserves the confirmed scope, `available_intents` lists exact selectors, `response_route` is `next`, and `numbered_prose_question` provides the engine-authored numbered rendering. The question supplies complete shell-safe commands for separate new work and composition using the same pending token; use those commands, preserving the description and scope unless the human corrects them. Continuation selects the chosen listed intent and follows its `print`; reshape selects its target before resuming the pending composition request. Without pending work, the engine emits `intent-pick`: choose the `select_commands` entry whose `selector` matches the selected `available_intents` value and execute its `command` verbatim, never interpolate the selector into a command.
