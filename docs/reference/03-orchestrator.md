...
The engine stores each request as `{id, description, proposedScope, createdAt, origin}` (plus, for a `new-work-routing` ask, the records it asked about, and once claimed, the settings and record its creation used) in its own gitignored file under `aidlc/.aidlc-sessions/pending-requests/`, so concurrent sessions in one clone never share or invalidate each other's asks; re-running `next` for the same prose returns an equivalent ask with a fresh id, and the earlier id stays valid. On POSIX the directory is owner-only (0700) and each record 0600, because records hold request text; each mode is set through a descriptor opened without following links, and only after the path is re-checked and proven to still name that descriptor inside the project. Records owned by another account are ignored and never pruned. The request and its proposed scope travel through confirmation, composer dispatch, and creation prints; a `new-work-routing` ask mints its own request, bound to the space and records (by folder and uuid) it asked about; its compose route reshapes only one of those, and asks the routing question again when none is selected. Intent creation claims the id inside its workspace transaction, after every refusal and before anything is minted, so one request creates at most one intent, and completes it when the transaction finishes. The workflow state is creation's last durable write, so until it lands the record holds only its creation stub, which `next` refuses to route. Requests expire seven days after they are asked, claimed, or completed. A completed id names the intent it created (run `next` to continue it). An interrupted creation is never retried or undone automatically: the claimed id cannot be used again, its refusal names any record the interrupted run left (continue it with `next`, or, if its setup never finished, set it aside with `intent archive <record>`), and a fresh single-use `intent create` command with every setting that creation used (scope, label, depth, test strategy, review, guard policy, ceremony, repos, and space) creates the request again; a selected record left with only the creation stub gets the same per-record exit from `next` instead of workspace-wide archive advice. An expired or unknown id errors with a restate-the-request message. A token-bearing command that meets a workflow which became active after the question was asked is routed as new work (the `new-work-routing` ask, proposing the token's scope, which is validated), never as a scope change or reshape of that workflow; only a `new-work-routing` ask's own compose route reshapes existing work. On a legacy flat-layout (pre-workspace) project, a token-backed creation refuses before anything moves and keeps the request; the refusal names the one-time `intent create --scope <scope>` that moves the flat workflow into its own intent, after which the same command creates the request. Conductors do not read or rewrite this runtime transport and must not substitute the preview for the full request.
