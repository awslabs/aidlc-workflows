...
- **bun 1.3.14** -- The contributor/CI toolchain pin in `package.json` (`packageManager`). Required to build, package, test, directly run authored TypeScript sources, and use locally generated `dist/<harness>/` projections. Parser bundle regeneration refuses other Bun versions. Native release runtimes use the installed `aidlc` command. Install via `curl -fsSL https://bun.sh/install | bash`; on Windows use `powershell -c "irm bun.sh/install.ps1 | iex"`, then select the pinned version.
...
core/tools/vendor/   # Committed parser bundle + MIT notice inventory (regenerate only with the vendoring script)
...
scripts/vendor-markdown-parser.ts # Pinned-Bun regeneration of the committed parser dependency
...
`core/tools/vendor/markdown-parser.js` bundles exact-pinned development
dependencies `micromark@4.0.2` + `micromark-extension-gfm@3.0.0` into a
self-contained Bun ESM file. The header contains every bundled package's MIT
notice; `core/tools/vendor/LICENSES.md` inventories them. Commit this generated
dependency so core source, packaged copy, and compiled native execution all use
the same parser without runtime package installation.

Use `markdownBlocks` in `core/tools/aidlc-lib.ts` for block visibility,
containers, link reference definitions, and claim splitting. Its
`visibleMarkdownLines` projection preserves consumer-specific visibility
options. Do not restore a hand-rolled scanner or reference-definition grammar.
`Bun.markdown.render` review-authority rendering is a deliberate separate path;
do not fold that security boundary into the block adapter.

Regenerate only through the dedicated script, never by hand-editing the bundle
or rebuilding it in the packager:

```bash
# Requires the pinned Bun 1.3.14 and installed development dependencies.
bun scripts/vendor-markdown-parser.ts
bun test --timeout 120000 tests/unit/t340-vendored-markdown-parser.test.ts
bun scripts/package.ts
```

The script asserts Bun **1.3.14** and agreement with `package.json`'s
`packageManager` before writing. `t340` rebuilds and compares bytes under the
pinned version; on another Bun it explicitly skips parity rather than accepting
different generated bytes. CI runs the pinned version. For a Bun upgrade,
update the script constant, `packageManager`, and CI pins together. For parser
upgrades, update the exact dependency pins and lockfile. In either case,
regenerate the bundle, reconcile `core/tools/vendor/LICENSES.md` with its header,
and run the [consumer and compatibility coverage](09-testing.md#vendored-markdown-parser-coverage).
The normal packager copies `tools/vendor/` into every harness's `dist/` and
`dist-release/`; native compilation follows the relative bundle dependency.
