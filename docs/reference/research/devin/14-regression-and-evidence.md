...
| `tests/fixtures/devin-hook-payloads/captured-3000.10.31.json` with `capture-provenance-3000.10.31.json` | Sanitized 2026-09-19 re-capture on 3000.10.31: background launch/terminal/repeated-read strings, Stop timing (a Stop fires while a background child is in-flight; a second Stop follows the completion notification under the same prompt_id), denied-tool child reads as `completed`, `resume` field, child Stops entering the parent stream, `SubagentStop` proven not loadable (poisons hooks.v1.json); C13/C15 interactive-only BLOCKED | Interactive cancel/Ctrl+B behavior; structured child identity; an `exited with an error` read capture (that string is binary-pinned only) |
...
| `evidence/devin-e2e-run/background-lifecycle-run/` | Attended 2026-09-19 run on Devin CLI 3000.10.31 at `10b71cb5` plus the uncommitted Item 3 change: background `subagent_explore` launch with no audit row, Stop allowed mid-flight by the pending-subagent carve-out (drops line between launch ack and read), one `SUBAGENT_COMPLETED` at the read naming profile and agent id, repeated read adding nothing, foreground developer completion isolated, full `hello.py` workflow to `WORKFLOW_COMPLETED`, doctor 58/0; SHA-256 manifest; dispatch evidence from `sessions.db` (export covers only preamble + post-compaction tail) | A failing/cancelled child; interactive cancel or Ctrl+B; the express graph dispatches no background support on its own, so the background agent was user-prompted (Track B) |
