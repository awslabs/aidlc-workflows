...
| `evidence/devin-e2e-run/` (removed; raw transcripts no longer ship — see `tests/fixtures/devin-hook-payloads/PROVENANCE.md`) | Historical headless and interactive workflow runs: first-run disclosed permissive mode and manually created approval evidence; second/third runs were blocked at Plan Approval; fourth-run session exports informed the native question schema/response fixtures | Genuine native human approval or complete delegated workflow acceptance; a blanket all-topology PASS; an independent raw hook capture for every tool result |
