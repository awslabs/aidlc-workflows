...
The same fact is reported twice on a Devin project: the shared machine-section `Harness CLI: devin …` row (warn when missing or below floor; the shared floor in `HARNESS_CLI.devin.minimumVersion`) and the Devin-specific `devin CLI …` row (hard fail, controls the exit code). The split is deliberate as of the PR #996 Item 7 review — the Devin-specific row adds Desktop fallback discovery, a bounded timeout, and distinct error classes — but it means a missing host CLI is a red doctor on Devin where Claude reports only the shared warn. Retained as policy; revisit only as an explicit owner decision.
...
| Discovery | PATH precedes Desktop; checked source/path is reported without claiming Desktop execution; Windows candidates match an observed install, macOS/Linux unverified | t334 discovery table |
...
| Do the Linux candidates `~/.local/share/Devin/devin` and `/opt/Devin/devin` ever exist? | Docs mention Devin Desktop on Windows/Linux via Command Palette (`Ctrl+Shift+P`); no Linux install observed | A Linux Devin Desktop install to inspect, or vendor confirmation |
