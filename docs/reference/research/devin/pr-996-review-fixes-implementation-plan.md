...
2. **Baseline update (2026-09-12):** the floor is now the shared `3000.10.21` support baseline in `core/tools/aidlc-devin-version.ts`. The earlier proposal to raise the minimum to `3000.3.22` — documenting the exit-code-2/stderr blocking compatibility requirement — is superseded historical context, as are the earlier release fixes it cited.
