...
The SDK trace also records each turn's isolated Claude config directory and hook responses (event, outcome, exit code, stdout, and stderr). Use the `Stop` responses to distinguish a hook-forced continuation from the model continuing on its own. Set `AIDLC_KEEP_TEMP=1` to retain the isolated config directories and their persisted session transcripts after a diagnostic run.
