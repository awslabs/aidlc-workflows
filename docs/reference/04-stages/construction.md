...
Aggregate inputs for every Unit under `<record>/construction/units/`. For a
zero-Unit scope, read the stage-level equivalents directly under
`<record>/construction/` instead.
...
- Every applicable artifact under each Unit's
  `<record>/construction/units/<unit>/nfr-requirements/` and
  `<record>/construction/units/<unit>/nfr-design/` directories
...
| support_agents    | (none)                                                                                            |
| mode              | inline                                                                                            |
| Inputs            | Code generation output from Stage 3.5 and infrastructure design from Stage 3.4 for every Unit under `<record>/construction/units/`, build/test results from Stage 3.6 |
...
This stage aggregates per-unit inputs for every Unit under `<record>/construction/units/`:

- Build/test results from `<record>/construction/build-and-test/`
- Code summaries from `<record>/construction/units/<unit>/code-generation/`
  (if they exist)
- Infrastructure design from `<record>/construction/units/<unit>/infrastructure-design/`
  (if it exists)
- Workspace profile for existing CI configuration

### Steps

1. **Load Prior Context** -- Read build/test results, aggregate code summaries
   and infrastructure design for every Unit under `<record>/construction/units/`
   (if they exist), and read the workspace profile for existing CI configuration.
