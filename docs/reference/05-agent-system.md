...
| `balanced` | `model: sonnet`, no `effort:` line | `model = "openai.gpt-5.4"`, no effort key | field OMITTED (#601 -- see below) | no tier entry | `model: amazon-bedrock/global.anthropic.claude-sonnet-4-6`, no variant key |
| `templated` | `model: sonnet`, `effort: medium` | `model = "openai.gpt-5.4"`, `model_reasoning_effort = "medium"` | field OMITTED (#601 -- see below) | no tier entry | same model, `variant: medium` |
