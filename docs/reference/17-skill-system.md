...
| `load-steering` | Yes | Apply `rules_content` in order, retain it as the active stage's rule bundle, and immediately execute `continue_command`. Keep `continue_token` only as recovery and correlation data. Do not report or narrate chunk progress. |
...
- **The transitions `report` commits** - the workflow / phase / stage machines and the 87-event audit taxonomy. See [State Machine](12-state-machine.md).
