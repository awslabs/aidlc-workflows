...
|--------|----------------|--------------------------|
| `print` | Yes | Do exactly what `directive.message` says — it is authoritative. Three shapes: **terminal** (names a read-only utility such as status/help/doctor/version to run, or carries the completed typed-config output; print the actual output verbatim, STOP), **run-then-continue** (names a mutating tool such as a scope change, a jump `execute`, or fresh-workspace `intent-create`; run it, then return to step 1), and **run-then-stop** (a confirmed `--new-intent` creation; run `intent-create`, STOP, and hand off using the harness-specific fresh-session flow without re-running `next`). For workflow dispatches, the mutation lives in the named tool; explicit typed config requests execute their canonical config tool within `next`. |
...
**Pending request transport.** Scope/compose asks carry the request's directions once in `intent_text` (a terminal pasted `<document>` block stays in the pending store as data and never enters an ask); their `question` includes only a preview of at most 240 characters, ending in `...` when truncated. A `new-work-routing` ask carries them once in `new_work_description` and its route commands in dedicated fields. The engine stores each request as `{id, description, proposedScope, createdAt, origin}` (plus, for a `new-work-routing` ask, the records it asked about, and once claimed, the settings and record its creation used) in its own gitignored file under `aidlc/.aidlc-sessions/pending-requests/`, so concurrent sessions in one clone never share or invalidate each other's asks; re-running `next` for the same prose returns an equivalent ask with a fresh id, and the earlier id stays valid. Commands resolve `--pending-request <8hex id>` from that store; the full request and proposed scope travel through composer dispatch and creation prints (a `new-work-routing` ask, including the second hop after `scope-confirm`, mints its own, and its compose route reshapes only a record it asked about) until intent creation claims the id inside its workspace transaction, so one request creates at most one intent. Requests expire seven days after they are asked, claimed, or completed. A used id names the intent it created (run `next` to continue it); an interrupted creation is never retried or undone automatically, and its refusal names any record it left and a fresh single-use `intent create` command with the same settings; an expired or unknown id errors with a restate-the-request message. Records are owner-only on POSIX. Conductors execute the supplied commands without appending request text or editing the runtime files.
...
`$ARGUMENTS` passes through to the first `next` verbatim — the engine parses the flags (`--status`, `--stage`, `--scope`, `--depth`, freeform text), so the conductor never pre-parses or strips them. Workflow routing does not execute stage work: `report` commits the transition, so the next `next` reads fresh state. Explicit typed config requests finish their canonical config operation before returning a terminal response and do not advance the workflow. Request asks may maintain local pending-request transport without changing workflow state or approval authority.
...
These seven `SWARM_*` events are part of the 105-event audit taxonomy (see [State Machine](12-state-machine.md)). On an exit-2 envelope the conductor takes the baton back - failure always halts and re-engages the human regardless of autonomy mode.
...
- **The transitions `report` commits** - the workflow / phase / stage machines and the 105-event audit taxonomy. See [State Machine](12-state-machine.md).
