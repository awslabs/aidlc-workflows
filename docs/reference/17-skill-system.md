...
|------------|------|-------|
| `prepare --batch <n> --units <a,b,c> [--base <branch>] [--degraded-from <subagent\|ultracode>] [--resume-existing]` | Protected Code Generation, under legacy autonomy or new checkpoints, requires each Unit’s current Plan Approval plus committed, reproducible approved parent application source. Read-only preflight covers every Unit before any initial child is created; uncommitted source returns commit-and-retry without an orphan, and no commit is automatic. A rejected revision uses `--resume-existing` after fresh Plan Approval: preserve surviving child source and archive metadata, or recreate a removed child from already-landed parent source when native landing evidence permits it. | `SWARM_STARTED` (plus `SWARM_DEGRADED` when a loud downgrade is reported). |
...
- **The transitions `report` commits** - the workflow / phase / stage machines and the 102-event audit taxonomy.
