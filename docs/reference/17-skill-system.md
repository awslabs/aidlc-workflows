...
Terminal workspace routing also stays outside workflow engagement on the transcript path: `next space-create teamB` can finish without a Stop-hook continuation into the active workflow. The classifier uses the shared workspace grammar. Intent creation remains workflow work, and a genuine workflow command later in the turn still requires normal forwarding.
