...
| `ask` | Yes | Every engine ask carries `ask_type` and `response_route`; render `directive.question`, then follow that route and the named command fields. `scope-confirm` carries `proposed_scope`, `intent_text`, `confirm_command`, `compose_command`, and `scope_command_template`; `compose-offer` carries `intent_text`, `compose_command`, and `scope_command_template`; `intent-pick` carries exact `available_intents` selectors and `select_command_template`; `unit-paused` carries `stage`, `unit`, and `resume_command`. A `"next"` route runs the command named for the chosen answer and never calls `report`; a `"command"` route runs the named command and then re-runs `next`. `new-work-routing` keeps its direct `next` route fields, `unit-claim` keeps its `"claim"` flow, and legacy Plan Approval recovery keeps its bare-`next` choice. The prompt-rendered resume menu is the only `"report"` route and uses `report --result resumed --user-input "<choice>"`. The engine never asks the human itself; it defers the turn to the conductor. |
...
These seven `SWARM_*` events are part of the 91-event audit taxonomy (see [State Machine](12-state-machine.md)). On an exit-2 envelope the conductor takes the baton back - failure always halts and re-engages the human regardless of autonomy mode.
...
- **The transitions `report` commits** - the workflow / phase / stage machines and the 91-event audit taxonomy. See [State Machine](12-state-machine.md).
