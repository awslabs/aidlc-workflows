...
| `ask` | Yes | Render `directive.question` via the harness question tool. A cold-start scope-confirm ask has no state transition to report, so re-run `next` with the original task plus `compose` or `--scope <scope>`. For asks on an existing workflow, feed the answer back on `report` via `--user-input`. The engine never calls the question tool itself — it defers the human turn to the conductor. |
