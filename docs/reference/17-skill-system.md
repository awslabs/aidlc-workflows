...
**Pending request transport.** Scope/compose asks carry the full request once in `intent_text`; their `question` includes only a preview of at most 240 characters, ending in `...` when truncated. A `new-work-routing` ask carries the full text once in `new_work_description` and its route commands in dedicated fields. The engine stores each request as `{id, description, proposedScope, createdAt}` in its own gitignored file under `aidlc/.aidlc-sessions/pending-requests/`, so concurrent sessions in one clone never invalidate each other's asks, and asking again for the same request and scope returns the same id. Commands resolve `--pending-request <8hex id>` from that store, and a request that arrives with a live id keeps it, preserving the full request and proposed scope from `scope-confirm` to `new-work-routing` when existing intents have no selected cursor, through composer dispatch, and through creation prints until successful intent creation consumes it. Unanswered requests expire after seven days; a consumed or expired id errors with a restate-the-request message. Conductors execute the supplied commands without appending request text or editing the runtime files.
