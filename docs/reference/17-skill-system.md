...
**Pending request transport.** Scope/compose asks carry the full request once in `intent_text`; their `question` includes only a preview of at most 240 characters plus an ellipsis when truncated. A `new-work-routing` ask carries the full text once in its existing `new_work_description` field, not in new command fields. The engine stores `{id, description, proposedScope, createdAt}` in `aidlc/.aidlc-sessions/pending-request.json` alongside the per-person cursor. A new request ask overwrites the slot. Commands resolve `--pending-request <8hex id>` from that store, preserving the full request and proposed scope through `scope-confirm` → `new-work-routing` when existing intents have no selected cursor, through composer dispatch, and through creation prints. Only successful intent creation consumes the slot. Conductors execute the supplied commands without appending request text or editing the runtime file.
