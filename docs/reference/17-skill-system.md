...
1. **Identity** lives in one file per scope at `dist/claude/.claude/scopes/aidlc-<name>.md` — frontmatter (`name`, `depth`, `keywords`, `description`, optional `runner`) plus prose describing the scope. The shipped set is `bugfix`, `discovery`, `enterprise`, `feature`, `infra`, `mvp`, `poc`, `refactor`, `security-patch`, `workshop`.
