...
**Pending request transport.** Scope/compose asks carry the request's directions once in `intent_text` (a terminal pasted `<document>` block stays in the pending store as data and never enters an ask); their `question` includes only a preview of at most 240 characters, ending in `...` when truncated. A `new-work-routing` ask carries them once in `new_work_description` and its route commands in dedicated fields. The engine stores each request as `{id, description, proposedScope, createdAt}` in its own gitignored file under `aidlc/.aidlc-sessions/pending-requests/`, so concurrent sessions in one clone never share or invalidate each other's asks; re-running `next` for the same prose returns an equivalent ask with a fresh id, and the earlier id stays valid. Commands resolve `--pending-request <8hex id>` from that store, and a request that arrives with a live id keeps it, preserving the full request and proposed scope from `scope-confirm` to `new-work-routing` when existing intents have no selected cursor, through composer dispatch, and through creation prints until intent creation claims it inside its workspace transaction, so one request creates at most one intent. Unanswered requests expire after seven days. A used id names the intent it created (run `next` to continue it); an expired or unknown id errors with a restate-the-request message. Records are owner-only on POSIX. Conductors execute the supplied commands without appending request text or editing the runtime files.
