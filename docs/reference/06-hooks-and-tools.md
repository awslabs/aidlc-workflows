...
3. **Compose the engine:** Runs `bun .claude/tools/aidlc-orchestrate.ts next --project-dir <dir>` and parses the directive `kind` plus the bounded `message` carried by an error directive. It does not re-derive state — it composes the engine.
...
8. **`error` -> deliver once:** The hook fingerprints the active intent UUID, session ID, exact state digest, stage, and bounded message. A new fingerprint is persisted under `<record>/.aidlc-stop-hook/`, best-effort emits `ERROR_LOGGED`, and blocks once with the exact engine message quoted as a diagnostic. The same fingerprint on the next Stop allows the turn; a changed message or state digest receives one new delivery. Fingerprint I/O fails open, and audit failure never changes the decision.
...
12. **Fail open:** Any unexpected failure (unreadable state, an engine that exits non-zero or returns no parseable directive, malformed stdin) allows the stop and records a drop. Failing open is the only safe failure mode for a hook that can otherwise trap a turn.
