...
6. **Pending → block and inject:** For any other (pending) directive — `run-stage`, `dispatch-subagent`, `invoke-swarm`, `present-gate`, `ask`, `print`, `error` — it prints `{"decision":"block","reason":<on-task continuation>}`, so the same session resumes with the next move injected.
7. **Fail open:** Any unexpected failure (unreadable state, an engine that exits non-zero or returns no parseable directive, malformed stdin) allows the stop and records a drop. Failing open is the only safe failure mode for a hook that can otherwise trap a turn.
...
**Test-run events:** The `--test-run` flag on emitting paths (`aidlc-orchestrate.ts report --stage <slug> --result approved --test-run`, `aidlc-log.ts answer --test-run`, `aidlc-jump.ts execute --test-run`) tags the canonical event with `Test-Run: true`. There is no separate auto-event category — the tag preserves observability without taxonomy bloat.
