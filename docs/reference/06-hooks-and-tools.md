...
> **Registration is a harness parameter.** This chapter documents the Claude Code expression (`settings.json`). On Cursor the same hooks register in `.cursor/hooks.json` (version 1, camelCase events), which routes every event through one authored adapter — `.cursor/hooks/aidlc-cursor-adapter.ts` — to the core hook targets: `sessionStart`→session-start (the context re-keyed to `{"additional_context"}`), `sessionEnd`→session-end, `beforeSubmitPrompt`→mint (`HUMAN_TURN`), `afterFileEdit`→audit-and-sensors, `afterShellExecution`→shell-sync (ONE adapter process fanning to runtime-compile then state-sync audit-tail), `subagentStop`→log-subagent, `preCompact`→validate-state, `stop`→stop (adapter payload contract pinned by `t222`). No statusline is wired on Cursor (use `/aidlc --status`); the two Cursor deltas — the Stop hook's `followup_message` translation and the unwired reviewer-scope hook — are covered in those hooks' sections below.
...
**Cursor translation — same discipline, different mechanics.** Cursor's `stop` hook cannot hard-block a turn, so on that harness the adapter (`.cursor/hooks/aidlc-cursor-adapter.ts`) translates this hook's `{"decision":"block","reason":…}` into Cursor's `{"followup_message": reason}`, which the harness auto-submits as the next user message. The auto-submission carries its own ceiling — `loop_limit: 12` in `.cursor/hooks.json` — set above the core hook's run-mode-aware caps (2 interactive / 8 autonomous), so the core caps govern release.
