...
| `scope-change` | Re-plan which stages execute and apply any of the seven setting flags in one atomic update. A same-scope request still applies settings. Scope-owned Change Control/ceremony rows follow new defaults; human overrides and absent legacy rows are preserved. Memory-enforced strict keeps the stored Change Control row on implicit scope changes. | `SCOPE_CHANGED` when scope changes, plus changed-setting events |
