...
**`--source-file` and `--heading-file` exist for the same reason, and on the onboard path none of these is optional — `--source-file` on `capture` as well as `persist-rule`.** A `source_path` read back from the capture ledger is a *captured filename*, so it is as attacker-influenced as the document text: a customer folder may contain `policy'; touch pwned; #.md`, and wrapping it in single quotes does not help — the filename closes the quote itself. Single quotes are not an encoding. Any path or document text travels by file — there is **no trusted-path exception**. Who supplied the value is irrelevant; what matters is that a POSIX filename may legally contain `'`, which no quoting neutralises. `capture` therefore takes `--source-file` too, and `persist-rule` takes `--heading-file` — the routed heading is document-derived (the skill picks it from what the material names), so it is no safer to interpolate than the rule text. Every value the skill derives from a document has a file transport; only `--scope`, a closed two-value enum, is safe to type. The bare `--heading` is additionally shape-checked (`^(## )?[A-Za-z0-9][A-Za-z0-9 ._\-&/()]*$`).
...
- **The ledger is committed, so rows store the captured path RELATIVE to the onboard dir** and resolve it against the current checkout at read time; `source_path` is provenance only and is never resolved. A clone, move, or worktree at a different path still classifies.
