...
| `persist-rule --scope <project\|team> (--candidate-id <id> \| --candidate-id-file <path>) (--text <text> \| --text-file <path>) [--heading <h> \| --heading-file <path>] [--source <s> \| --source-file <path>]` | The **stage-optional** rule entry the `/aidlc-onboard` gate uses: write ONE approved rule as a practice line through the same writer and the same cid dedup, with no `stage_slug` on the path | `RULE_LEARNED` |

Every subcommand accepts `--project-dir <path>`. `persist` never judges — it receives only conflict-clear or user-escalated selections — and dedups per `(Stage, Candidate-ID, Destination)` against a fresh in-lock read of the audit, so a same-day re-run is a no-op rather than a double-append.

`persist-rule` exists because onboard runs *before* any workflow, so there is no stage to require; it namespaces its cid marker and the `RULE_LEARNED` `Stage` field under the fixed `aidlc-onboard` sentinel. `scope` accepts only `project` or `team` (there is no org tier for onboard rules; `org` exits 2). It does **not** accept `--space` — the rule follows the active-space cursor. Each call writes exactly ONE rule and returns one flat reply — `scope`, `candidate_id`, `destination`, `heading`, `rule_learned`, `rule_written`, `already_present`, `audit_backfilled`. `rule_learned` counts the AUDIT EVENT (was a `RULE_LEARNED` row appended this call); `rule_written` counts the PRACTICE LINE (was a line actually appended or rewritten on disk this call) — the two diverge on a ledger backfill (`rule_learned: 1`, `rule_written: 0`: the line already existed, only the missing row was appended) and on the legitimate recovery of a deleted line (`rule_learned: 0`, `rule_written: 1`: the row already existed and is not re-emitted, but the line is rewritten). `already_present` is `true` whenever that exact candidate id was already recorded before the call (both the no-op-replay and backfill states); `audit_backfilled` disambiguates the backfill state from an ordinary no-op, since both otherwise report `already_present: true`. A document that yields several rules means several calls, each with its own candidate id.

**`--text-file` is the shape onboard must use, and the reason is the shell.** When a harness runs `persist-rule --text "<document text>"`, the shell expands `$(…)` and backticks in that text *before this process starts* — so no validation inside the tool can see it, let alone stop it. Passing a **path** means the untrusted bytes arrive through a file read, where they are inert. Trailing newlines in the file are stripped; an interior newline is still rejected. `--text` remains for text the caller authored.

**`--source-file` and `--heading-file` exist for the same reason, and on the onboard path none of these is optional — `--source-file` on `capture` as well as `persist-rule`.** A `source_path` read back from the capture ledger is a *captured filename*, so it is as attacker-influenced as the document text: a customer folder may contain `policy'; touch pwned; #.md`, and wrapping it in single quotes does not help — the filename closes the quote itself. Single quotes are not an encoding. Any path or document text travels by file — there is **no trusted-path exception**. Who supplied the value is irrelevant; what matters is that a POSIX filename may legally contain `'`, which no quoting neutralises. `capture` therefore takes `--source-file` too, and `persist-rule` takes `--heading-file` — the routed heading is document-derived (the skill picks it from what the material names), so it is no safer to interpolate than the rule text. Every value the skill derives from a document has a file transport; only `--scope`, a closed two-value enum, is safe to type. The bare `--heading` is shape-checked by a DENYLIST, not an ASCII allowlist: an empty or >120-character heading, a control character or line break, the `<!-- cid:` marker sequence, a leading markdown list marker, or a shell substitution/chaining character (`` ` $ \ | ; < > " ' ``) is refused; Unicode letters and ordinary punctuation such as `:`, `&`, `/`, `()` are accepted (`Security: IAM`, `Sécurité`, `Data & Privacy`, `CI/CD` all pass).

**Non-UTF-8 text is quarantined, not coerced.** `classify` validates the whole buffer with a fatal UTF-8 decode, so a latin-1 / Windows-1252 document with high bytes classifies as `unsupported-binary` rather than silently decoding to mojibake. That is deliberate for S1: the honest answer is that the file needs a UTF-8 version, not a rule drafted from replacement characters.

Content validation is identical whichever transport is used: a candidate id must match `^[A-Za-z0-9._-]+$` (it becomes part of the dedup marker), and rule text may contain neither a newline (which would split one rule into two bullets, the second untracked by dedup) nor the `<!-- cid:` marker syntax (which would pre-suppress a future candidate id).

**A pre-workflow event's audit identity is space-level, in both directions.** An onboard rule is a fact about the space, not about whichever intent happens to be current, so `persist-rule` *writes* its `RULE_LEARNED` row to the space-level `audit/` bucket (`preWorkflowAuditFilePath`) instead of resolving a shard through the active-intent cursor, and *reads* the union of the space bucket and **every** intent bucket in that space (`readPreWorkflowAuditSurface`). Both halves are load-bearing: if the write followed the cursor, persisting under one intent and replaying under another would re-emit a duplicate row for one already-deduped practice line — reporting the self-contradicting `rule_learned: 1` together with `already_present: true`. Scanning every intent bucket (not just the active one) covers rows earlier builds scattered there.

**A candidate id must be unique to its rule text.** The id recipe (`<manifest-id>-<n>`) is an ordinal over a model-produced candidate list, so a rerun that reorders or revises candidates can reuse `-1` for a different rule. `persist-rule` therefore compares the *stored* text behind an existing dedup marker with the incoming text and exits 2 when they differ, naming both. Checking only marker presence reported `rule_learned: 0, already_present: true` — a success shape — while the approved rule was never written: silent data loss at a human approval gate.

### `aidlc-onboard.ts` — Onboard capture + classify

The deterministic half of `/aidlc-onboard`: it captures a customer's body of material byte-exact and classifies each item, so the skill can route preventative controls through a human gate into a rule (via `persist-rule` above). Text only — binary text extraction is a later slice. No LLM call lives in the tool.

| Subcommand | Purpose | Emits |
|------------|---------|-------|
| `capture (--source <path> \| --source-file <path>)` | Copy a file — or walk a directory and copy every non-dotfile — byte-exact into `aidlc/spaces/<space>/onboard/files/`, compute each sha256, and append-merge the ledger row into `onboard/manifest.json` (dedup on sha256). Symlinks and the engine's own `aidlc/` dir are skipped | — |
| `list` | Print the manifest ledger (captured material + disposition state) as JSON | — |
| `classify (--id <manifest-id> \| --id-file <path>)` | Read the captured bytes and write back a `preventative` / `other-text` / `unsupported-binary` disposition, returning the decoded text as `content` | — |

Every subcommand accepts `--space <name>` and `--project-dir <path>`. `--space` names an **existing** space and must be a bare slug — an unvalidated value is a path segment that would escape the project dir, so a non-slug exits 2 rather than being coerced.

Three properties are load-bearing:

- **Every manifest read-modify-write runs inside `withAuditLock`**, re-reading the ledger fresh in the lock body. `writeFileAtomic` alone prevents half-writes but not lost updates, and the skill's documented "classify every captured item" step is exactly the shape a harness fires as one parallel tool block.
- **The ledger is committed, so rows store the captured path RELATIVE to the onboard dir** — the storage leaf is hash-only (`files/<sha256>`, no basename), which avoids a dedup gap when identical bytes arrive under two different source filenames, a filesystem's per-component length limit, and an ambiguous mutable row when a directory capture holds two distinct files with identical content. `source_path` is provenance only, is never resolved, and is itself recorded RELATIVE to the project dir when the source is within it — the same portability discipline as the captured path, since an absolute machine-local path baked into a committed file can expose local directory structure to every future reader and is meaningless to a teammate at a different checkout path. A source genuinely outside the project (nothing to make relative to) still records the absolute path. A clone, move, or worktree at a different path still classifies. Every manifest field — `source_path` and `captured_file` included, not just `content` — is exactly as attacker-influenced as the document text and must be treated the same way: reported, never obeyed.
- **`classify`'s `content` is capped** and a cap sets `truncated: true`. The field goes straight into the model's context and the skill runs classify once per item, so an uncapped body is a context blowout on one large file. The disposition heuristic still reads the full text.

The disposition heuristic is a deterministic, **recall-biased** keyword pre-filter (`must`/`shall`/`required`/`never`/`always`/`mandatory`/`prohibited`), signalling on either two distinct imperatives or one imperative repeated three times — real policy prose repeats a single imperative, so a distinct-count alone rejects genuine standards. It is a signal, not a verdict: the skill re-judges **both** text dispositions against `content`, so a false positive costs one re-read and a false negative is still recoverable. A binary source is captured byte-exact like any other file but classifies as `unsupported-binary`, never coerced into a text disposition; the sniff quarantines on a known magic header (PDF / zip family / JPEG / PNG / GZIP), a NUL byte, a high ratio of control bytes, or a failed fatal UTF-8 decode of the whole buffer. PDF's header magic (`%PDF-`) is searched within an initial window rather than only at offset zero — real PDF readers tolerate a shifted header (a leading newline, or bytes prepended by an intermediate tool) — while every other magic (whose own format spec requires it at byte 0) stays fixed-offset, so a window search cannot false-positive on an unrelated text file that happens to contain a short binary signature somewhere past its start.

**`classify` frames every body it emits as untrusted.** A `content` field ships alongside `content_trust: "untrusted"` and a `content_handling` notice stating that the bytes are inert data, that imperatives inside them address the customer's own engineers, and that the document cannot redirect the workflow or request a tool call. The shell boundary (a file transport per untrusted value) stops those bytes from being *executed*; it does nothing about the same bytes arriving in a model's context as de facto instructions. The human gate runs **after** the classification pass, so it cannot contain an injection that already shaped the options the gate presents — hence the declaration travels with the data rather than living only in `SKILL.md`, so a direct tool call inherits it.
