...
| `plan-approval-guard.ts` | PreToolUse | Project-wide (settings.json) | `Task\|Agent\|Edit\|Write\|Bash` (plus harness-native patch aliases) | **Flow-altering.** Enforce code-generation's plan-before-generation ordering (stage Steps 2-4) and its plan authorship deterministically. The active directive selects one authority: `construction/<unit>/code-generation/` when `unit` is present, otherwise zero-Unit `construction/code-generation/`. A developer PLANNING dispatch (exactly one `AIDLC-PLANNING: <unit>` or `AIDLC-PLANNING: code-generation` marker, no generation marker, a target the directive planned) is admitted without approval evidence, never starts generation, and opens the planning dispatch record (`<record>/.aidlc-planning-dispatch.json`, one at a time, removed by the SubagentStop hook, stale after one hour) that confines every mutation to that target's record directory while it is live. Developer GENERATION dispatch and workspace mutation are refused until the target has a current Testing Contract, fingerprinted plan/instructions, and explicit "Approve Plan" answer; writes inside the selected record directory remain available for the questions file and diary, but once an `[Approval Fingerprint]` is recorded (or while planning is live) only the developer agent may write `code-generation-plan.md` and `unit-test-instructions.md`: a main-session write (no `agent_type`) is refused, and a Kiro IDE payload (`agent_identity_unavailable`) is not judged. Generation delegation uses exactly one `AIDLC-UNIT: <unit>` or `AIDLC-STAGE: code-generation` marker. Each refusal emits `PLAN_APPROVAL_BLOCKED`; missing, conflicting, or unknown markers block instead of guessing from prompt prose. `AIDLC_DISABLE_PLAN_APPROVAL_GUARD=1` disables this PreToolUse hook only; it does **not** disable the autonomous `aidlc-swarm.ts prepare` precondition, and protected Plan Approval authority remains enforced by the owning decision, answer, and generation-start tools. |
...
| `log-subagent.ts` | SubagentStop | Project-wide (settings.json) | (empty) | Remove one background-subagent ledger entry for the completing session, close the session's Code Generation planning dispatch record when the finishing agent is the developer agent, and log subagent completion events |
...
8. **Log entry:** Appends a canonical `ARTIFACT_CREATED` (for Write to a net-new path) or `ARTIFACT_UPDATED` (for Edit, or Write overwriting existing) event via `appendAuditEntry`. Fields: Timestamp, Event, Tool, File, Context, and `Summary Authorization Id` when the written stage (and Unit, for a per-Unit Construction path) has an active summary confirmation. The id is read from `<record>/.aidlc-summary-authorization/<stage>/<unit or stage-level>.json`, which `aidlc-log.ts answer --checkpoint summary-confirmation` writes on `Looks correct` and removes on `Request changes`. A per-Unit path with no confirmation of its own is stamped from the stage-level record only when that record belongs to the isolated `single-stage:<stage>` run (which confirms its one Unit at stage level); a main-workflow stage-level confirmation never authorizes a Unit's outputs. Stage completion asks whether every required output's newest write carries the id of the current confirmation (descent), not whether it landed after the receipt (order); a legacy receipt without an id still uses the order check. That descent check is a governed Change Control read: under `strict` an output saved without the current confirmation refuses (`SUMMARY_ARTIFACT_UNAUTHORIZED`, save the document again); under `relaxed` the gate or completion continues, writes one `CHANGE_ACCEPTED` row for the output, and tells the human once (`<output> was saved without the current summary confirmation. Continuing (Change Control: relaxed).`). The confirmation and its receipt are untouched either way.
9. **Attribution:** Adds three optional fields, each derived deterministically and omitted (never written empty) when it cannot be derived, so rows that carry none are byte-identical to earlier releases. `Role` is the dispatched agent's `agent_type` when the hook payload names one, else the state file's `Active Agent` (which `aidlc-state.ts` sets at stage start). `Mode` is the compiled graph node's `mode` for the state file's `Current Stage`. `Actor` is `subagent:<agent_type>` when the payload names an agent, `main-session` when it names none on a harness whose payloads carry identity (Claude Code and Codex natively; the Kiro CLI through each worker agent's scoped `audit-and-sensors <agent>` registration; Cursor, opencode, and Copilot through their delegation ledgers), and absent when the payload declares `agent_identity_unavailable` (Kiro IDE, whose payloads carry no agent identity; an ambiguous Cursor ledger). None of the three reads prompt text or model output.
...
**Planning dispatch.** The developer agent authors the plan, so the guard admits a developer dispatch that carries exactly one `AIDLC-PLANNING: <unit>` line (or `AIDLC-PLANNING: code-generation` for zero-Unit work) and no generation marker WITHOUT approval evidence, provided the target is one the active directive planned: the run-stage's `unit` (or stage level), or a member of an `invoke-swarm` batch's `units`. A brief that mixes a planning marker with `AIDLC-UNIT`, `AIDLC-STAGE`, or `AIDLC-TESTING-CONTRACT` is refused, as is an ambiguous or unplanned planning target. An admitted planning dispatch never calls the generation-start path (`beginCodeGeneration`), so the receipt is untouched and no `CHANGE_ACCEPTED` row is written. Admission writes `<record>/.aidlc-planning-dispatch.json` (`{version, agent, stage, sessionId, target, startedAtMs}`) under the audit lock; one planning dispatch is live at a time, and a second is refused with the live target named until the first returns. `aidlc-log-subagent.ts` removes the record on the SubagentStop of that session's developer dispatch; a record older than one hour is an orphan from a crashed session and is ignored and removed on the next read. While the record is fresh, every mutation the guard sees, from any writer, must land inside the planned target's code-generation record directory: a workspace path, a sibling target, a path outside the project, or an opaque shell command is refused with the planning confinement reason. Framework tool invocations (`bun <harness>/tools/aidlc-*.ts`) and read-only commands pass as before, so the worker can run `aidlc-testing-posture.ts render`.

**Plan authorship.** Only the developer agent writes `code-generation-plan.md` and `unit-test-instructions.md` once a target has an author. Concretely: while the planning record is live, and after the target's questions file records a non-blank `[Approval Fingerprint]` tag (including after a `Request Changes` answer, whose revision is a fresh planning dispatch), a Write/Edit/MultiEdit or inspected shell redirection to either file is refused unless the payload's `agent_type` is `aidlc-developer-agent`. A payload without `agent_type` is the main session (the conductor) on Claude Code and Codex; the Kiro CLI adapter forwards each worker agent's own name from its scoped `plan-approval-guard <agent>` registration, so the conductor's registration in `aidlc.json` reads as the main session and the developer worker as itself; Cursor, opencode, and Copilot correlate the name from their delegation ledgers. A payload that declares `agent_identity_unavailable` (Kiro IDE, whose payloads carry no agent identity) is not judged by this rule; there the stage prose alone carries it. The questions file, the fingerprint and `[Planned Source]` tags, the `[Answer]:` line, `code-summary.md`, `source-manifest.json`, `traceability.json`, and the diary are unaffected. The refusal names the remedy: re-dispatch the developer agent with the planning marker and the human's feedback, then re-fingerprint and re-present.
...
| `plan-approval-guard.ts` | `PLAN_APPROVAL_BLOCKED` | A code-generation developer generation dispatch refused before the plan is approved, a workspace mutation refused before approval or during a live planning dispatch, a planning dispatch refused (mixed or unplanned markers, or another planning dispatch live), or a non-developer write to the plan or instructions refused once the plan is fingerprinted (PreToolUse) |
