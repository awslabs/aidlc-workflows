...
All immutable reads and source-bound Git operations use the shared `immutableGit` invoker, which disables replacement objects with both the first Git argument `--no-replace-objects` and `GIT_NO_REPLACE_OBJECTS=1` so replacement refs cannot substitute unreviewed commits or blobs.
The write barrier examines complete shell words plus attached `key=value` path operands and single-dash attached values for known writers, treating unresolved values (including variable or substitution expansions) as `unprovable`.
A source-bound merge refuses before target mutation if its combined-tree proof is unavailable or fails, directing the caller to upgrade Git for `merge-tree --write-tree`, resolve merge conflicts, or use `AIDLC_SKIP_SOURCE_FRESHNESS=1` only with human approval.
