...
repo-scoped `ARTIFACT_REUSED` row exempts a reused repo, and isolated `--single`
link rows never satisfy the main workflow. Bypass with
...
| `ARTIFACT_REUSED` | `tools/aidlc-state.ts` | `reuse-artifact` subcommand — keep/modify/redo decisions; optional `Repo` scopes a current-attempt pipeline reuse exemption to one registered repo |
