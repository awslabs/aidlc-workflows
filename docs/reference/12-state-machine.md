...
| `REVIEW_UI_FEEDBACK` | `tools/aidlc-orchestrate.ts` | Browser feedback consumed by an approved or rejected report; records Stage, optional Unit, Revision, Result, Files, and the concatenated-body Digest. It follows `GATE_REJECTED` on rejection and carries approved bodies forward as non-blocking `approval_notes`. |
| `REVIEW_UI_RESPONSES` | `tools/aidlc-orchestrate.ts` | The agent's per-remark replies (`responses-NNN.md`, one `applied` / `kept` / `answered` line per remark id) recorded by `report --result revised --responses`; records Stage, optional Unit, Revision, File, and Remarks count. Written after the revise transition and before the manifest is published. |
