...
| Event | Emitter | Trigger |
|---|---|---|
| `WORKTREE_CREATED` | `tools/aidlc-worktree.ts` | Audit-first per-Bolt creation records the immutable Base commit and `Base Source Listing` hash over the raw-aware source listing computed before `git worktree add`; swarm prepare also stamps intent/Unit/batch/stage/floor provenance (subcommand: `create`) |
