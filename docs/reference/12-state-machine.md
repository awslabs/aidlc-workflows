...
outside `aidlc/` and the harness dir. A failure writes nothing. Optional outputs do not participate. For
...
reverse engineering requires a complete chain per registered repo. Bypass with
...
| `PIPELINE_LINK_COMPLETED` | `tools/aidlc-log.ts` | Fires after one declared pipeline link returns. Carries `Stage`, `Link`, and `Position k/N`; multi-repo chains also carry `Repo`. The tool refuses undeclared, duplicate, or out-of-order current-attempt links. Pipeline gate-start, approval, advance, finalize, and workflow completion require every current-attempt link receipt. |
