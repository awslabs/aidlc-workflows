...
| `PLAN_APPROVAL_RECORDED` | `tools/aidlc-log.ts` | Human-backed Code Generation plan receipt. The authority it records binds to intent, stage or Unit target, stage attempt (run floor), content fingerprint (the projected plan and instructions plus the Testing Contract hash), prompt, session response, and questions digest, never to the identity of the directive that presented the question nor to row order. The row also carries the directive epoch as provenance, which is recorded and never compared. Protected runtime state is the authority; this row is provenance only. |
