...
**Source-freshness binding (#629/#646).** On a `workspace_requires` stage, the reviewer's terminal receipt carries a `Source Fingerprint` of the workspace source the reviewer inspected. `approve`, `advance`, `finalize`, and `complete-workflow` compare the newest source fingerprint carried by a receipt with a syntactically valid `Artifact Fingerprint`; current artifact equality independently controls whether that receipt satisfies review existence/cardinality, so source freshness never makes a stale artifact receipt valid. An already-completed-stage recovery may proceed when receipts are absent, but any valid recorded source fingerprint must still match even when declared artifacts also changed, so partial-approval recovery remains fail-closed while fresh-clone/moved-record recovery remains possible. A newly stamped `unbindable` value also refuses; only a legacy receipt without the field keeps the migration fail-open. `AIDLC_SKIP_SOURCE_FRESHNESS=1` turns the feature off at all four routes and at `aidlc-swarm.ts finalize`. A swarm convergence finalized under the switch is explicitly marked: merge it with the same switch present, or discard the Bolt and redo the unit from prepare through review/finalize. See the `REVIEW_COMPLETED` row below for what the comparison does and does not prove.
...
| `REVIEW_COMPLETED` | `tools/aidlc-log.ts` | Fires when a `READY` or `NOT-READY` reviewer verdict is read and records an `Artifact Fingerprint` over the declared output paths and bytes. All completing state transitions (`approve`, `advance`, `finalize`, and `complete-workflow`) require a matching receipt from the current workflow attempt whose artifact fingerprint still matches; per-unit stages require one per applicable unit and scope invalidation to that unit. Autonomous swarm finalization additionally requires each configured unit's matching receipt after its Bolt started and every applicable required artifact to exist as a file in that Bolt worktree; absent optional outputs remain valid fingerprint entries. For a `workspace_requires` stage (code-generation), the same receipt also carries a `Source Fingerprint` (#629/#646), a git-native content hash of the workspace source the reviewer inspected. Completion routes compare the newest recorded source fingerprint with the current tree; a settled autonomous swarm performs that comparison in each unit's worktree during finalization. Legacy receipts without the source field keep the pre-#629 migration behavior. Bypass with `AIDLC_SKIP_SOURCE_FRESHNESS=1`. |
...
| `SWARM_UNIT_CONVERGED` | `tools/aidlc-swarm.ts` | A swarm Unit re-verified green and untampered, with its newest configured post-Bolt reviewer receipt's `Artifact Fingerprint` matching its required artifacts and, for a `workspace_requires` stage, its `Source Fingerprint` matching the unit's own worktree (#646), then merged back. A bound convergence carries the immutable `Source Commit` that the later source merge must recheck and merge instead of the movable Bolt branch; the commit remains retained until merge or discard. A bypassed convergence carries `Source Freshness Bypass: true` instead; its source merge requires `AIDLC_SKIP_SOURCE_FRESHNESS=1` again and refuses uncommitted or ignored application paths not represented by the Bolt branch. A unit whose merge-back failed gets no convergence row until a finalize retry succeeds. |
