...
> - **`dist/<harness>/…`** -- an **ignored local generated** source/development and manual-copy projection (`dist/claude/.claude/`, `dist/kiro/.kiro/`, `dist/kiro-ide/.kiro/`, `dist/codex/`, `dist/opencode/`, `dist/copilot/`). Never hand-edited or committed; materialized by `bun scripts/package.ts`. Releases carry it under `runtime/<harness>/` inside the Bun-shaped `aidlc-runtime-X.Y.Z.tar.gz`.
> - **`dist-release/<harness>/…`** -- the corresponding ignored native projection. Releases carry it under `runtime/<harness>/` inside `aidlc-native-runtime-X.Y.Z.tar.gz`, which the native installers and lifecycle commands consume.
...
| [Hooks and Tools](06-hooks-and-tools.md) | Hook system, CLI tools, 98-event audit taxonomy |
...
| [State Machine](12-state-machine.md) | Workflow / phase / stage machines, 98-event taxonomy, audit-first rules |
