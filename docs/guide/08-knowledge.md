...
persists it with `summarize <id>`, bound to the revision it describes. Edit the
original without re-summarizing and the summary reports `invalidated` and is
...
omitting it leaves the existing tags alone.
