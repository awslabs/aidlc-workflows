...
A useful rule of thumb: **if a human reviewer would reject a stage's output when the rule is violated, it belongs in `.claude/rules/`.** If they would use the rule as background context when reviewing, it is knowledge.
