...
Core ships 11 named scopes. Each scope defines a stage set, a default depth level, and a default Guard Policy value (strict on `enterprise`, `security-patch`, and `infra`; off on `express`; relaxed on the rest; see [Guard Policy](13-customization.md#guard-policy) for what the value does and how to set it). Plugin installs can add more scopes, and an install can narrow which plugin scopes are visible with `aidlc engine plugin select <names>`. When a `plugins` selection disables core (`aidlc` omitted), the core scope files remain installed but are not valid runtime scopes until core is re-enabled; the Initialization stages still run for every enabled scope.
...
**Settings at the gate:** besides the stage grid, a front/report proposal names the plan's Guard Policy and its four scope settings (`sensors`, `learnings`, `summary_confirmation`, and `review_cap`), each group on its own row with a one-line reason. That lets a composed plan trim the ceremony inside its stages as well as the stages themselves: a small, well-understood fix might run with sensors and learnings off and reviews capped at advisory. A matched proposal shows the stock scope's own values; a CUSTOM scope stores the approved values in its frontmatter. You can flip any of them before approving, and flipping one on a matched proposal turns it into a custom scope. No setting removes an approval gate. Mid-workflow these are per-intent switches (`/aidlc --sensors off` and the rest, see [Ceremony Switches](13-customization.md#ceremony-switches)), not part of a recompose; `--review` there only lowers reviews, so lifting a scope's review cap mid-workflow takes a scope change that also clears any earlier lowering (`/aidlc --scope <name> --review adversarial`).
...
**In-flight recompose:** mid-workflow, `/aidlc compose` proposes re-shaping the PENDING stages of the running workflow - skip what you no longer need, add back a pending stage you realize you need. The composer re-estimates the entropy components from what completed stages actually resolved, so each proposed flip names the evidence that moved the score ("feasibility settled the integration questions - risk re-scores MED"). Flips apply only to pending, ahead-of-cursor stages (completed and in-progress stages are frozen), are strict-validated so no remaining stage is starved of a required input, and land through the deterministic `recompose` verb under the audit lock with a `RECOMPOSED` audit event. The first EXECUTE stage of Construction cannot be flipped by recompose. This protects the stage-routing anchor; it does not make a first design-stage review proof of a working integrated skeleton.
