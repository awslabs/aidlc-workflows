...
- When the compose request included task text, the same-turn birth carries that text verbatim (`next --scope <name> "<original task text>"`) so the state Project field and intent-record slug preserve the description. A taskless composition such as bare `--report` omits the text argument.
