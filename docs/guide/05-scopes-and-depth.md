...
**Disambiguation rule:** If your input contains both a scope keyword and a longer project description (more than 5 words), the match is treated as incidental and the compose offer fires instead (below). This prevents mismatches like "Fix the infrastructure monitoring dashboard" being routed to `infra` when a tailored plan is more appropriate. Naming the plan explicitly — `<name> scope`, `<name> plan`, `<name> workflow`, or `scope <name>` — is never incidental: "create a todo application using the express scope" starts the `express` workflow directly, with no confirmation, exactly as `--scope express` would; the whole sentence becomes the intent's description. It outranks an incidental keyword elsewhere in the text ("fix the login bug with the mvp plan" is `mvp`). The confirmation question is reserved for inferred matches (a short input that merely contains a keyword).
