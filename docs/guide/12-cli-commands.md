...
| Step | What happens |
|------|--------------|
| Capture | `aidlc-onboard.ts capture --source-file <path>` (the path itself written to a scratch file, never placed on a command line) copies the file (or every file under a walked directory) byte-exact, records its sha256, and append-merges it into a capture manifest at `aidlc/spaces/<active-space>/onboard/manifest.json`. Re-capturing identical bytes updates the existing ledger row instead of duplicating it. The stored copy is named by its digest alone (`files/<sha256>`), so the same bytes captured under two filenames are one committed file and a long source filename cannot overflow the path limit; the original name is kept as the row's `source_path`, recorded relative to the project when the source is inside it so the committed ledger carries no username or private directory names. Symlinks and the engine's own `aidlc/` directory are skipped. The target space resolves from the active-space pointer and defaults to `default` — onboard never stands up a new space for this. |
| Classify | `aidlc-onboard.ts classify --id-file <path>` (the manifest id written to a scratch file) reads a captured item's text and emits a `preventative` / `other-text` / `unsupported-binary` signal, plus the text itself (capped — a cap sets `truncated: true`). The signal is a keyword pre-filter the skill re-judges in both directions, so an `other-text` item that plainly is a standard still reaches the gate. This slice is **text only** — PDF and binary extraction ship in a later release; a binary source is still captured byte-exact, it just can't be classified yet. A recognised binary is quarantined on its magic bytes even when they do not sit at offset zero (a PDF whose header follows a leading newline is still a PDF), so a binary payload cannot be handed back as document text. Text must be **valid UTF-8**: a latin-1 / Windows-1252 document classifies as `unsupported-binary` rather than being decoded to mojibake, so re-save it as UTF-8 if a prose file is unexpectedly quarantined. |
...
| Promote | Each confirmed candidate's id, text, and the item's `source_path` are written to scratch files and handed to `aidlc-learnings.ts persist-rule --candidate-id-file <path> --text-file <path> --source-file <path> --heading-file <path>` — the candidate id, the text, the captured filename and the routed heading are all derived from the customer's document or the committed manifest, so none of them ever reaches a command line. It writes the line into `aidlc/spaces/<active-space>/memory/{project,team}.md` — the exact same file the learning loop writes to — and emits `RULE_LEARNED` on a fresh write. The reply reports `rule_learned` (was an audit event appended) and `rule_written` (was a practice line appended) separately, because the two legitimately diverge — a ledger backfill is `1`/`0`, recovering a deleted line is `0`/`1`. A file rather than a `--candidate-id`/`--text` argument because both came from a customer document or a committed ledger, and a shell would expand `$(…)` inside them before the tool could inspect it. The rules take effect on the *next* compile, same as a learning-loop rule. |
