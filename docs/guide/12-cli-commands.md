...
Validate that all of this implementation's prerequisites, configuration, and stage-graph integrity are in place. Exits 0 on full pass, 1 on any failure; the full report writes to stdout in both cases so the orchestrator surfaces it either way. Core doctor checks are **read-only** with respect to the project — on a fresh shell with no intent yet (no `audit/` shards) they create no files there, so the command is safe to run before the first intent is created; once an intent exists it records a `HEALTH_CHECKED` audit row. One narrow exception lives outside the project: with `AIDLC_REVIEW_UI=1`, `AIDLC_REVIEW_STRICT=1`, and a live daemon, the `review-ui` row mints a single-use browser link, which writes one nonce file under `~/.aidlc/review-ui/<project-id>/nonces/` (or the `AIDLC_REVIEW_HOME` equivalent — the same thing `/aidlc --status` does). In the default mode the row prints the daemon's origin and writes nothing. Plugin checks execute installed plugin code: plugin authors are required by convention to keep those scripts read-only, but the runtime cannot enforce that property.
