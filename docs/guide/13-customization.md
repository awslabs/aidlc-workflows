...
3. **The intent.** The intent stores the scope-derived value. Chat can raise it to `strict` and can turn a fence `on`, but cannot lower the policy or a fence: supported hook payloads do not authenticate whether a person or a model supplied their text. Configure `relaxed` or `off` in the scope before creating or changing the intent, or use a harness environment kill switch set outside the agent where one exists. A sole retired `Change Control: relaxed|off` field is normalized automatically without changing its value or writing a policy audit row.
