...
Shipped agents are authored with a `tier:` (`judgment` | `balanced` | `templated`) that the build projects into each harness's native model/effort keys — judgment agents inherit your session's model and effort, and balanced and templated agents pin a mid-size model at medium effort (on Claude Code, Codex, and opencode; on Kiro all tiers inherit the session model). See [Agent System](../reference/05-agent-system.md) for the full projection table.
