...
> + Starlark rules, and Cursor in `.cursor/hooks.json` + `.cursor/rules/aidlc.mdc`
> + `.cursor/cli.json` — see [Running on Kiro CLI](harnesses/kiro-cli.md),
> [Running on Codex CLI](harnesses/codex-cli.md), and
> [Running on Cursor](harnesses/cursor.md) for each harness's surfaces.
...
On **Claude Code**, this implementation displays a statusline in the terminal status bar showing workflow progress. Kiro, Codex, and Cursor have no statusline — they surface workflow position through `/aidlc --status` (Kiro and Cursor) and the `update_plan` task-progress item plus `$aidlc --status` (Codex):
