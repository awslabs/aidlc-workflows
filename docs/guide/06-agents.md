...
<!-- Text fallback: The SKILL.md conductor adopts each stage's lead and support agents inline on the 29 inline stages and dispatches agents only where a stage's mode calls for it: aidlc-developer-agent (Code Generation planning and generation, the Reverse Engineering scan, the Practices Discovery and User Stories ensembles, and Deployment Execution migrations), aidlc-architect-agent (the Reverse Engineering synthesis link), aidlc-pipeline-deploy-agent (Practices Discovery lead), aidlc-quality-agent and aidlc-devsecops-agent (Practices Discovery spokes; aidlc-quality-agent and aidlc-design-agent also as User Stories mob supports), the two reviewer agents, and the composer. aidlc-product-agent, aidlc-delivery-agent, aidlc-aws-platform-agent, aidlc-compliance-agent, and aidlc-operations-agent are adopted inline and never dispatched. Key flows: aidlc-product-agent sends requirements/stories to aidlc-architect-agent, who sends specs to aidlc-developer-agent. aidlc-developer-agent sends code to aidlc-quality-agent, who sends test results back. aidlc-aws-platform-agent provisions infrastructure for aidlc-pipeline-deploy-agent, who deploys for aidlc-operations-agent. The feedback loop: aidlc-operations-agent sends operational insights back to aidlc-product-agent, closing the cycle. -->
