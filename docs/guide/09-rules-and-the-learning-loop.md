...
A confirmed learning *is* a practice: it lands in the same active-space memory files (`aidlc/spaces/<active-space>/memory/project.md`, `team.md`) that practices-discovery affirms — there is no separate rolling `*-learnings.md` surface. The two paths into those files differ by lifecycle: practices-discovery affirms a whole section deterministically, while the learning loop appends one dated, topically-headed entry at a time through the gate. The date rides inside the entry's own dedup marker rather than in the rule text, so the visible line is exactly the practice you approved.

Two properties of that append are worth knowing, because both are about not losing a rule you already confirmed. A **candidate id must be unique to its rule text**: it is the dedup key, so re-running a selection with the same id but revised text is refused, naming both texts, rather than reported as an idempotent no-op that quietly keeps the old rule. And a **batch of selections commits whole or not at all** — if one selection is refused, no earlier selection in that batch is left half-applied with a ledger row but no practice line. Re-running an unchanged selection, including one persisted by an earlier release of AI-DLC, is always a safe no-op.
