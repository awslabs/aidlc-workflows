...
Rules live as Markdown files in the active space memory layer at `aidlc/spaces/<active-space>/memory/` — a single hand-editable set at the workspace root, read by every harness via its native include (Devin names them for the agent to open; Claude `@`-import stub, Kiro CLI resources or IDE steering, Codex `AIDLC_RULES_DIR`, opencode `instructions` glob, Copilot `AGENTS.md` `@`-imports). Each file is named for its scope:
