...
| `Providers` shows `[needs]` with `no recorded answers` although the harness already has its own model access | On Kiro the row reads `[ok]` and `aidlc config providers` asks nothing, because model access comes with Kiro. On every other harness the row is genuine: answer `amazon-bedrock`, or choose `keep current` (`--provider current`) to preserve and record the provider already configured for the harness. `--provider other --acknowledge` records a different provider you configured yourself. |
...
| `cannot coexist in one project` | The second harness shares an engine directory (`kiro` uses `.kiro` for both its IDE and CLI surfaces; `opencode` / `copilot` use `.aidlc`) or an exclusive managed block (Copilot's `AGENTS.md`). Choose distinct engine directories and avoid exclusive blocks. The neutral `AGENTS.md` block is shared across Kiro CLI, Kiro IDE, Codex, Cursor, and OpenCode; `.gitignore` combines shipped entries. |
...
Every match is a warning. Kiro CLI and IDE share one conductor, and only the
IDE applies these rules: when AI-DLC runs in Kiro IDE, its agent cannot read the
files a rule hides, while Kiro CLI ignores the global files and uses a workspace
`.kiroignore` only to filter search results. Doctor cannot tell which of the two
runs the project, so it warns rather than failing a CLI-only setup. Project
`.gitignore` and `.kiroignore` matches also depend on the IDE setting
`kiroAgent.agentIgnoreFiles`, which doctor cannot read.
...
On Kiro, it also checks each ignore source independently for rules hiding `.kiro/`, naming the file and line. Matches warn: when AI-DLC runs in Kiro IDE, its agent cannot read the files they hide, but Kiro CLI does not apply them and doctor cannot tell which of the two runs the project; workspace-source matches also depend on the `kiroAgent.agentIgnoreFiles` setting. See [Kiro IDE Read Denials](#kiro-ide-read-denials).
