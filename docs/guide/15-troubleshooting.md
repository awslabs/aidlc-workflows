...
| `host inventory unavailable` from a plugin command | Restore the Claude/Codex host registry, or run sync from a session injecting the plugin root for manual installs. Storeless harnesses installed through `aidlc plugin install` use `<harness-dir>/plugins/<name>/` as full inventory. Missing or malformed inventory is never proof that pruning is safe. |
...
| Plugin install exits `5` after checksum verification | On Claude/Codex, run the printed native marketplace/install commands and approve the host trust prompt. This is an intentional handoff, not a completed host install. |
| Plugin install reports a catalog digest mismatch (exit `4`) | Nothing was installed. Ask the marketplace maintainer to regenerate and review the catalog/archive pairing; do not bypass verification. |
| Marketplace is blocked by machine allowlist | Ask the administrator responsible for `<install-root>/aidlc.settings.json`; project/local registrations cannot widen `plugins.allowedMarketplaces`. |
| Plugin list reports `superseded by core v<version>` | Upgrade core to that version or later, then follow the plugin's migration/removal note. Removing a marketplace registration does not uninstall its plugins. |
