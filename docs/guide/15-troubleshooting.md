...
| `predates shared onboarding` | Refresh the named installed harness with `aidlc config --harness <name>` before adding another harness that shares the neutral `AGENTS.md` block. `--force` does not bypass this compatibility check. |
