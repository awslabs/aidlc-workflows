...
| `refreshing project files needs release source bytes` on a copy-channel project | A Bun-invoking projection has no installed runtime to refresh from. Rerun the same command with `--from <the runtime/<name>/ root you copied from, or a checkout's dist/<name>/ tree>`; the root refresh and `config project` both accept it. Or install the native `aidlc` command and rerun without `--from`. The record-only sections (`models`, `providers`, `trust`, `flags`) never need a runtime. |
