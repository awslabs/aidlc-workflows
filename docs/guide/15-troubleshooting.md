...
| `predates shared onboarding` | The named installed harness is older than the selected release (or has no numeric recorded version) and its block is not shared. Refresh it with `aidlc config --harness <name>` before adding another harness that shares the neutral `AGENTS.md` block. Current exclusive blocks, including Copilot's, instead report `cannot coexist in one project`. `--force` does not bypass this compatibility check. |
...
| `has no readable projection descriptor` | Repair the named installed harness with `aidlc config --harness <name>` before adding or refreshing another harness. |
