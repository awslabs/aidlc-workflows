...
| `predates shared onboarding` | The named installed harness is older than the selected release (or has no valid recorded version) and its block is not shared. A preview sorts before the stable release with the same base version. Refresh it with `aidlc config --harness <name>` before adding another harness that shares the neutral `AGENTS.md` block. Current exclusive blocks, including Copilot's, instead report `cannot coexist in one project`. `--force` does not bypass this compatibility check. |
...
| `has no readable projection descriptor` when adding a harness | Repair the named installed harness with `aidlc config --harness <name>` before adding another harness. Refreshes of existing harnesses remain possible, so if several descriptors are missing, restore them one harness at a time. |
