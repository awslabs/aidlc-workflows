...
those workspace files apply. The Kiro CLI and IDE share one conductor, so a global
match fails only when the project shows it is opened in Kiro IDE (a workspace
`.vscode/settings.json` with a `kiroAgent` setting); otherwise it warns, because
the CLI does not apply these rules.
...
On Kiro IDE, it also checks each ignore source independently for rules hiding `.kiro/`, naming the file and line. Global-source matches fail when the project shows it is opened in Kiro IDE (a workspace `.vscode/settings.json` with a `kiroAgent` setting) and warn otherwise, because the Kiro CLI shares the conductor but does not apply these rules; workspace-source matches warn because `kiroAgent.agentIgnoreFiles` governs whether they apply. See [Kiro IDE Read Denials](#kiro-ide-read-denials).
