...
| `has no readable projection descriptor` | Repair the named installed harness with `aidlc config --harness <name>` before adding another harness. An existing harness may refresh only when the sibling's baseline does not show co-ownership of a non-union managed block. If the refusal says `co-owns AGENTS.md`, restore the named sibling's descriptor first; `--force` cannot bypass this guard. Legacy trees with no baseline evidence of co-ownership can restore their descriptors one harness at a time. |
