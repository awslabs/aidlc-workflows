...
| `.gitignore` shows `preserve (owned by <harness>)` (`action: "preserve"`, `detail: "owned by <harness>"` in `aidlc config --dry-run --json`) | This is not an error: the first-installed harness owns the shared block, and the invoking harness's own ignore entries are not added automatically. If needed, add them by hand outside the block: Claude's `.claude/settings.local.json`; Kiro / Kiro IDE's `aidlc/.aidlc-turn-counter` and `aidlc/.aidlc-readonly-latch`; Cursor's `aidlc/.aidlc-cursor-subagent-*.json` and `aidlc/.aidlc-cursor-subagents/`. |
