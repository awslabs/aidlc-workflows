...
| `has no readable projection descriptor` | Repair the named installed harness with `aidlc config --harness <name>` before adding another harness. For co-owned blocks, a same-release refresh is allowed when the source declares the block shared and leaves the current block unchanged, matching the sibling's baseline, even when that sibling's descriptor is missing; this allows both missing descriptors to be repaired one harness at a time. Otherwise, a `co-owns AGENTS.md` refusal requires restoring the named sibling's descriptor first; `--force` cannot bypass this guard. Legacy trees without baseline evidence of co-ownership can also restore descriptors one harness at a time. A sibling that has lost both its descriptor and its baseline is treated as a legacy tree during another harness's refresh; refresh that sibling first when it shares the root block. |
