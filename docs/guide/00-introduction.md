...
AI-DLC (AI-Driven Development Life Cycle) is a methodology for structuring AI-assisted software development into repeatable, traceable phases. It originated from the [AWS AI-DLC methodology](https://aws.amazon.com/blogs/devops/ai-driven-development-life-cycle/). This repository implements it natively from one harness-neutral core, so it runs inside the agent harness you already use — today Claude Code, Kiro (IDE and CLI), Codex CLI, Cursor, opencode, or GitHub Copilot. This guide is harness-neutral; where a detail differs by harness, it says so and points you to your harness's chapter (see [Running on other harnesses](harnesses/README.md)). Examples are shown in Claude Code unless noted.
...
| [Running on other harnesses](harnesses/README.md) | Install and run on Kiro (IDE and CLI), Codex CLI, Cursor, opencode, or GitHub Copilot, and what differs per harness |
