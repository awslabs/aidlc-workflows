...
AI-DLC installs by copying its distribution for your harness into your project. The steps below cover **Claude Code** (the `dist/claude/.claude/` tree). Running Kiro, Codex, or Cursor? Each ships its own distribution and install steps — see [Running on Kiro IDE](harnesses/kiro-ide.md), [Running on Codex CLI](harnesses/codex-cli.md), or [Running on Cursor](harnesses/cursor.md). The Claude Code implementation ships as a `.claude/` directory that you copy into your project.
