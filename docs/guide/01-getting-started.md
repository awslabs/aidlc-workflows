...
Windows, it installs for the current account even when PowerShell is elevated,
registers the bin directory in persistent User PATH, and updates the current
PowerShell process. If another session cannot find `aidlc`, open a new terminal.
