...
| **bun** | Required for all CLI tools and all 16 hooks (state management, audit logging, sensor dispatch, runtime-graph compile, loop enforcement, exact dispatch-rule delivery, state-transition, reviewer-scope, and review-freeze enforcement, statusline, human-turn mint, token-usage folding). Everything is TypeScript, run via bun (~20ms startup). No additional dependencies — works identically on macOS, Linux, and native Windows PowerShell. | `curl -fsSL https://bun.sh/install \| bash` ([docs](https://bun.sh)). On Windows: `npm install -g bun` or `powershell -c "irm bun.sh/install.ps1 \| iex"` |
