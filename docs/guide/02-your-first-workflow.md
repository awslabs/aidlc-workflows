...
> The `$<usd>` is a rough local estimate from public list prices, not a bill — and because the harness runs on Bedrock by default it does not reflect Bedrock, negotiated, or subscription pricing. Turn it off with `AIDLC_DISABLE_USAGE_TRACKING=1` before customer-facing use (see [Troubleshooting](15-troubleshooting.md)).
...
| `↑<in> ↓<out> $<usd>` | Token usage and priceable cost for the active workflow and current transcript/session (Claude Code only; omitted before usage is available; disabled by `AIDLC_DISABLE_USAGE_TRACKING=1`). `$` is a rough public-list-price estimate — not a Bedrock/negotiated/subscription bill; turn it off before customer-facing use |
