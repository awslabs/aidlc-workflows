...
The [Devin engineering findings](../../reference/research/devin/index.md) distinguish implemented behavior from open acceptance gaps. Native dispatch field translation, reviewer read/search isolation, and background terminal bookkeeping are implemented against the captured 3000.6.14 payload contract; the `tool_use_id` issuer formats those paths depend on are version-specific observations, not a permanent vendor schema. Outcome classification is anchored to the notification head grammar (`Subagent <id> <verb>`) — `completed successfully` is captured text; the failure/cancellation verbs are extrapolations of the same grammar, and the free-form report body never influences classification. Question-response compatibility does not make an unknown or skipped choice an approval. Desktop binary discovery is not verified Desktop execution, and a version check is not a full workflow certification.
