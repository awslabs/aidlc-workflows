...
| T-013    | Medium   | TA1/TA4 uses prompt injection in artifact content to direct the Component→Code agent's file-reading tool to read host files outside the project root | Information Disclosure | Filesystem, Source code | M-014        | Mitigate | Mitigated                    |
...
| M-003        | All user-supplied content in HTML reports is escaped: sidebar rendering uses `html.escape()` (Python-side), client-side rendering uses `escapeHtml()` (JavaScript-side), and the embedded artifact-data JSON block neutralizes `</` → `<\/` so a crafted title/description cannot break out of the `<script>` context. No external resources loaded. | T-002              | Complete             | `generators/html.py:116-117`, `generators/html.py:382`, `generators/html.py:514-527` |
| M-004        | All AI-discovered relationships are validated against the set of known parsed artifact IDs. Relationships referencing non-existent IDs are silently discarded. Count of discarded relationships is logged.            | T-003, T-009       | Complete             | `agent.py:232-271`                                         |
...
| M-009        | Pipeline uses per-file error handling. Code parser reads with `errors="ignore"`. AI agent limits source reading to 30 files, 200 lines each. NetworkX operations are bounded by artifact count.                       | T-008              | Complete             | `pipeline.py:88-94`, `agent.py:93-116`                     |
...
| M-012        | File discovery is bounded to `src/`, `lib/`, `app/`, `packages/` under project root. Excluded directories include `.git`, `venv`, `node_modules`. Symlinks are skipped (`is_symlink()`) and each discovered file's resolved path is verified to remain within the project root, so a symlink pointing outside the project is neither followed nor read. Tool operates read-only on discovered files. | T-011              | Complete             | `discovery.py:83-88`, `discovery.py:98-135`                |
...
| M-014        | The Component→Code agent's `read_source_code_file` tool is created by a factory (`make_source_code_reader`) that binds the project root in a closure. Every requested path is resolved and rejected if it falls outside the project boundary (traversal, absolute path, or symlink escape), so prompt injection cannot read arbitrary host files. Mirrors the sibling `_resolve_safe()` pattern in `aidlc-evaluator`. | T-013              | Complete             | `agent.py:66-117`, `agent.py:193-224`                      |
