...
//
// The fix mirrors tui-bun-process.ts: the CLI joins a kill-on-close Job
// Object owned by this test process, so every descendant is bound to the
// drive. terminate() ends the whole tree after each drive (success, failure,
// abort and timeout all reach the finally in driveAidlc), and the OS ends it
// when the last job handle closes, which is what happens if this process
// dies first. Neither BREAKAWAY flag is set (windowsJobLimits), so a detached
// grandchild cannot leave the job.
//
// The CLI is assigned right after spawn returns. Windows does not add a
// process's existing children when it joins a job, but the CLI has only just
// been created at that point and cannot have spawned anything yet: it still
// has to load its runtime before it reads its first message.
//
...
export interface SdkContainmentReport {
  /** PID of the CLI the SDK asked us to spawn, if the spawn produced one. */
  cliPid?: number;
  /** True when the CLI had already exited when terminate() started. */
  cliExited: boolean;
  /** Job members observed after the optional grace wait, before termination. */
  membersBeforeTerminate: number[];
  /** True when TerminateJobObject was needed. */
  terminated: boolean;
...
export function describeSdkContainment(report: SdkContainmentReport): string {
  ...
  return `SDK drive cli=${report.cliPid ?? "none"} cliExited=${report.cliExited} ` +
    `members=[${report.membersBeforeTerminate.join(", ")}] terminated=${report.terminated} ` +
    `${survivors} waited=${report.waitedMs}ms`;
}
 ...
 */
export async function createSdkProcessContainment(): Promise<SdkProcessContainment | undefined> {
  ...
  const assign = (child: ChildProcess): void => {
    ...
    try {
      if (!api.AssignProcessToJobObject(job, handle)) {
        const code = api.GetLastError();
        // A CLI that already exited cannot join; the SDK observes that exit.
      ...
      if (member[0] !== 1) {
        child.kill("SIGKILL");
        throw new Error(`SDK CLI ${pid} is not in its Job Object`);
      }
    } finally {
      check(api.CloseHandle(handle), "CloseHandle(cli)");
    }
  };
  ...
  const cliAlive = (): boolean =>
    cli !== undefined && cli.pid !== undefined && cli.exitCode === null && cli.signalCode === null;

  const waitForCliExit = (ms: number): Promise<void> => new Promise((resolve) => {
    if (!cliAlive()) { resolve(); return; }
    const child = cli!;
  ...
  return {
    spawn(options) {
      ...
      if (cli) throw new Error("SDK process containment spawns one CLI per drive");
      ...
      const debug = /^(1|true|yes)$/i.test(options.env.DEBUG_CLAUDE_AGENT_SDK ?? "");
      ...
      if (child.pid !== undefined) assign(child);
      return child;
    },
    memberPids,
    async terminate(options = {}) {
      ...
      const cliExited = !cliAlive();
      const graceMs = Math.max(0, Math.min(options.graceMs ?? 0, deadline - Date.now()));
      if (!cliExited && graceMs > 0) await waitForCliExit(graceMs);
