...
// (plan 10). Drives a REAL workflow creation through the Claude Agent SDK and
// asserts ONLY on deterministic surfaces: the verbatim creation CLI stdout in the
...
// creation audit events are
// unchanged (creation still writes them) - only the LOCATION moved (per-intent) and
// the dispatch verb (intent-create, not init). This test resolves the created record
...
// a real driveAidlc creation run, and we ADD audit-event assertions the .sh never
// made (the creation handler's events), reaching equal-or-stronger parity. The
// creation path is gate-free: naming a scope on a fresh workspace creates and STOPs.
...
// audit event sequence (per-clone shard). The Bash tool_result carries the creation
// stdout bytes; the files land in the created intent's record. We assert on both.
//
...
//   2 audit shard exists               -> the created record's audit/ shard dir holds a *.md shard
//   3 state has "State Version.*: 7$"  -> assertStateField(r,"State Version","7")  (creation template)
...
//   - The creation CLI actually RAN: assertToolResultContains(r,"Bash",<verbatim summary>)
//     proves the deterministic tool fired and its fixed stdout reached us
//     ("Intent created:" / "State initialized:" / "Project type:", utility.ts:2374-2382) -
...
//   - Creation audit events: the creation handler emits WORKFLOW_STARTED (utility.ts:2065),
...
//   - creation audit events:     WORKFLOW_STARTED / WORKSPACE_SCAFFOLDED / WORKSPACE_INITIALISED (utility.ts:2065/2110/2331)
//
...
// P4: creation writes the workflow record PER-INTENT under
...
// created record from the
// active-space + active-intent cursors (flat fallback for a not-yet-created project).
...
// GONE; creation prints the intent-created + state-init block instead.)
const INTENT_CREATED_LINE = "Intent created:"; // utility.ts:2375
...
describe("t21 /aidlc workflow creation (sdk)", () => {
  // -------------------------------------------------------------------------
  // First-run creation from a project with NO aidlc-docs/ (--no-aidlc-docs, as the
  // .sh seeded). Naming a scope on the empty workspace creates the first intent.
  // Re-expresses .sh tests 1-7 on the per-intent deterministic surfaces and adds
  // the creation CLI-ran proof + creation audit events.
  // -------------------------------------------------------------------------
  test(
    ...
    async () => {
      ...
      try {
        ...
        // clean creation on an empty workspace (NOT a migration; needsFlatMigration
        ...
        // STRENGTHENING: the deterministic creation CLI actually fired via Bash and
        ...
        // creation path RAN through the tool, not that prose merely mentioned it.
        assertToolResultContains(r, "Bash", INTENT_CREATED_LINE);
        assertToolResultContains(r, "Bash", INIT_STATE_SUMMARY);

        // The created intent's record dir (resolved from the active cursors).
        ...
        // .sh test 2: an audit shard was written under the created record's audit/.
        ...
        // DIRECTORY, not merely a present path. Creation ensures the SPACE-level
        ...
        // STRENGTHENING: the creation handler's audit events. The .sh checked the
        ...
        // audit shard exists and grew. (assertAuditEvent reads the created record's
        ...
        // The creation tool exited 0. The SDK driver intentionally aborts as soon
        // as the deterministic creation stdout lands, so the model cannot continue
        // into unrelated workflow execution after the creation contract is proven.
