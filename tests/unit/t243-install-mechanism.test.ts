...
describe("t243 project initialization", () => {
  ...
  test("a harness that shares an engine directory cannot coexist", () => {
    // Upstream wrote this against kiro + kiro-ide, which both claimed `.kiro`. This
    // branch serves one Kiro row, so that pair no longer exists and `kiro-ide` is not
    // a harness id the engine knows. The guard itself is unchanged, and the other
    // colliding pair the feature names is opencode + copilot, which both claim
    // `.aidlc` — so the contract is exercised through that pair instead.
    ...
    expect(shared.stdout).toContain(
      "harness copilot shares directory .aidlc with installed opencode",
    );
  }, 60_000);
    ...
    const expected: Record<string, Record<string, string[]>> = {
      ...
      codex: {
        "AGENTS.md": [
          ...
          "sha256:12cb3589efa84a44c8ca658f1452ae6fb4af2b93322d68a4cfa4fc7b1d2ef5d1",
        ],
      },
      kiro: {
        "AGENTS.md": [
          ...
          "sha256:60e6f7a8a8abf0d601fb91971336f0f832091beb975025d2812c4907eb813aa0",
        ],
      },
    };
