...
describe("t283 engine-owned continuation cursor", () => {
  ...
  test("every delivered receipt advances once and the final run-stage carries no receipt", () => {
    for (const harness of HARNESSES) {
      ...
      // error and never as the earlier part it once named: the issued run-stage,
      // re-answered from the marker. The one exception is a Kiro project with a
      // LIVE legacy Plan Approval host, whose protected choices are rotated by
      // publication, so it never re-answers from the marker. This row decides
      // that from the host record rather than the harness name - one row serves
      // both Kiro surfaces - and this fixture has no legacy host, so Kiro takes
      // the reuse like every other harness.
