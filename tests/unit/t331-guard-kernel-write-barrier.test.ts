...
describe("guard kernel protected-store write barrier", () => {
  ...
  test("attached dd output operand: a protected destination overlaps the store", () => {
    const current = fixture("attached-dd-protected");
    const target = join(current.storeDir, "entry.json");

    expectKind(
      barrier(current, "Bash", { command: `dd of=${target}` }),
      "store-overlap",
    );
  });

  test("attached dd output operand: an outside destination remains proven disjoint", () => {
    const current = fixture("attached-dd-outside");
    const target = join(current.root, "outside", "entry.json");
  ...
  test("attached dd output operand: an unresolved expansion is unprovable", () => {
    const current = fixture("attached-dd-expansion");

    expectKind(
      barrier(current, "Bash", { command: "dd of=$P/entry.json" }),
      "unprovable",
    );
  });

  test("attached long output option: tee cannot hide a protected destination after equals", () => {
    const current = fixture("attached-tee-output");
    const target = join(current.storeDir, "entry.json");

    expectKind(
      barrier(current, "Bash", { command: `tee --output=${target}` }),
      "store-overlap",
    );
  });

  test("attached arbitrary key operand: path proof includes the value without recognizing the key", () => {
    const current = fixture("attached-arbitrary-key");
    const target = join(current.storeDir, "entry.json");

    expectKind(
      barrier(current, "Bash", { command: `custom-writer any-key=${target}` }),
      "store-overlap",
    );
  });

  test("attached short output option: a known writer cannot hide a protected destination after -o", () => {
    const current = fixture("attached-short-output");
    const target = join(current.storeDir, "entry.json");

    expectKind(
      barrier(current, "Bash", { command: `curl -o${target}` }),
      "store-overlap",
    );
  });
