...
describe("t151 neutral and native onboarding", () => {
  ...
  test("all four identical-sharing harnesses ship byte-identical root instructions", () => {
    ...
    // Four, not five: one Kiro row now serves IDE and CLI, so `kiro-ide` is no longer
    // a declaring harness. The byte-identity contract itself is unchanged.
    expect(sharing.map((h) => h.name)).toEqual(["codex", "cursor", "kiro", "opencode"]);
