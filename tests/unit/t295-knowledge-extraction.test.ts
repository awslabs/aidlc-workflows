...
describe("t295 Finding 4: the per-document size check runs BEFORE the read, not after", () => {
  ...
  test("behavioral: a REAL over-cap file never gets fully buffered — RSS stays far below its size", () => {
    ...
    // Comfortable slack (48 MiB) for module loading and loaded-suite allocator
    // noise, while still failing hard if the whole file were materialised:
    // 48 MiB is less than half of the 96 MiB file.
