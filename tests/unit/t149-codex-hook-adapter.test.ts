...
// In-process unit test of the codex adapter's exported selection parser.
// Imported from the dist tree (the same tree the subprocess tests above
...
// In-process unit tests of the codex adapter's exported selection parser.
// These call hasExplicitHumanSelection directly (no subprocess) to pin the
// Devin ask_user_question answer shape recognition alongside the existing
// Claude Code shape. The function is imported from the SOURCE tree
// (harness/codex/hooks/aidlc-codex-adapter.ts) — the Devin-shape fix lands
// there first; dist/ is regenerated by package.ts. explicitHumanSelectionText
// is NOT exported, so only the boolean gate is tested in-process here; the
// text-extraction path is covered indirectly by the record-human-turn
// subprocess tests above.
describe("t149 codex adapter — hasExplicitHumanSelection recognizes Devin's ask_user_question shape", () => {
  // Shared tool_input: a single question keyed by id "approve" with text
  // "approve?" and offered labels "yes"/"no". The Devin shape keys its answers
  // by question TEXT; the Claude Code shape keys by question id.
  const toolInput = {
    questions: [
      {
        id: "approve",
  ...
  test("recognizes Devin native shape (predefined option)", () => {
    const toolResponse = {
      success: true,
      output: JSON.stringify({
        answers: { "approve?": [{ selected: ["yes"], custom_text: "" }] },
      }),
      error: null,
    };
    expect(hasExplicitHumanSelection(toolResponse, toolInput)).toBe(true);
  });

  test("recognizes Devin Other free-text shape", () => {
    const toolResponse = {
      success: true,
      output: JSON.stringify({
        answers: {
          "approve?": [{ selected: ["Other"], custom_text: "yes, but add a test" }],
        },
      }),
      error: null,
    };
  ...
  test("still recognizes Claude Code shape (no regression)", () => {
    const toolResponse = {
      success: true,
      output: JSON.stringify({
  ...
  test("rejects empty/cancelled Devin shape", () => {
    // "cancelled" is a non-answer (matched by isNonAnswer) and is not an
    // offered option label, so the Devin-path validation must reject it.
    const toolResponse = {
      success: true,
      output: JSON.stringify({
        answers: { "approve?": [{ selected: ["cancelled"], custom_text: "" }] },
      }),
      error: null,
    };
    expect(hasExplicitHumanSelection(toolResponse, toolInput)).toBe(false);
  });
});
