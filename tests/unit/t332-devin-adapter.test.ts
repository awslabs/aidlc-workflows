...
describe("t332 devin adapter — stdin shim normalizes Devin payloads to core hooks", () => {
  ...
  //
  // Payloads are cloned from the minimal 3000.10.31 native-contract fixture
  // (six named events; the capture-case/index provenance lives in
  // native-contracts-3000.10.31.provenance.json) with <placeholder>
  // substitution; every classifier string below is verbatim from the capture
  // except where a case is marked binary-pinned (the 'exited with an error'
  // read string did not appear on this host — C12's denied-tool child read
  // back as 'completed').

  const NATIVE_CONTRACTS = JSON.parse(
    readFileSync(
      join(
        REPO_ROOT,
        "tests",
        "fixtures",
        "devin-hook-payloads",
        "native-contracts-3000.10.31.json",
      ),
      "utf-8",
    ),
  ) as { events: Record<string, Record<string, unknown>> };

  /** Clone one captured native event by its semantic name, substituting the
   *  fixture's <placeholder> tokens (e.g. "<agent-id>") with concrete
   *  values. */
  function nativeEvent(
    name: string,
    subs: Record<string, string> = {},
  ): Record<string, unknown> {
    const event = NATIVE_CONTRACTS.events[name];
    if (!event) {
      throw new Error(`native-contracts fixture has no event "${name}"`);
    }
   ...
   *  event envelope is the repeated-read capture; the output string is
  ...
  function capturedReadPostToolUse(
    ...
    const event = nativeEvent("repeatedTerminalReadPost", subs);
    if (output !== undefined) {
  ...
  function launchBackground(dir: string, session: string, agentId: string): void {
    ...
    expect(
      runAdapter(
        ...
        "log-subagent",
        withCwd(
          nativeEvent("backgroundLaunchPost", {
            "<session>": session,
            "<agent-id>": agentId,
          }),
          dir,
  ...
  function readTerminal(dir: string, session: string, agentId: string): void {
    expect(
      runAdapter(
        ...
        "log-subagent",
        withCwd(
          nativeEvent("backgroundTerminalReadPost", {
            "<session>": session,
            "<agent-id>": agentId,
          }),
          dir,
    ...
    try {
      ...
      expect(
        runAdapter(
          ...
          "log-subagent",
          withCwd(
            nativeEvent("repeatedTerminalReadPost", {
              "<session>": session,
              "<agent-id>": "aa37dc28",
            }),
            dir,
    ...
    try {
      ...
      expect(
        runAdapter(
          ...
          "log-subagent",
          withCwd(
            nativeEvent("resumedCompletionPost", {
              "<session>": session,
              "<agent-id>": "aa37dc28",
            }),
            dir,
  ...
  // C17/C19 on 3000.10.31: no profile/agent_type/agent_id on any child event,
  // and background children interleave under the same session_id). The adapter
  // therefore maintains a foreground reviewer window —
  // <record>/.aidlc-engine/reviewer-window/<session>.json — opened by the
  // parent's reviewer-profile run_subagent PreToolUse (the ONLY host-carried
  // identity) and closed at the dispatch PostToolUse / Stop / prompt / session
  // boundaries. While open, every path/search/shell call on that session is
  // forwarded to the core hook with agent_type = the window's profile; the
  // core's existing identity check and matcher decide.
  //
  // Payloads below reuse the C17/C18/C19/C21 event shapes from the Item 4
  // capture (relative file_path, grep glob_pattern, absolute exec workdir,
  // the six-key child envelope); the parent PostToolUse comes from the
  // shared native-contract fixture's `reviewerForegroundCompletionPost`.

  const REVIEWER = "aidlc-architecture-reviewer-agent";
  const OTHER_REVIEWER = "aidlc-product-lead-agent";
  const S = "item4-session-s";

  /** Seed the reviewer-scope fixture: a fresh dispatch record naming
   *  U03-scoring with one exempt U01-infra file, plus the construction tree
   *  the paths resolve against. Returns the exempt file's absolute path. */
  function seedReviewerDispatch(dir: string, reviewer = REVIEWER): string {
    const recordRoot = dirname(dirname(reviewerDispatchPath(dir)));
    const exemptFile = join(
      recordRoot,
      "construction",
      "U01-infra",
      "api-contract.md",
    ...
    for (const rel of [
      "construction/U03-scoring/design.md",
      "construction/U01-infra/design.md",
      "construction/U01-infra/api-contract.md",
      "construction/U01-infra/notebooks/sib.ipynb",
    ]) {
      const p = join(recordRoot, rel);
    ...
    mkdirSync(dirname(reviewerDispatchPath(dir)), { recursive: true });
    writeFileSync(
      reviewerDispatchPath(dir),
      JSON.stringify({
        reviewer,
  ...
  /** The reviewer-window file for a session under this project's record. */
  function windowFile(dir: string, session: string): string {
    return join(
  ...
  /** A Devin-native run_subagent PreToolUse through the reviewer-scope arm —
   *  "open the window" when the profile is a reviewer and the record matches. */
  function dispatch(
    dir: string,
    session: string = S,
    extraInput: Record<string, unknown> = {},
    ...
    return runAdapter(
      ...
      "reviewer-scope",
      {
        ...
        tool_name: "run_subagent",
        tool_input: {
          title: "Item 4 reviewer dispatch",
          profile: REVIEWER,
          task: "Review construction/U03-scoring per the passed contracts.",
          is_background: false,
          ...extraInput,
        },
        tool_use_id: "toolu_item4dispatch",
        session_id: session,
        prompt_id: "item4-prompt",
      },
      env,
    );
  }

  /** A Devin-native child tool PreToolUse through the reviewer-scope arm
   *  (C17's six-key envelope: no cwd, no identity fields). */
  function childCall(
    dir: string,
    tool: string,
  ...
  test("Item 4 case 1: attributed child read of the current unit is allowed", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(true);
      const r = childCall(dir, "read", {
        file_path: "construction/U03-scoring/design.md",
      });
      expect(r.code, r.stderr).toBe(0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("Item 4 case 2: attributed child read of a sibling unit blocks with a scope reason and a REVIEWER_SCOPE_BLOCKED row", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const r = childCall(dir, "read", {
        file_path: "construction/U01-infra/design.md",
      });
      expect(r.code).toBe(2);
  ...
  test("Item 4 case 3: attributed child read of the exempt sibling file is allowed", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      // construction/U01-infra/api-contract.md is the record's exempt entry.
      const r = childCall(dir, "read", {
        file_path: "construction/U01-infra/api-contract.md",
      });
      expect(r.code, r.stderr).toBe(0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("Item 4 case 4: attributed grep whose content pattern names a sibling but whose path stays in-unit is allowed", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const r = childCall(dir, "grep", {
        pattern: "U01-infra",
        path: "construction/U03-scoring",
      });
      expect(r.code, r.stderr).toBe(0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("Item 4 case 5: attributed grep with a cross-unit glob_pattern blocks (glob_pattern -> glob)", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const r = childCall(dir, "grep", {
        pattern: "x",
        glob_pattern: "construction/*/**",
      });
      expect(r.code).toBe(2);
      expect(readAudit(dir)).toContain("**Tool**: Grep");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  test("Item 4 case 6: attributed grep rooted at bare construction/ blocks, and an implied-root grep blocks with the defaulted reason", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const rooted = childCall(dir, "grep", {
        pattern: "x",
        path: "construction",
      });
      expect(rooted.code).toBe(2);
      // grep {pattern} alone: the adapter forwards no glob/path, so the core
      // judges the implied "." search root — which sweeps construction/ — and
      // blocks with the defaulted wording ("names no path").
      const implied = childCall(dir, "grep", { pattern: "x" });
      expect(implied.code).toBe(2);
      expect(implied.stderr).toContain("names no path");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  test("Item 4 case 7: attributed glob over a sibling subtree blocks, pattern-rooted and path-rooted", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const byPattern = childCall(dir, "glob", {
        pattern: "construction/U01-infra/**",
      });
      expect(byPattern.code).toBe(2);
      const byPath = childCall(dir, "glob", {
        pattern: "*.md",
        path: "construction/U01-infra",
      });
      expect(byPath.code).toBe(2);
      expect(readAudit(dir)).toContain("**Tool**: Glob");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  test("Item 4 case 8: attributed notebook_read and notebook_edit on a sibling notebook block", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const nb = "construction/U01-infra/notebooks/sib.ipynb";
      expect(childCall(dir, "notebook_read", { notebook_path: nb }).code).toBe(2);
      expect(childCall(dir, "notebook_edit", { notebook_path: nb }).code).toBe(2);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  test("Item 4 case 9: attributed edit/write/apply_patch on a sibling block; current-unit edits pass", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const sib = "construction/U01-infra/design.md";
      expect(
        childCall(dir, "edit", {
          file_path: sib,
          old_string: "a",
      ...
      expect(
        childCall(dir, "write", {
          file_path: "construction/U01-infra/new.md",
          content: "x",
        }).code,
      ).toBe(2);
      expect(
      ...
      expect(
        childCall(dir, "write", {
          file_path: "construction/U03-scoring/notes.md",
          content: "x",
        }).code,
      ).toBe(0);
      const patch = childCall(dir, "apply_patch", {
        command:
          "*** Begin Patch\n*** Add File: construction/U01-infra/patched.md\n+x\n*** End Patch\n",
      });
  ...
  test("Item 4 case 10: attributed exec blocks on sibling paths, resolves workdir-relative traversal, and allows an in-unit rg", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      expect(
        childCall(dir, "exec", {
          command: "cat construction/U01-infra/design.md",
        }).code,
      ).toBe(2);
      // workdir arrives absolute (C17); ../U01-infra resolves into the sibling.
      const recordRoot = dirname(dirname(reviewerDispatchPath(dir)));
      expect(
        childCall(dir, "exec", {
          command: "cat ../U01-infra/design.md",
          workdir: join(recordRoot, "construction", "U03-scoring"),
        }).code,
      ).toBe(2);
      expect(
        childCall(dir, "exec", {
          command: "rg TODO construction/U03-scoring",
        }).code,
      ).toBe(0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("Item 4 case 11: with no window the same sibling read/grep/exec payloads pass unattributed (conductor)", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      // No dispatch — the conductor's own calls are never attributed.
      expect(
        childCall(dir, "read", {
          file_path: "construction/U01-infra/design.md",
        }).code,
      ...
      expect(readAudit(dir)).not.toContain("REVIEWER_SCOPE_BLOCKED");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  test("Item 4 case 12: the parent run_subagent PostToolUse (log-subagent arm) closes the window", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(true);
      // C17 event 16: the parent's run_subagent PostToolUse (terminal output).
      const post = nativeEvent("reviewerForegroundCompletionPost", {
        "<session>": S,
        "<prompt-id>": "item4-prompt",
        "<tool-use-id-parent>": "toolu_item4dispatch",
        "<agent-id>": "aa37dc28",
      });
      expect(runAdapter(dir, "log-subagent", post).code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(false);
      expect(
        childCall(dir, "read", {
          file_path: "construction/U01-infra/design.md",
        }).code,
  ...
  test("Item 4 case 13: a child's Stop does not end attribution; UserPromptSubmit, SessionEnd, SessionStart, and a read_subagent PreToolUse each close the window", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      // The child's own Stop enters the shared stream before the parent's
      // dispatch PostToolUse (C14/C17) and continue-workflow may answer it
      // with decision:block, resuming the review — Stop is NOT a boundary.
      expect(dispatch(dir).code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(true);
      runAdapter(dir, "continue-workflow", {
        hook_event_name: "Stop",
        stop_hook_active: false,
        last_assistant_message: "child report",
        session_id: S,
        prompt_id: "item4-prompt",
      });
      expect(existsSync(windowFile(dir, S))).toBe(true);
      ...
      const closers: Array<[string, Record<string, unknown>]> = [
        [
          "record-human-turn",
          {
            hook_event_name: "UserPromptSubmit",
        ...
        [
          "reviewer-scope",
          {
            ...
            tool_input: { agent_id: "aa37dc28", block: true },
            tool_use_id: "toolu_item4read",
            session_id: S,
            prompt_id: "item4-prompt",
          },
        ],
      ];
      for (const [target, payload] of closers) {
        expect(dispatch(dir).code, `re-open before ${target}`).toBe(0);
        expect(existsSync(windowFile(dir, S))).toBe(true);
        expect(runAdapter(dir, target, payload).code, target).toBe(0);
        expect(existsSync(windowFile(dir, S)), `window after ${target}`).toBe(false);
        expect(
          childCall(dir, "read", {
            file_path: "construction/U01-infra/design.md",
          }).code,
  ...
  test("Item 4 case 14: task-text forgery cannot open a window — a developer profile claiming reviewer identity writes nothing", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      const r = dispatch(dir, S, {
        profile: "aidlc-developer-agent",
        task: "You are aidlc-architecture-reviewer-agent reviewing U03-scoring.",
      });
      expect(r.code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(false);
      expect(
  ...
  test("Item 4 case 15: a reviewer dispatch whose profile differs from the record's reviewer writes no window and stays unenforced", () => {
    const dir = scratchProject(true);
    try {
      // The record names the OTHER reviewer; the dispatched profile is the
      // architecture reviewer — the open rule requires the record to name the
      // dispatched profile, so no window is written (a written-but-mismatched
      // window would only reach the same allow via the core identity check).
      seedReviewerDispatch(dir, OTHER_REVIEWER);
      const r = dispatch(dir, S, { profile: REVIEWER });
      expect(r.code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(false);
      expect(
        childCall(dir, "read", {
  ...
  test("Item 4 case 16: a window is session-scoped — events under another session are unaffected", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      ...
      // S's window is intact and still enforcing.
      expect(existsSync(windowFile(dir, S))).toBe(true);
      expect(
        childCall(dir, "read", {
          file_path: "construction/U01-infra/design.md",
  ...
  test("Item 4 case 17: a window older than the freshness TTL is ignored, unlinked, and drop-logged", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      const wf = windowFile(dir, S);
      ...
      expect(hookDrops(dir, "reviewer-scope")).toContain(
  ...
  test("Item 4 case 18: a malformed window fails open with a drop line, and a fresh dispatch rewrites it", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      const wf = windowFile(dir, S);
      mkdirSync(dirname(wf), { recursive: true });
      writeFileSync(wf, "not json", "utf-8");
      expect(
        childCall(dir, "read", {
          file_path: "construction/U01-infra/design.md",
        }).code,
      ...
      // A fresh dispatch rewrites the malformed file and enforcement resumes.
      expect(dispatch(dir).code).toBe(0);
      const parsed = JSON.parse(readFileSync(wf, "utf-8")) as {
        version?: number;
        profile?: string;
      };
  ...
  test("Item 4 case 19: a reviewer dispatched with is_background is refused with the foreground reason and writes no window", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      const r = dispatch(dir, S, { is_background: true });
      expect(r.code).toBe(2);
      expect(r.stderr).toContain("must run in the foreground on Devin");
      expect(existsSync(windowFile(dir, S))).toBe(false);
      expect(hookDrops(dir, "reviewer-scope")).toContain(
  ...
  test("Item 4 case 20: a reviewer dispatch while a background subagent is pending for the session is refused; another session's pending entry does not block", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      writeFileSync(
        subagentInflightMarkerPath(dir),
        JSON.stringify({
          version: 1,
          entries: [{ sessionId: S, startedAtMs: Date.now() }],
        }),
        "utf-8",
      );
      const refused = dispatch(dir);
      expect(refused.code).toBe(2);
      expect(refused.stderr).toContain(
        "Read every pending background subagent with read_subagent",
      );
      expect(existsSync(windowFile(dir, S))).toBe(false);
      expect(hookDrops(dir, "reviewer-scope")).toContain(
        "refused a reviewer dispatch while background subagents are pending",
      );
      // A pending entry for a different session does not block S's dispatch.
      writeFileSync(
        subagentInflightMarkerPath(dir),
        JSON.stringify({
          version: 1,
          entries: [{ sessionId: "item4-other", startedAtMs: Date.now() }],
        }),
        "utf-8",
      );
      expect(dispatch(dir).code).toBe(0);
  ...
  test("Item 4 case 21: while attributed, exec/read naming the reviewer-window state are refused (protected state)", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const wf = windowFile(dir, S);
      const exec = childCall(dir, "exec", { command: `cat ${wf}` });
      expect(exec.code).toBe(2);
      expect(exec.stderr).toContain(
        "AI-DLC reviewer attribution state is read-only for the reviewer.",
      );
      const read = childCall(dir, "read", { file_path: wf });
      expect(read.code).toBe(2);
      expect(read.stderr).toContain(
        "AI-DLC reviewer attribution state is read-only for the reviewer.",
      );
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("Item 4 case 22: a run_subagent denial in deliver-stage-rules unlinks the window the reviewer-scope arm just opened", () => {
    // plan-approval-guard can never deny a reviewer dispatch (it only judges
    // aidlc-developer-agent), so the exercisable arm is deliver-stage-rules:
    // with an unseeded memory dir the stage bundle is unresolvable and the
    // core hook exits 2 — the adapter must close the window it just opened
    // because Devin emits no PostToolUse for a hook-denied dispatch (C21).
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      const payload = {
        ...
        tool_name: "run_subagent",
        tool_input: {
          title: "Item 4 denied reviewer dispatch",
          profile: REVIEWER,
          task: "Run .devin/skills/aidlc/stages/inception/requirements-analysis.md",
          is_background: false,
        },
        tool_use_id: "toolu_item4dispatch",
        session_id: S,
      ...
      expect(runAdapter(dir, "reviewer-scope", payload).code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(true);
      const denied = runAdapter(dir, "deliver-stage-rules", payload);
  ...
  test("Item 4 case 23: a reviewer dispatch with no record writes no window; an attributed call after the record is gone lands the core missing-record advisory", () => {
    const dir = scratchProject(true);
    try {
      // No reviewer-dispatch.json: the dispatch is allowed but unattributed.
      expect(dispatch(dir).code).toBe(0);
      expect(existsSync(windowFile(dir, S))).toBe(false);
      // The core's advisory fires once identity flows: open a window with a
      // record present, remove the record, then an attributed call touching
      // construction/ lands the missing-record drop line. The advisory only
      // fires when a per-unit review was owed, so seed a live directive
      // marker carrying the unit (digest must match the seeded state).
      seedReviewerDispatch(dir);
      expect(dispatch(dir).code).toBe(0);
      const owedState = readFileSync(seededStateFile(dir), "utf-8");
      writeActiveDirectiveMarker(dir, {
        kind: "run-stage",
        stage: "functional-design",
        unit: "U03-scoring",
        state_sha256: stateDigest(owedState),
      });
      rmSync(reviewerDispatchPath(dir));
      expect(
        childCall(dir, "read", {
          file_path: "construction/U03-scoring/design.md",
        }).code,
      ...
      expect(hookDrops(dir, "reviewer-scope")).toContain(
        "touched construction/ paths with no reviewer dispatch record",
      );
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  test("Item 4 case 24: AIDLC_DISABLE_REVIEWER_SCOPE_HOOK=1 disables every blocking path (scope denials and dispatch refusals)", () => {
    const dir = scratchProject(true);
    try {
      seedReviewerDispatch(dir);
      const env = { AIDLC_DISABLE_REVIEWER_SCOPE_HOOK: "1" };
      expect(dispatch(dir).code).toBe(0); // window still opens (lifecycle only)
      for (const [tool, input] of [
        ["read", { file_path: "construction/U01-infra/design.md" }],
        ["grep", { pattern: "x", glob_pattern: "construction/*/**" }],
        ["grep", { pattern: "x" }],
        ["glob", { pattern: "construction/U01-infra/**" }],
        ["notebook_read", { notebook_path: "construction/U01-infra/notebooks/sib.ipynb" }],
        ["edit", { file_path: "construction/U01-infra/design.md" }],
        ["write", { file_path: "construction/U01-infra/new.md" }],
        ["exec", { command: "cat construction/U01-infra/design.md" }],
        ["exec", { command: `cat ${windowFile(dir, S)}` }],
      ] as Array<[string, Record<string, unknown>]>) {
        expect(
          childCall(dir, tool, input, S, env).code,
          `disabled: ${tool}`,
        ).toBe(0);
      }
      // Dispatch-time refusals are enforcement too — disabled with the flag.
      expect(dispatch(dir, S, { is_background: true }, env).code).toBe(0);
      writeFileSync(
        subagentInflightMarkerPath(dir),
        JSON.stringify({
          version: 1,
      ...
      expect(dispatch(dir, S, {}, env).code).toBe(0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
// --- Fixture hygiene (Item 9): the directory carries only the retained
// minimal contract; raw runs and broad captures are not tracked -----------
