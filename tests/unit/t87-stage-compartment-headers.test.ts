...
describe("t87 stage-file compartment headers (migrated from t87-stage-compartment-headers.sh, plan 64)", () => {
  ...
  // .sh L10: plan 64. The shipped tree has since grown to 36 stages (2 × 36 =
  // 72 rows) with the four ideation discovery stages. Re-count to pin the plan
  // and guard against a stage being silently dropped from (or added to) the
  // shipped tree without the suite noticing.
  test("covers EXACTLY 36 stages × 2 compartments = 72 assertions (TAP plan parity)", () => {
    const COMPARTMENTS_PER_STAGE = 2;
    expect(STAGE_FILES.length).toBe(36);
    expect(STAGE_FILES.length * COMPARTMENTS_PER_STAGE).toBe(72);
