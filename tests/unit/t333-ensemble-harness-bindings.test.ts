...
describe("t333 ensemble protocol harness-binding parity", () => {
  ...
  test("the Devin binding names run_subagent, the profile field, the is_background split, and the default subagent model", () => {
    ...
    // The per-unit reviewer is a foreground dispatch after every pending
    // background result is read — the reviewer-scope hook refuses otherwise.
    expect(body).toContain(
      "A per-unit reviewer is always a foreground `run_subagent`",
    );
    expect(body).toContain("read via `read_subagent`");
