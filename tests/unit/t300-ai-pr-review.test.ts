...
`);
  ...
  test("workflow reviews internal PRs only and isolates model credentials from publication", () => {
    ...
    expect(WORKFLOW).not.toContain("AI reviewer controls changed; self-review uses head");
    expect(WORKFLOW).not.toContain("Reviewer controls changed; self-review runs only");
