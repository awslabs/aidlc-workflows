...
describe("t340 Kiro IDE ignore sources doctor", () => {
  ...
  // One Markdown conductor serves Kiro CLI and IDE, so the row follows that file:
  // absent without it, present once the conductor is wired.
  test("the doctor row is emitted only when the Kiro conductor is wired", () => {
    const { project, globalFile, env } = setupProject();
    writeFileSync(globalFile, ".kiro/\n");
    ...
    const unwired = run();
    expect(unwired).toContain("fail  agents/aidlc.md present (conductor wiring)");
    expect(unwired).not.toContain("Kiro IDE ignore sources:");
  ...
  test("a plugin composed by the real Kiro folder-drop hook is probed, and selection removes it", () => {
    ...
    cpSync(join(repo, "dist", "kiro"), project, { recursive: true });
    cpSync(join(repo, "dist", "plugins", "test-pro", "kiro"), project, { recursive: true });
    ...
    const compose = spawnSync(process.execPath, ["./hooks/aidlc-plugin-compose.ts", ".kiro", "kiro"], {
      cwd: project,
      encoding: "utf-8",
      env: composeEnv,
    });
