...
//   .sh test 1  (9 shipped scope files)                     -> "exactly 10 shipped aidlc-*.md scope files exist" (the set grew by `tutorial`)
...
// The scopes the framework ships, alphabetical — the .sh's hard-coded
// expectation (t125:62) plus `tutorial`, added later. Each is a literal
// independent of source iteration.
...
describe("shipped scope files — frontmatter + derived metadata (in-process)", () => {
  test("exactly 10 shipped aidlc-*.md scope files exist [.sh test 1]", () => {
    ...
    expect(files.length).toBe(SHIPPED_SCOPES.length);
  });
