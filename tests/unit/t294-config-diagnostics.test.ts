...
describe("t294 config diagnostics CLI", () => {
  ...
  test("pristine Codex refresh remains byte-idempotent without restoration notes", () => {
    const project = install("codex");
    const env = runtimeEnv();
    const configPath = join(project, ".codex", "config.toml");
    ...
    expect(firstText).toBe(installed);
    expect(first.stdout).not.toContain("Note:");
  ...
  test("provider mutation keeps project fields and refresh restores Codex framework entries", () => {
    const env = runtimeEnv();
    ...
    expect(codexRefreshed.stdout).toContain("Note: restored the shipped developer_instructions assignment");
    expect(codexRefreshed.stdout).toContain("Note: restored the shipped [shell_environment_policy] table");
    expect(readFileSync(codexPath, "utf-8"))
  ...
  test("Codex refresh repairs only the owned root sandbox mode, not prose or custom-table keys", () => {
    ...
    const refreshed = run(args, project, env);
    expect(refreshed.status, refreshed.stdout + refreshed.stderr).toBe(0);
    expect(refreshed.stdout).toContain("Note: restored the shipped sandbox_mode assignment");
    ...
    expect(restored.stdout).toContain("Note: restored the shipped sandbox_mode assignment");
  ...
  test("Codex refresh normalizes equivalent string assignments and quoted framework tables", () => {
    ...
    for (const replacement of [
      ...
      '"developer\\u005finstructions" = "team instructions"\n',
    ]) {
      const project = install("codex");
      const configPath = join(project, ".codex", "config.toml");
      const shipped = readFileSync(configPath, "utf-8");
    ...
    for (const replacement of [
      ...
      const configPath = join(project, ".codex", "config.toml");
      const shipped = readFileSync(configPath, "utf-8");
      const edited = shipped.replace(/\[tui\][\s\S]*$/, replacement);
      ...
      const after = readFileSync(configPath, "utf-8");
      expect(() => parseToml(after)).not.toThrow();
      expect(parseToml(after).tui).toEqual(parseToml(shipped).tui);
      expect(after).toContain("[tui]");
  ...
  test("Codex refresh removes retired framework-owned assignments and tables", () => {
    const project = install("codex");
    const env = runtimeEnv();
    const source = temp("aidlc-t294-codex-retired-source-");
    cpSync(join(DIST_RELEASE, "codex"), source, { recursive: true });
    const sourcePath = join(source, ".codex", "config.toml");
    writeFileSync(
      sourcePath,
      readFileSync(sourcePath, "utf-8")
        .replace(/^sandbox_mode\s*=.*(?:\r?\n|$)/m, "")
        .replace(/\[tui\][\s\S]*$/, ""),
    );

    const refreshed = run([
      "config", "--project-dir", project,
      "--from", source, "--harness", "codex", "--yes",
    ], project, env);
    expect(refreshed.status, refreshed.stdout + refreshed.stderr).toBe(0);
    const configPath = join(project, ".codex", "config.toml");
    ...
    expect(refreshed.stdout).not.toContain("it had local changes");

    const manifest = JSON.parse(readFileSync(
      join(project, ".codex", "tools", "data", "aidlc-manifest.json"),
      "utf-8",
    ));
    expect(manifest.entries[".codex/config.toml"].sandbox_mode).toBeUndefined();
    expect(manifest.entries[".codex/config.toml"].tui).toBeUndefined();
  }, 60_000);

  test("Codex refresh describes locally changed retired entries as removed", () => {
    const project = install("codex");
    const env = runtimeEnv();
    const configPath = join(project, ".codex", "config.toml");
    writeFileSync(
    ...
    const source = temp("aidlc-t294-codex-retired-local-source-");
    cpSync(join(DIST_RELEASE, "codex"), source, { recursive: true });
    const sourcePath = join(source, ".codex", "config.toml");
    writeFileSync(
      sourcePath,
    ...
    expect(refreshed.status, refreshed.stdout + refreshed.stderr).toBe(0);
    expect(refreshed.stdout).toContain(
      "removed the retired AI-DLC-owned sandbox_mode assignment",
    );
    expect(refreshed.stdout).toContain(
      "removed the retired AI-DLC-owned [tui] table",
    );
    expect(refreshed.stdout).not.toContain("restored the shipped sandbox_mode");
    expect(refreshed.stdout).not.toContain("restored the shipped [tui]");
  }, 60_000);

  test("Codex refresh preserves comments leading into a project table", () => {
    const project = install("codex");
    const env = runtimeEnv();
    const configPath = join(project, ".codex", "config.toml");
    const projectComment = "# Team MCP configuration must stay below";
    writeFileSync(
      configPath,
    ...
    expect(parseToml(after).mcp_servers).toEqual({
      team: { command: "team-mcp" },
    });
  }, 60_000);
  ...
  test("Codex refresh preserves nested project configuration under framework roots", () => {
    const project = install("codex");
    const env = runtimeEnv();
    const configPath = join(project, ".codex", "config.toml");
    const roots = [
    ...
    const config = `${readFileSync(configPath, "utf-8").trimEnd()}\n\n${
      roots.map((root) =>
        `[${root}.project_table]\nvalue.inner = "keep-table-${root}"\n\n` +
        `[[${root}.project_items]]\nvalue = "keep-array-${root}"`
      ).join("\n\n")
    }\n`;
    ...
    expect(refreshed.status, refreshed.stdout + refreshed.stderr).toBe(0);
    const parsed = parseToml(readFileSync(configPath, "utf-8")) as Record<string, {
      project_table: { value: { inner: string } };
      project_items: Array<{ value: string }>;
    }>;
    for (const root of roots) {
      expect(parsed[root].project_table.value.inner).toBe(`keep-table-${root}`);
      expect(parsed[root].project_items).toEqual([
        { value: `keep-array-${root}` },
      ]);
    }
  }, 60_000);
  ...
  test("release changes to owned entries are not reported as local edits", () => {
    const env = runtimeEnv();
    const codex = install("codex");
    const codexSource = temp("aidlc-t294-codex-release-change-");
    cpSync(join(DIST_RELEASE, "codex"), codexSource, { recursive: true });
    const codexSourcePath = join(codexSource, ".codex", "config.toml");
    writeFileSync(
      codexSourcePath,
      readFileSync(codexSourcePath, "utf-8").replace(
        'sandbox_mode = "workspace-write"',
        'sandbox_mode = "read-only"',
      ),
    );
    const codexRefresh = run([
      "config", "--project-dir", codex,
      "--from", codexSource, "--harness", "codex", "--yes",
    ], codex, env);
    expect(codexRefresh.status, codexRefresh.stdout + codexRefresh.stderr).toBe(0);
    expect(parseToml(readFileSync(join(codex, ".codex", "config.toml"), "utf-8")).sandbox_mode)
      .toBe("read-only");
    expect(codexRefresh.stdout).not.toContain("it had local changes");
  ...
  test("refresh restores deleted Codex developer instructions without force", () => {
    const project = install("codex");
    const env = runtimeEnv();
    const configPath = join(project, ".codex", "config.toml");
    ...
    expect(refreshed.status, refreshed.stdout + refreshed.stderr).toBe(0);
    expect(refreshed.stdout).toContain("Note: restored the shipped developer_instructions assignment");
    expect(parseToml(readFileSync(configPath, "utf-8")).developer_instructions).toBe(shipped);
  }, 60_000);
...
`;
  ...
  test("Claude refresh re-adds shipped allow entries before user entries and retains retired entries", () => {
    ...
    settings.permissions.allow = [
      ...shippedAllow.filter((entry) => entry !== "Glob"),
      "Bash(team-tool:*)",
    ];
    writeFileSync(settingsPath, `${JSON.stringify(settings, null, 2)}\n`);
    const refreshed = run([
    ...
    release.permissions.allow = shippedAllow.filter((entry) => entry !== "Glob");
    ...
    expect(JSON.parse(readFileSync(settingsPath, "utf-8")).permissions.allow)
      .toEqual([...release.permissions.allow, "Glob", "Bash(team-tool:*)"]);
  ...
  test("refresh restores Codex framework tables and preserves user MCP tables", () => {
    ...
    const project = install("codex");
    const configPath = join(project, ".codex", "config.toml");
    const shippedTui = parseToml(readFileSync(configPath, "utf-8")).tui;
    ...
    expect(ordinary.stdout).toContain("Note: restored the shipped [tui] table");
    expect(ordinary.stdout).toContain("personal Codex settings belong in ~/.codex/config.toml");
    const ordinaryConfig = parseToml(readFileSync(configPath, "utf-8"));
    expect(ordinaryConfig.tui).toEqual(shippedTui);
    expect(ordinaryConfig.mcp_servers).toEqual({ x: { command: "team-mcp", args: ["--stdio"] } });
  ...
  test("provider answers preserve project fields until release refresh restores Codex framework entries", () => {
    const env = runtimeEnv();
    ...
    const codex = install("codex");
    const codexPath = join(codex, ".codex", "config.toml");
    const shippedTui = parseToml(readFileSync(codexPath, "utf-8")).tui;
    ...
    expect(codexAfter).not.toContain('model_provider = "amazon-bedrock"');
    expect(parseToml(codexAfter).tui).toEqual({ status_line: userStatus });
    const refreshed = run([
      "config", "--project-dir", codex,
      "--from", join(DIST_RELEASE, "codex"), "--harness", "codex", "--yes",
    ], codex, env);
    expect(refreshed.status, refreshed.stdout + refreshed.stderr).toBe(0);
    expect(refreshed.stdout).toContain("Note: restored the shipped [tui] table");
    expect(parseToml(readFileSync(codexPath, "utf-8")).tui).toEqual(shippedTui);
  }, 120_000);
