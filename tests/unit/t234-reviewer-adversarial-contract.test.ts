...
describe("t234 adversarial review contract pins (reviewer-as-verifier)", () => {
  ...
  test("intent-capture binds product-lead review to the grounded artifact contract", () => {
    ...
    expect(stage).toContain("post-summary confirmation with a blank");
    expect(stage).toContain("record a new summary-confirmation decision and human answer");
    expect(stage).toContain("do not append a second section with the same heading");
