...
//
// Native Windows CI exercises the shipped hook with a case-varied project prefix.
// POSIX CI verifies that a project-supplied test-platform env cannot change the
// production hook decision.
...
describe("t07 audit-logger PostToolUse hook (mechanism cli — spawned hook + stdin seam)", () => {
  ...
  test.skipIf(process.platform !== "win32")("win32 semantics: a record write whose path differs only in case is logged with the record breadcrumb", () => {
  ...
  test.skipIf(process.platform !== "win32")("win32 semantics: Edit on a case-varied record path emits ARTIFACT_UPDATED", () => {
    expect(proj.toUpperCase()).not.toBe(proj);
    const { auditDir, recordRoot } = seedIntentShard(proj);
    fire(
  ...
  test.skipIf(process.platform !== "win32")("win32 semantics: a case-varied codekb write carries the codekb breadcrumb", () => {
    expect(proj.toUpperCase()).not.toBe(proj);
    const { auditDir } = seedIntentShard(proj);
  ...
  test.skipIf(process.platform !== "win32")("win32 semantics: a case-varied path outside the record and codekb still emits nothing", () => {
    expect(proj.toUpperCase()).not.toBe(proj);
    const { auditDir } = seedIntentShard(proj);
    const before = readShards(auditDir);
  ...
  test.skipIf(process.platform === "win32")("project test-platform env cannot weaken POSIX containment", () => {
    expect(proj.toUpperCase()).not.toBe(proj);
    const { auditDir, recordRoot } = seedIntentShard(proj);
