...
import { gitCommitSourceListing } from "../../core/tools/aidlc-lib.ts";
...
describe("guard kernel immutable evidence", () => {
  ...
  test("commit replacement: source listings and immutable Git reads retain the named commit's own tree and bytes", () => {
    const repo = initRepo(join(root, "repo"));
    writePath(repo, "source.txt", "original commit bytes\n");
    const commitA = commitAll(repo, "original commit");
    const expected = treeListing(committedTree(repo, commitA));
    const originalListing = gitCommitSourceListing(repo, commitA, false);
    expect(originalListing).not.toBeNull();
    const originalCommit = gitBytes(repo, ["cat-file", "commit", commitA]);

    writePath(repo, "source.txt", "replacement commit bytes\n");
    writePath(repo, "replacement-only.txt", "not in the original tree\n");
    const commitB = commitAll(repo, "replacement commit");
    const replacementCommit = gitBytes(repo, ["cat-file", "commit", commitB]);
    gitText(repo, ["replace", commitA, commitB]);

    expect(gitBytes(repo, ["cat-file", "commit", commitA])).toEqual(
      replacementCommit,
    );
    expect(listingRows(gitCommitSourceListing(repo, commitA, false))).toEqual(
      listingRows(originalListing),
    );
    expect(
      listingRows(immutableCommitSourceListing(repo, commitA, false)),
    ).toEqual([...expected.entries()]);

    const read = immutableGit(repo, ["cat-file", "commit", commitA], {
      encoding: null,
      env: { GIT_NO_REPLACE_OBJECTS: "0" },
    });
    expect(read.status).toBe(0);
    expect(read.stdout).toEqual(originalCommit);
  });

  test("blob replacement: source content identities and immutable blob reads retain the named blob's own bytes", () => {
    const repo = initRepo(join(root, "repo"));
    const originalBytes = Buffer.from("original blob\0bytes\n", "utf-8");
    const replacementBytes = Buffer.from("replacement blob\0bytes\n", "utf-8");
    writePath(repo, "source.bin", originalBytes.toString("utf-8"));
    const commitA = commitAll(repo, "original blob");
    const expected = treeListing(committedTree(repo, commitA));
    const originalListing = gitCommitSourceListing(repo, commitA, false);
    expect(originalListing).not.toBeNull();
    const blobA = gitText(repo, ["rev-parse", `${commitA}:source.bin`]).trim();

    writePath(repo, "source.bin", replacementBytes.toString("utf-8"));
    const commitB = commitAll(repo, "replacement blob");
    const blobB = gitText(repo, ["rev-parse", `${commitB}:source.bin`]).trim();
    gitText(repo, ["replace", blobA, blobB]);

    expect(gitBytes(repo, ["cat-file", "blob", blobA])).toEqual(replacementBytes);
    expect(listingRows(gitCommitSourceListing(repo, commitA, false))).toEqual(
      listingRows(originalListing),
    );
    expect(
    ...
    expect(immutableBlobBytes(repo, blobA)).toEqual(originalBytes);
    expect(immutableBlobSha256(repo, blobA)).toBe(
      `sha256:${createHash("sha256").update(originalBytes).digest("hex")}`,
    );

    const read = immutableGit(repo, ["cat-file", "blob", blobA], {
      encoding: null,
      env: { GIT_NO_REPLACE_OBJECTS: "0" },
    });
    expect(read.status).toBe(0);
    expect(read.stdout).toEqual(originalBytes);
  });
