  ...
  test.skipIf(SKIP_REASON !== null)(
    ...
    () => {
      ...
      try {
        ...
        // Read-only: the status path must not scaffold a workflow record.
        // Session-runtime scratch is expected and gitignored — the PreToolUse
        // hooks write heartbeats to intents/.aidlc-engine/hooks-health/, the
        // session lifecycle writes aidlc/.aidlc-sessions/ and the active-space
        // cursor. What must NOT exist: intents.json, an active-intent cursor,
        // or any intent record dir.
        const intentsDir = join(proj, "aidlc", "spaces", "default", "intents");
        const intentEntries = existsSync(intentsDir) ? readdirSync(intentsDir) : [];
        expect(intentEntries.filter((e) => e !== ".aidlc-engine")).toEqual([]);
        expect(existsSync(join(intentsDir, "intents.json"))).toBe(false);
        expect(existsSync(join(intentsDir, "active-intent"))).toBe(false);
      } finally {
