...
Historically derived from live 3000.6.14 captures that were removed from the tracked tree under PR #996 Item 9; the source capture and provenance remain available only in the pre-rewrite Git history. This table preserves the resulting field contract.
...
S06–S09 tests consume the minimal current-build events in
`native-contracts-3000.10.31.json` for captured replay and `payloads.json` only
for legacy adapter behavior coverage. The 3000.6.14 field contract in this
document is historical knowledge, not an executable fixture.
