...
// Command names only add potential path designators; they never prove an allow.
const ATTACHED_PATH_WRITERS =
  /^(?:dd|tee|curl|wget|cp|mv|install|tar|sed|awk|cc|gcc|g\+\+|clang|clang\+\+|ld|ar)$/i;
...
function shellBarrier(
  ...
  // Keep every complete word, and also inspect attached operands rather than
  // mistaking `of=/store/x` for a relative filename literally beginning `of=`.
  for (const segment of parsed.segments) {
    const writer = segment.some((word) => ATTACHED_PATH_WRITERS.test(basename(word)));
    for (const word of segment) {
      for (let at = word.indexOf("="); at !== -1; at = word.indexOf("=", at + 1)) {
        const operand = word.slice(at + 1);
        if (!operand || /[$`*?[\]{}~]/.test(operand)) {
          return { kind: "unprovable", reason: "an attached operand has no canonical path identity" };
        }
        parsed.words.push(operand);
      }
      if (writer && /^-[A-Za-z].+/.test(word)) {
        parsed.words.push(word.slice(2));
      }
    }
  }
...
/** Invoke Git against original objects, never mutable replacement refs. */
export function immutableGit(
  ...
  return spawnSync("git", ["--no-replace-objects", "-C", repoDir, ...args], {
...
function provenCommit(repoDir: string, commit: string): boolean {
  ...
  const type = immutableGit(repoDir, ["cat-file", "-t", commit]);
  if (type.status !== 0 || String(type.stdout).trim() !== "commit") return false;
  return immutableGit(repoDir, ["cat-file", "-e", `${commit}^{commit}`]).status === 0;
}
...
function withCommitIndex<T>(
  ...
  const objectPath = immutableGit(repoDir, ["rev-parse", "--git-path", "objects"]);
  if (objectPath.status !== 0 || !String(objectPath.stdout).trim()) return null;
  const objectFormat = immutableGit(repoDir, ["rev-parse", "--show-object-format"]);
  ...
  try {
    ...
    const initialized = immutableGit(
      repoDir,
    ...
    const readTree = immutableGit(repoDir, ["read-tree", commit], { env });
...
export function immutableCommitSourceListing(
  ...
  return withCommitIndex(repoDir, commit, (env) => {
    ...
    const listed = immutableGit(repoDir, ["ls-files", "-s", "-z"], { env });
...
export function immutableBlobBytes(
  ...
  const type = immutableGit(repoDir, ["cat-file", "-t", oid]);
  if (type.status !== 0 || String(type.stdout).trim() !== "blob") return null;
  const blob = immutableGit(repoDir, ["cat-file", "blob", oid], { encoding: null });
