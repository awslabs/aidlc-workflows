...
export function readConfigDiagnosticRecords(harnessRoot: string): ConfigDiagnosticRecords {
  ...
  // A persisted `kiro-ide` stamp is still READ - the guard above accepts it on purpose,
  // because an install made by the retired row must remain readable. Map it to its
  // successor before the model-access decision, the same one-directional resolution the
  // runtime uses; rejecting the name above would make that install unreadable instead.
  const modelHarness = currentDistribution(distribution) as ModelHarness;
  return {
    runtime: normalizeRuntimeRecord(value.runtime),
    providers: providers
      ? harnessOwnsModelAccess(modelHarness)
        ? providers
        : reconcileProviderActions(providers, modelHarness, false)
      : null,
