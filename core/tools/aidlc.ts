...
const SENSOR_WORKERS = [
  ...
  ["html-shape", TOOLS.sensorHtmlShape],
...
export const ROUTES: readonly Route[] = [
  ...
  {
    // The review UI: a browser for reviewing stage artifacts, answering question
    // rounds, and - with an agent runner - starting and driving intents. `start`
    // needs no harness session.
    id: "top-ui",
    namespace: "public",
    group: "top",
    kind: "top-prefix",
    ...
    outputModes: ["human", "json"],
    human: [
      { command: "ui start", summary: "start the review UI for this project and open it in the browser" },
    ],
    all: [
      "ui start [--project-dir <path>]",
      "ui serve --project-dir <absolute-path>",
      "ui status [--project-dir <path>] [--json]",
      "ui stop [--project-dir <path>]",
      "ui open [--project-dir <path>]",
      "ui vendor-agent [--project-dir <path>]",
    ],
  },
  {
    id: "top-version",
  ...
  },
  {
    ...
    kind: "noun-passthrough",
    classification: "passthrough",
    verbs: ["decision", "answer", "review", "link", "answers-apply", "answers-wait", "decision-wait", "decision-apply"],
    tool: TOOLS.log,
    ...HIDDEN_ENGINE,
  },
  {
  ...
  {
    // HTML stage artifacts and browser explainers: scaffold, check, export.
    id: "html",
    group: "html",
    kind: "noun-passthrough",
    classification: "passthrough",
    verbs: ["text", "check", "export", "scaffold"],
    tool: TOOLS.html,
    ...HIDDEN_ENGINE,
  },
  {
...
export function renderHumanHelp(): string {
  ...
  return [
    ...
    "",
    heading("REVIEW IN THE BROWSER", out),
    row("ui", "Start, inspect, or stop the review UI for this project (no agent session needed)"),
    "",
    ...
    `  ${cmd(`${invoke} ui start`, out)}                   open the review UI and start work from the browser`,
    "",
...
export function renderCommandHelp(command: PublicCommand): string {
  ...
  if (command === "ui") {
    const commandRow = (name: string, summary: string): string =>
      `  ${cmd(name, out)}${" ".repeat(14 - name.length)}${summary}`;
    return [
      "Review stage artifacts, answer questions, and start work in the browser",
      "",
      heading("USAGE", out),
      `  ${cmd(`${invoke} ui <command> [options]`, out)}`,
      "",
      heading("COMMANDS", out),
      commandRow("start", "Start the review UI for this project (detached) and open it; no agent session needed"),
      commandRow("status", "Print whether the review UI is running, and its address (--json for a record)"),
      commandRow("open", "Open the running review UI in the browser"),
      commandRow("stop", "Stop the review UI for this project"),
      commandRow("serve", "Run the review UI in the foreground (what start launches detached)"),
      commandRow("vendor-agent", "Install this harness's pinned agent adapter so start works without a package registry"),
      "",
      heading("OPTIONS", out),
      ...
      "",
      heading("EXAMPLES", out),
      `  ${cmd(`${invoke} ui start`, out)}`,
      `  ${cmd(`${invoke} ui status --json`, out)}`,
      "",
      dim("Set AIDLC_REVIEW_UI=1 in the harness's environment to publish review links at approval gates.", out),
      dim(`Run '${invoke} --help' for the public command list.`, out),
      "",
    ].join("\n");
  }
  const descriptions: Record<Exclude<PublicCommand, "config" | "ui">, string> = {
  ...
  const examples: Partial<Record<Exclude<PublicCommand, "config" | "ui">, string[]>> = {
...
async function loadDelegate(tool: string): Promise<DelegateModule | null> {
  switch (tool) {
    ...
    case TOOLS.sensorHtmlShape:
      return import("./aidlc-sensor-html-shape.ts");
    ...
    case TOOLS.reviewUi:
      return import("./aidlc-review-ui.ts");
    case TOOLS.html:
      return import("./aidlc-html.ts");
    case TOOLS.validate:
...
async function publicCommandGrammarError(
  ...
  if (command === "ui") return validatePublicUiArgs(normalized);
  return validateSimplePublicGrammar(command, normalized);
}

const UI_COMMANDS = new Set(["start", "serve", "status", "stop", "open", "vendor-agent"]);

/** `aidlc ui <command> [--project-dir <path>] [--json]`; --json is status-only. */
function validatePublicUiArgs(argv: readonly string[]): string | null {
  const subcommand = argv[1];
  if (subcommand === undefined) return "ui requires a command: start, status, open, stop, serve, or vendor-agent";
  if (!UI_COMMANDS.has(subcommand)) {
    return `unknown ui command ${JSON.stringify(subcommand)}; expected start, status, open, stop, serve, or vendor-agent`;
  }
  const seen = new Set<string>();
  for (let index = 2; index < argv.length; index++) {
    ...
    if (token === "--json") {
      if (subcommand !== "status") return "--json is accepted by ui status only";
      if (seen.has(token)) return `${token} may be specified only once`;
      seen.add(token);
      continue;
    }
    if (token.startsWith("--")) return `unknown ui option ${token}`;
    return `unexpected ui positional ${JSON.stringify(token)}`;
  }
  return null;
}
