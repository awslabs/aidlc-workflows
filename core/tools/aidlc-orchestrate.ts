...
function composeDispatchDirective(
  ...
  if (inFlight) {
    parts.push(
      ...
      "A request to turn sensors, learnings, summary confirmation, or reviews on or off is not a stage flip and recompose cannot land it: name the per-intent switch the human types instead (/aidlc --sensors on|off, --learnings on|off, or --summary-confirmation on|off, with $aidlc on Codex). --review advisory|none only lowers reviews, and --review adversarial clears a lowering but never lifts the running scope's review_cap: for stronger reviews than that cap, name the cap and the scope change that would allow them (/aidlc --scope <name>, which also recalculates the pending stages), never --review.",
  ...
  const proposalShape = inFlight
    ...
    : "mode matched|custom, scopeName, a nonblank creationDescription, an ars block (the five component scores with method codekb|fallback), an arsRationale, the per-stage EXECUTE/SKIP grid, ONE guardPolicy value (strict|relaxed|off: a matched proposal carries the stock scope's default, a custom one the composer's choice) with a one-line guardPolicyRationale, the four scopeSettings (sensors, learnings, and summary_confirmation on|off, review_cap adversarial|advisory|none: a matched proposal carries the stock scope's values, a custom one the composer's choice) with a one-line scopeSettingsRationale, a per-SKIP rationale, a summary the validator computed, and two pre-rendered markdown tables (ARS scores with bands; per-stage decisions with reasoning)";
