...
/**
 * argv prefix that re-enters this install's own CLI when it runs as the
 * compiled `aidlc` binary (`[executable, ...route]`), or null under bun, where
 * callers spawn the sibling tool file with `process.execPath` instead.
 */
export function compiledInvocationArgv(...route: string[]): string[] | null {
  const executable = compiledExecutable();
  return executable ? [executable, ...route] : null;
}
