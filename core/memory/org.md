...
- **Cross-references — link them, don't leave them bare**: A stable ID (`FR1.2`, `NFR3`, `US1.1`, `AC1.1.1`, `ENT-001`, `ADR-001`, `unit-4`, `BR1.1`) stays a preserved literal — never translated or renumbered — but when it appears as a **cross-reference to the artifact that defines it**, wrap that literal in a relative Markdown link to the definition's anchor so a reader can click through: `[FR1.2](./requirements.md#fr1-2)` across files, or `[FR1.2](#fr1-2)` within the same file. **Anchor derivation:** the definition heading carries a portable HTML anchor `<a id="..."></a>` whose value is the ID lower-cased with every non-alphanumeric run replaced by a single hyphen — `FR1.2` → `fr1-2`, `NFR3` → `nfr3`, `ADR-001` → `adr-001`, `ENT-001` → `ent-001` — e.g. `#### <a id="fr1-2"></a>FR1.2: …`. Use `<a id>` rather than a Kramdown `{#fr1-2}` attribute: `<a id>` renders a jump target in CommonMark and on GitHub, whereas `{#…}` does not render on GitHub (it is a Kramdown/Pandoc extension), so the link would be dead there. **Two exceptions, both stay bare:** an ID at its **own point of definition** (the heading that introduces it — the anchor lives there, so it is not a link to itself), and a token that names **no defined artifact yet** (a forward reference to an ID whose file does not exist — it stays bare until the target is authored). ADRs already link the ID literal rather than leaving it bare (the superseding examples in `knowledge/aidlc-architect-agent/adr-template.md` link each `ADR-NNN` to its own file); this adds the portable `<a id>` anchor so same-file references resolve too, and generalizes linking to the other ID families. The link is presentation around the token: the visible ID text is unchanged, and the lower-cased anchor is invisible to the claim-sources / traceability / required-sections sensors — claim-sources reads the visible link text, and the traceability ID regexes are case-sensitive so a lower-cased anchor is not re-matched (an UPPER-cased anchor would be, which is a further reason the derivation rule lower-cases it).
