...
- **Onboard skill** (read-write, user-invocable): `{{HARNESS_DIR}}/skills/aidlc-onboard/` — typed as `{{INVOKE}}-onboard --source <path>`. The **bulk, up-front** path for external material a human hands the engine (coding standards, security policy, any file or directory): it captures the material byte-exact into `aidlc/spaces/<active-space>/onboard/` via `bun {{HARNESS_DIR}}/tools/aidlc-onboard.ts`, classifies each item, and — only after an explicit human yes at a gate — promotes preventative controls into `memory/{project,team}.md` as rules via `bun {{HARNESS_DIR}}/tools/aidlc-learnings.ts persist-rule --text-file <path>` (approved rule text goes through a scratch file, never a `--text` command-line argument — document-derived text must never reach a shell, which would expand `$(…)` inside it before the tool could inspect it). It shares the rule-write core with the in-workflow learning loop but is a distinct front door: onboard runs *before* any workflow, so no stage is on its path and its `RULE_LEARNED` rows carry the `aidlc-onboard` namespace instead of a stage slug (which is why they do not appear in the per-stage learnings rollup). It never advances the workflow stage pointer. Text only today — a binary source is captured byte-exact but classifies as `unsupported-binary`.
