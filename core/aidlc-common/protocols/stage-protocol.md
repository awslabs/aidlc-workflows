...
| **Construction checkpoint** | An engine-issued verification and approval of one completed Unit, or of the first integrated skeleton Unit. Current artifact/source/attempt-bound proof is required to approve; on a fresh clone, an already-approved checkpoint keeps its committed receipts instead of the machine-local proof. Skeleton approval is always human; an ordinary Unit may be approved automatically under a recorded grant. Team-owned `unit_gate` and legacy stage gates use their own policies. |
