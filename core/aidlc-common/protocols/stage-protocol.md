...
- On done run `bun {{HARNESS_DIR}}/tools/aidlc-log.ts answers-apply --stage <slug> --questions-file <path> [--unit <unit>]`, then continue exactly as Step 3b: read the file, present the consolidated summary, and run the same **Looks correct / Request changes** checkpoint.
