...
**CWD drift warning**: If a stage runs `cd` in Bash (e.g., `cd todo-app/server && npm install`), subsequent `bun {{HARNESS_DIR}}/tools/...` calls using relative paths will fail with "Module not found". Never change the working directory in the main shell: run `cd` commands in subshells, `(cd subdir && npm install)`, so every engine command keeps the bare relative form the harness pre-approves. An absolute-path form, a `cd ... &&` prefix, or an environment-variable prefix falls outside that pre-approval and prompts the user. A pipe, a redirection, or a `$(...)` capture is stronger than a prompt on the harnesses that enforce a shell boundary — Kiro, Copilot and OpenCode — where a pre-approved command carrying any of them is REFUSED outright rather than offered for approval, because a wildcard at the end of a pattern matches trailing text those hosts do not treat as a command boundary, so a redirection would otherwise write a file without ever presenting as a file write. A single trailing `2>&1` is the one permitted exception on Kiro and Copilot; OpenCode refuses it along with every other redirection. So keep every engine command in the bare relative form: if you need its output in a file, run the command and write the file with a file tool.
