...
| `consumes[].kinds` | string[] | optional | Inline list of Unit kinds (`service` \| `spec` \| `ui` \| `packaging` \| `library`) this consume applies to; only valid on a `for_each: unit-of-work` stage. Omit for every kind. A Unit whose kind is not listed does not receive the consume in its directive and is not asked to reference it by `upstream-coverage`; an untagged Unit keeps it. The consumer-side twin of `produces_kinds` |
