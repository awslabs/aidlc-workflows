...
1. **Invoke reviewer sub-agent.** Before every dispatch, not only the first,
   ...
   - The required Findings table columns: `ID | Severity | Location | Finding | Required action | Status`, with a Markdown separator row. Include all six; new findings have status `New`.
