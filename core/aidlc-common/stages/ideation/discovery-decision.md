...
the workflow explicitly: `bun {{HARNESS_DIR}}/tools/aidlc-orchestrate.ts park`.
...
2. Run `bun {{HARNESS_DIR}}/tools/aidlc-orchestrate.ts next` once: the engine
   ...
   `bun {{HARNESS_DIR}}/tools/aidlc-utility.ts set-status --stage <named stage>`.
3. Mark every ideation stage the new scope expects but this run already
   ...
   run: `bun {{HARNESS_DIR}}/tools/aidlc-state.ts skip <slug> --reason
