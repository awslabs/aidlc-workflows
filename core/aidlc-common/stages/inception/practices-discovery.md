...
2. Read the affirmed `Integration mode` field. For `pr`, run:
   ...
   ```bash
   {{INVOKE}} engine state set-integration-mode pr
   {{INVOKE}} engine recompose --add pr-integration
   ```

   For `direct`, run:

   ```bash
   {{INVOKE}} engine state set-integration-mode absent
   {{INVOKE}} engine recompose --skip pr-integration
   ```
