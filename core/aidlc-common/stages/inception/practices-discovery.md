...
- **Brownfield:** use available reverse-engineering artifacts and workspace
  ...
  ```bash
  bun {{HARNESS_DIR}}/tools/aidlc-pr.ts detect --repo <owner/repo>
  ```
...
2. Read the affirmed `Integration mode` field. For `pr`, run:
   ...
   ```bash
   bun {{HARNESS_DIR}}/tools/aidlc-state.ts set-integration-mode pr
   bun {{HARNESS_DIR}}/tools/aidlc-utility.ts recompose --add pr-integration
   ```

   For `direct`, run:

   ```bash
   bun {{HARNESS_DIR}}/tools/aidlc-state.ts set-integration-mode absent
   bun {{HARNESS_DIR}}/tools/aidlc-utility.ts recompose --skip pr-integration
   ```
