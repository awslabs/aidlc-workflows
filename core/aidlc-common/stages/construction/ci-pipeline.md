...
scopes:
  ...
  - classic
  - workshop
inputs: Code generation output and infrastructure design for every Unit under <record>/construction/units/, build/test results from the stage-level build-and-test directory
...
- Read code summaries from `<record>/construction/units/<unit>/code-generation/` (if they exist)
