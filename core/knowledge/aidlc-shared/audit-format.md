...
| Event | When | Required Fields | Emitter |
|-------|------|-----------------|---------|
| `ARTIFACT_CREATED` | New artifact written in the active intent record or space-level codekb tree | Timestamp, Tool, File, Context, optional Summary Authorization Id (the active summary confirmation for the written stage and Unit at write time, read from `<record>/.aidlc-summary-authorization/<stage>/<unit or stage-level>.json`; absent when no confirmation is active), optional Role (the dispatched agent's `agent_type` when the payload carries one, else the state file's `Active Agent`), optional Mode (the compiled graph node's `mode` for the state file's Current Stage), optional Actor (`subagent:<agent_type>`, or `main-session` when the harness payload carries identity and names none; absent when the payload declares identity unavailable). The three attribution fields are derived from state, graph, and payload identity only, never from prompt text | `hooks/aidlc-write-audit-log.ts` (PostToolUse; Write to net-new path) |
| `ARTIFACT_UPDATED` | Existing artifact modified in either tree | Timestamp, Tool, File, Context, optional Summary Authorization Id, optional Role, optional Mode, optional Actor (all as for `ARTIFACT_CREATED`) | `hooks/aidlc-write-audit-log.ts` (PostToolUse; Edit, or Write overwriting existing) |
