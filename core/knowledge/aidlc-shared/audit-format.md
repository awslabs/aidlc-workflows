...
| Event | When | Required Fields | Emitter |
|-------|------|-----------------|---------|
| `GUARD_POLICY_SET` | `config-change --guard-policy strict` raises the policy, `scope-change` carries a scope-supplied value, or a governed checkpoint observes a memory edit changing the effective value. Renaming a sole retired field without changing its value emits no row | Timestamp, Old Value, New Value, Source (`you`, `scope <name>`, or `<layer>.md`). Configuration and scope changes record the previously persisted intent value in Old Value (raw text if invalid; `strict` when no line existed), not the memory-effective value. Checkpoint observations retain effective old/new values. | `tools/aidlc-utility.ts` batches configuration changes; `tools/aidlc-lib.ts` (`governedGuardPolicy` through `appendGuardPolicySetRow`) records checkpoint observations |
...
| `GUARD_STOOD_ASIDE` | A fence let an action through instead of refusing it because the Guard Policy word, compatible persisted `Guards Off` state, or its harness environment kill switch lowered it. Prompt text, a human turn, and a picked remedy never lower a fence. That configuration is separate from this row's conversational Authority. The row is emitted when an action passes the lowered fence, not when configuration changes | Timestamp, Guard (the fence), Authority (`grant`, `instruction`, or `none`: who was working when it passed, not what opened the fence), Grant (how a grant was proven, when there was one: `turn-marker`, `marker-sequence`, `dispatch-stamp`, or `none`), Actor (`main`, `subagent`, or `unattended`), optional Stage, Tool, Details | `tools/aidlc-lib.ts` (`recordGuardStoodAside`, called by the fence hooks) |
...
| `GUARD_DISABLED` | A tool call passed the Plan Approval guard because its deterministic off-switch was set while a workflow existed. One row is written per streak; compatible ledgers can also contain per-work switch rows from earlier builds of this release | Timestamp, Guard (`plan-approval-guard` from the hook; `plan-approval`, `review-freeze`, `state-transition`, or `reviewer-scope` from compatible historical rows), Tool (hook rows) or Scope and Source (`you`, historical switch rows) | `hooks/aidlc-plan-approval-guard.ts` (PreToolUse); historical rows can also come from earlier builds of this release |
