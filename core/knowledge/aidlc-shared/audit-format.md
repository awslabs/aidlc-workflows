...
## Event Registry (100 events, 25 categories)
...
| Event | When | Required Fields | Emitter |
|-------|------|-----------------|---------|
| `DECISION_RECORDED` | Before presenting a non-gate structured question, to record the options shown. Consolidated-summary and verification-command prompts also carry checkpoint identity; verification-command prompts bind the canonical command digest | Timestamp, Stage, Decision, Options; optional Checkpoint, Command SHA-256, Questions File, Unit, Attempt Generation, Workflow | `tools/aidlc-log.ts decision` |
...
| `VERIFICATION_COMMAND_RECORDED` | Human approved an intent's Construction verification command after a matching pending decision and a fresh human turn; the latest current-workflow receipt authorizes only the matching state command and is reserved from the public audit CLI | Timestamp, Stage, Checkpoint (`Construction Verification Command`), Command SHA-256 (hex SHA-256 of the canonical trimmed single-line command), Command Label (control characters removed, truncated to 120 characters), User Input (`Approve`); optional Workflow | `tools/aidlc-log.ts answer --checkpoint verification-command` |
