...
| `WORKTREE_DISCARDED` | Bolt's recoverable working-tree snapshot (or remaining branch tip) and reviewed source refs parked under `refs/aidlc/parked/<slug>/<stamp>/` before audit emission, then live checkout and branch removed | Timestamp, Bolt slug, Worktree path, Reason, Parked ref (namespace prefix), Parked commit (snapshot commit, or the remaining branch tip for a branch-only park marked by `<parked ref>/branch-tip`, or `-` when no commit could be parked) | `tools/aidlc-worktree.ts` (`discard`) |
