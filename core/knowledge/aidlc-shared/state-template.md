...
- **Guard Policy**: [strict/relaxed/off, then its source in parentheses: `(from scope <name>)`, `(from <layer>.md)`, or `(set by you)`; written at intent creation with the resolved value, raised by `/aidlc --guard-policy strict` or its plain-chat request, read by value only. When a record carries only the retired `**Change Control**` line, every `/aidlc` run announces that a carried-over relaxed or off value now also lowers fences for work nobody directed and every pass is recorded in the audit trail, and asks the person to keep it or raise the fences again; the notice repeats until the prompt-submit hook renames the field without changing its value or the person raises Guard Policy to strict; a retired strict line alone gets no notice. If both lines carry different policy words, strict applies with source `conflicting state lines` unless memory holds strict, and `next` repeats a notice containing both raw values until the person chooses strict; if both agree, `Guard Policy` is used. Any policy write removes the retired line and retains only `Guard Policy`; `next` writes nothing]
- **Guards Off**: [compatibility field for intents written by earlier builds: a comma list of `plan-approval`, `review-freeze`, `state-transition`, `reviewer-scope` followed by `(set by you)`, or `none`; new chat commands cannot add entries; a persisted human-presence entry is ignored]
- **Guards On**: [only present once a fence was forced on above the policy word for this piece of work: a comma list of `plan-approval`, `review-freeze`, `state-transition`, `reviewer-scope` followed by `(set by you)`, or `none`; written by `/aidlc config set guard.<fence> on`; a persisted human-presence entry is ignored; precedence is environment kill switch, Guards Off, Guards On, Guard Policy, then on by default]
