...
- **Guard Policy**: [strict/relaxed/off, then its source in parentheses: `(from scope <name>)`, `(from <layer>.md)`, or `(set by you)`; written at intent creation with the resolved value, rewritten by `/aidlc --guard-policy` or the plain-chat request, read by value only. A record from an earlier release carries this line as `**Change Control**`; every `/aidlc` run announces that the carried-over relaxed or off value now also lowers fences for work nobody directed and every pass is recorded in the audit trail, and asks the person to keep it or raise the fences again; the notice repeats until the person chooses a Guard Policy setting, which renames the line in place and stops the notice; `next` writes nothing; a retired strict line gets no notice and is renamed on the next Guard Policy setting write]
