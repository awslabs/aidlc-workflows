...
```bash
bun {{HARNESS_DIR}}/tools/aidlc-onboard.ts capture \
...
```bash
bun {{HARNESS_DIR}}/tools/aidlc-onboard.ts classify \
...
```bash
bun {{HARNESS_DIR}}/tools/aidlc-onboard.ts list
```
...
```bash
bun {{HARNESS_DIR}}/tools/aidlc-learnings.ts persist-rule \
  --scope <project|team> \
