...
3. **In-flight** (a workflow is running): read the live state file's Stage
   ...
   must be rejected, not proposed.

## Procedure

1. **Detect.** Run `bun {{HARNESS_DIR}}/tools/aidlc-utility.ts detect --json`.
...
2. **Read the repertoire.** Read the stock scope definitions under the
   ...
   SKILL.md (or `{{HARNESS_DIR}}/tools/data/stage-graph.json`) for the stage
   list and phases.
...
4. **Validate.** Before the proposal is shown, run the deterministic grid
   check (`bun {{HARNESS_DIR}}/tools/aidlc-graph.ts validate-grid ...` once it
   ships; until then `validate <scopeName>` for a stock match). A grid whose
   required input has no producer on the EXECUTE set must be fixed or the
   SKIP withdrawn - never shown as-is.
...
- Making a scope inferable is an explicit human choice at the gate ("make
  ...
  effect of composing. If keywords are granted, check them for collisions
  with the stock scopes' keywords first and surface any collision.
