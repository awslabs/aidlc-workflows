...
Open `your-project/` in Kiro IDE. The install ships `.kiro/settings/cli.json` with `chat.defaultAgent` set to `aidlc` and registers the framework hooks as `.kiro/hooks/aidlc-*.json` files (the IDE's v2 hook mechanism; Kiro IDE 1.x no longer executes the legacy `.kiro.hook` format). In the chat panel, run `/aidlc --doctor` to verify, then `/aidlc <description>` to start.
...
│   ├── kiro-ide/               #   manifest.ts · orchestrator · agent JSONs · v2 hook JSONs · settings · onboarding fills
