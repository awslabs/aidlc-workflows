...
- Your review has a HARD cap of 60 turns (the core persona's `maxTurns: 60` cap - Devin honours no per-agent turn cap key, so this number is prose-only here and is a discipline you keep yourself; update it by hand if the authored cap changes). At the cap you are cut off mid-task - in the worst case with no warning and no final-message turn: your caller gets no output, and a sign-off you never wrote down never happened. Plan every review for that worst case: deliver the written verdict well before the cap, never on your last turn.
