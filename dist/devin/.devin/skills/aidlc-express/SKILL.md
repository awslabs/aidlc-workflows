...
1. `directive = bun .devin/tools/aidlc-orchestrate.ts next --scope express $ARGUMENTS`
2. Before acting on each directive, read
   `.devin/aidlc-common/protocols/stage-protocol.md` once per session,
   then read every
   `.devin/aidlc-common/protocols/stage-protocol-<module>.md` named by
...
- **Default to CONTINUATION.** Treat the input as new-work ONLY when it clearly
  ...
  (`bun .devin/tools/aidlc.ts engine intent list --json` gives its `slug` and
...
- **On CONFIRM**, re-run `next` with `--new-intent`, the confirmed scope, and the
  ...
  ```bash
  bun .devin/tools/aidlc.ts engine orchestrate next --new-intent --scope <the confirmed scope> "<the new-work description>"
  ```
  ...
  fresh session (exit or restart Devin CLI and start a new session) and invoke `/aidlc` to begin the
