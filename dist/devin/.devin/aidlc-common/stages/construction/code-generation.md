...
```bash
bun .devin/tools/aidlc-testing-posture.ts render
```
...
```bash
bun .devin/tools/aidlc-testing-posture.ts fingerprint --unit "<directive.unit>"
```
...
```bash
bun .devin/tools/aidlc-testing-posture.ts fingerprint --stage-level
```
...
```bash
bun .devin/tools/aidlc-log.ts decision --stage code-generation \
...
- First, verbatim and unedited, the output of
  `bun .devin/tools/aidlc-testing-posture.ts brief --unit
...
`bun .devin/tools/aidlc.ts engine orchestrate report --stage code-generation --result <outcome>`.
