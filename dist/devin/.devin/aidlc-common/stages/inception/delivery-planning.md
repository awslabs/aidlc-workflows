...
`bun .devin/tools/aidlc.ts engine orchestrate report --stage delivery-planning --result <outcome>`.
...
`bun .devin/tools/aidlc.ts engine state set-construction-iteration unit-major`.
...
then
`bun .devin/tools/aidlc-state.ts set-unit-ownership team`. Team ownership
...
`bun .devin/tools/aidlc-state.ts set-unit-gate-rhythm per-stage` or
