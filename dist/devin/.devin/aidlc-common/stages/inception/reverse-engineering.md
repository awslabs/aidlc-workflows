...
`bun .devin/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .devin/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo>
```
...
```
bun .devin/tools/aidlc-state.ts reuse-artifact reverse-engineering --decision keep --artifacts "<codekb-path output>" [--repo <repo>] [--single]
```
...
```
bun .devin/tools/aidlc-utility.ts codekb-snapshot --repo <repo> --paths <source paths> --json
```
...
- If every repo is reused on an ordinary workflow run, report the stage as
  ...
  `bun .devin/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result skipped --reason "codekb reuse: all resolved stores CURRENT, human chose reuse"`.
...
Template in `.devin/knowledge/aidlc-developer-agent/re-artifacts.md`:
...
```
bun .devin/tools/aidlc-log.ts link --stage reverse-engineering --link aidlc-developer-agent --artifact "<developer scan handoff path>" [--repo <repo>] [--single]
```
...
its output verbatim:
   ...
   ```
   bun .devin/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo> --mint --paths <analyzed paths>
   ```
...
```
bun .devin/tools/aidlc.ts engine workspace codekb --repo <repo>
```
...
```
bun .devin/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo> --compare <record>/inception/reverse-engineering/scope-draft-<repo>.md
```
...
```
bun .devin/tools/aidlc-utility.ts codekb-publish \
  --repo <repo> \
...
```
bun .devin/tools/aidlc-log.ts link --stage reverse-engineering --link aidlc-architect-agent [--repo <repo>] [--single]
```
...
`bun .devin/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result <outcome>`.
