...
```bash
bun .devin/tools/aidlc.ts engine state practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .devin/tools/aidlc.ts engine orchestrate report --stage
...
```bash
bun .devin/tools/aidlc.ts engine state practices-promote \
