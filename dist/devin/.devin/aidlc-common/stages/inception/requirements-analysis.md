...
- Run the fixed command
  `bun .devin/tools/aidlc-utility.ts project-description` and use its
...
- Read the selected file only through the fixed command
  `bun .devin/tools/aidlc-utility.ts document-input`.
...
`bun .devin/tools/aidlc.ts engine orchestrate report --stage requirements-analysis --result <outcome>`.
...
`bun .devin/tools/aidlc.ts engine recompose --add user-stories`
