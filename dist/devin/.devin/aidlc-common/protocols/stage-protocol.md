...
2. [ ] **Log non-gate questions via `aidlc-log.ts`** — before presenting a structured question that is not an approval gate: `bun .devin/tools/aidlc.ts engine log decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .devin/tools/aidlc.ts engine log answer --stage <slug> --details "<exact choice>"`. Approval choices go only through `aidlc-orchestrate.ts report`. (§2, §3)
...
`.devin/aidlc-common/protocols/stage-protocol-construction.md`.
...
2. After the learnings answer is logged: `bun .devin/tools/aidlc.ts engine orchestrate report --stage <slug> --result awaiting-approval` marks `[-]` -> `[?]` and emits `STAGE_AWAITING_APPROVAL`. `/aidlc --status` now truthfully shows the held gate. These are internal bookkeeping steps: run them, never narrate them. This step is bookkeeping the user has no stake in: **SAY:** nothing for it, not that a gate is being opened, not that anything is being recorded. Go from the learnings answer straight into the question below.
   - If the report instead refuses because a blocking gate sensor found issues or could not produce a verified pass, the approval gate is NOT open. In interactive mode, run `bun .devin/tools/aidlc.ts engine log decision --stage <slug> --decision "Blocking gate sensor failure" --options "Fix findings,Override blocking sensors"` and present those two options as a separate structured question. END YOUR TURN.
...
`bun .devin/tools/aidlc.ts engine gen scope-table` when you need those
...
- **Consolidated summary before generation**: After all questions have been
  ...
  `bun .devin/tools/aidlc-review-brief.ts summary --stage "<directive.stage>" --questions-file "<questions-path>"`;
  ...
  `bun .devin/tools/aidlc.ts engine log decision --stage <slug>
  ...
  `bun .devin/tools/aidlc.ts engine log answer --stage <slug>
  --checkpoint summary-confirmation --questions-file "<questions-path>"
...
1. BEFORE presenting the approval question: `bun .devin/tools/aidlc.ts engine orchestrate report --stage <slug> --result awaiting-approval`.
...
1. BEFORE presenting the question: `bun .devin/tools/aidlc.ts engine log decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .devin/tools/aidlc.ts engine log answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
**Enforcement:** State file updates happen automatically via the PostToolUse hook when `TaskUpdate` sets a stage task to `in_progress` with a `[slug]` suffix in `activeForm`. At stage END, `bun .devin/tools/aidlc.ts engine orchestrate report --stage <slug> --result approved --user-input "<exact choice>"` marks the completed stage `[x]`, auto-advances to the next in-scope stage, and handles completion bookkeeping. Do not skip the intermediate `[-]` state by going directly from `[ ]` to `[x]`.
...
```bash
bun .devin/tools/aidlc.ts engine orchestrate report \
...
```bash
bun .devin/tools/aidlc.ts engine state lookup phase-of SLUG          # → phase name
bun .devin/tools/aidlc.ts engine state lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .devin/tools/aidlc.ts engine state lookup agent-for SLUG          # → lead agent name
bun .devin/tools/aidlc.ts engine state lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
`.devin/aidlc-common/protocols/stage-protocol-ensemble.md`.
...
`bun .devin/tools/aidlc.ts engine gen scope-table` for the current
...
| **Scope** | Controls which stages execute and at what depth. Eleven built-in scopes, one file per scope under `.devin/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, classic, workshop, express. Custom scopes can be added without editing this file. |
...
`.devin/aidlc-common/protocols/stage-protocol-reviewer.md`.
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .devin/tools/aidlc.ts engine learnings surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file — `{ stage_slug, space, intent, selections[] }`, where `space`/`intent` are carried verbatim from step 2's surface output — and call:
   ```bash
   bun .devin/tools/aidlc.ts engine learnings persist --slug <stage-slug> --selections-json <path>
   ```
...
```bash
bun .devin/tools/aidlc.ts engine state reuse-artifact <stage-slug> \
