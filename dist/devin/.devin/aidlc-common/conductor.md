...
1. The engine creates `memory.md` from
   `.devin/knowledge/aidlc-shared/memory-template.md` when it emits the
   ...
   command: `mkdir -p "$(dirname "<memory_path>")" && { [ -f "<memory_path>" ] || cp ".devin/knowledge/aidlc-shared/memory-template.md" "<memory_path>"; }`.
...
via `bun .devin/tools/aidlc.ts engine state practices-event --type override` before
