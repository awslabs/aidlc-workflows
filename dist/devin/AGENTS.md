...
- **Settings**: `.devin/config.json` pre-approves file tools, delegation, questions, and web tools. Framework shell grants cover `bun .devin/tools/*`, `bun run .devin/tools/*`, and `date -u`. No deny list ships. Hooks are not a general destructive-command security boundary; use Devin/OS security controls.
