...
under `.omp/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
`.omp/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.omp/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.omp/tools/data/stage-graph.json`. Consumers continue to read the compiled
