...
3. `.omp/knowledge/aidlc-operations-agent/` -- agent-specific methodology
4. `.omp/aidlc-common/rules/` -- team-affirmed practices (read per `knowledge/aidlc-shared/rules-reading.md` fallback chain `team.md → org.md → hardcoded defaults`). Consult `## Deployment` for release-cadence and on-call rotation expectations when designing observability dashboards, alert thresholds, and runbooks.
