...
3. `.omp/knowledge/aidlc-architect-agent/` — agent-specific methodology
4. `.omp/aidlc-common/rules/` — team-affirmed practices (read per `knowledge/aidlc-shared/rules-reading.md` fallback chain `team.md → org.md → hardcoded defaults`). Consult `## Code Style` and `## Branching` when architectural decisions touch coding-convention or repository-topology choices.
5. `aidlc-docs/knowledge/aidlc-shared/` — team shared knowledge (if exists)
6. `aidlc-docs/knowledge/aidlc-architect-agent/` — team agent-specific knowledge (if exists)
