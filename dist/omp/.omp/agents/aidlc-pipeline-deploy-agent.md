...
- Read team practices via `.omp/aidlc-common/rules/aidlc-team.md` (fall back to `org.md` per `knowledge/aidlc-shared/rules-reading.md`); match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `aidlc-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `bun .omp/tools/aidlc-worktree.ts` from the main repo checkout; `aidlc-worktree` itself emits the audit event audit-first before invoking git
...
1. `.omp/aidlc-common/rules/` -- execution guardrails
2. `.omp/knowledge/aidlc-shared/` -- shared methodology
3. `.omp/knowledge/aidlc-pipeline-deploy-agent/` -- agent-specific methodology
4. `.omp/aidlc-common/rules/` -- team-affirmed practices (read per `knowledge/aidlc-shared/rules-reading.md`)
