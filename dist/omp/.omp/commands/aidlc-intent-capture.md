---
description: >
  Run the AI-DLC `intent-capture` stage (ideation phase, 1.1) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage intent-capture --single`: the engine
  emits one run-stage directive for `intent-capture` and the conductor runs it, then the single-stage run
...
Run the `intent-capture` stage (Intent Capture & Framing) on its own. This is opt-in packaging over `/aidlc --stage intent-capture --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "intent-capture", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "intent-capture", "result": "completed" }
   ```
