---
description: >
  Run the AI-DLC `nfr-requirements` stage (construction phase, 3.2) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage nfr-requirements --single`: the engine
  emits one run-stage directive for `nfr-requirements` and the conductor runs it, then the single-stage run
...
Run the `nfr-requirements` stage (NFR Requirements) on its own. This is opt-in packaging over `/aidlc --stage nfr-requirements --single`; the same stage is always reachable via that flag without this command.
