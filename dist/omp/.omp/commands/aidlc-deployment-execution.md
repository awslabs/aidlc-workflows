---
description: >
  Run the AI-DLC `deployment-execution` stage (operation phase, 4.3) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage deployment-execution --single`: the engine
  emits one run-stage directive for `deployment-execution` and the conductor runs it, then the single-stage run
...
Run the `deployment-execution` stage (Deployment Execution) on its own. This is opt-in packaging over `/aidlc --stage deployment-execution --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "deployment-execution", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "deployment-execution", "result": "completed" }
   ```
