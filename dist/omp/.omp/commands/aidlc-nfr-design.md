---
description: >
  Run the AI-DLC `nfr-design` stage (construction phase, 3.3) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage nfr-design --single`: the engine
  emits one run-stage directive for `nfr-design` and the conductor runs it, then the single-stage run
...
Run the `nfr-design` stage (NFR Design) on its own. This is opt-in packaging over `/aidlc --stage nfr-design --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "nfr-design", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "nfr-design", "result": "completed" }
   ```
