---
description: >
  Run the AI-DLC `units-generation` stage (inception phase, 2.7) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage units-generation --single`: the engine
  emits one run-stage directive for `units-generation` and the conductor runs it, then the single-stage run
...
Run the `units-generation` stage (Units Generation) on its own. This is opt-in packaging over `/aidlc --stage units-generation --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "units-generation", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "units-generation", "result": "completed" }
   ```
