---
description: >
  Run the AI-DLC `feedback-optimization` stage (operation phase, 4.7) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage feedback-optimization --single`: the engine
  emits one run-stage directive for `feedback-optimization` and the conductor runs it, then the single-stage run
...
Run the `feedback-optimization` stage (Feedback & Optimization) on its own. This is opt-in packaging over `/aidlc --stage feedback-optimization --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "feedback-optimization", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "feedback-optimization", "result": "completed" }
   ```
