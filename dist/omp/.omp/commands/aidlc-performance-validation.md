---
description: >
  Run the AI-DLC `performance-validation` stage (operation phase, 4.6) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage performance-validation --single`: the engine
  emits one run-stage directive for `performance-validation` and the conductor runs it, then the single-stage run
...
Run the `performance-validation` stage (Performance Validation) on its own. This is opt-in packaging over `/aidlc --stage performance-validation --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "performance-validation", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "performance-validation", "result": "completed" }
   ```
