---
description: >
  Run the AI-DLC `rough-mockups` stage (ideation phase, 1.6) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage rough-mockups --single`: the engine
  emits one run-stage directive for `rough-mockups` and the conductor runs it, then the single-stage run
...
Run the `rough-mockups` stage (Rough Mockups) on its own. This is opt-in packaging over `/aidlc --stage rough-mockups --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "rough-mockups", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "rough-mockups", "result": "completed" }
   ```
