---
description: >
  Run the AI-DLC `functional-design` stage (construction phase, 3.1) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage functional-design --single`: the engine
  emits one run-stage directive for `functional-design` and the conductor runs it, then the single-stage run
...
Run the `functional-design` stage (Functional Design) on its own. This is opt-in packaging over `/aidlc --stage functional-design --single`; the same stage is always reachable via that flag without this command.
