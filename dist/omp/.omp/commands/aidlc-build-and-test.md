---
description: >
  Run the AI-DLC `build-and-test` stage (construction phase, 3.6) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage build-and-test --single`: the engine
  emits one run-stage directive for `build-and-test` and the conductor runs it, then the single-stage run
...
Run the `build-and-test` stage (Build and Test) on its own. This is opt-in packaging over `/aidlc --stage build-and-test --single`; the same stage is always reachable via that flag without this command.
...
1. Ask the engine for the single-stage directive by calling the `aidlc_orchestrate` custom tool:
   ...
   ```json
   { "subcommand": "next", "stage_slug": "build-and-test", "args": ["--single"] }
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```json
   { "subcommand": "report", "single": true, "stage_slug": "build-and-test", "result": "completed" }
   ```
