---
description: >
  Run the AI-DLC `team-formation` stage (ideation phase, 1.5) in isolation via the oh-my-pi custom tool,
  without advancing the main workflow. Equivalent to `/aidlc --stage team-formation --single`: the engine
  emits one run-stage directive for `team-formation` and the conductor runs it, then the single-stage run
...
Run the `team-formation` stage (Team Formation) on its own. This is opt-in packaging over `/aidlc --stage team-formation --single`; the same stage is always reachable via that flag without this command.
