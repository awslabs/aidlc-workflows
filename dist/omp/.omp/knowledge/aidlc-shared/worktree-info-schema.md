...
```
bun .omp/tools/aidlc-worktree.ts info --slug <kebab-slug>
```
...
- Implementation: `.omp/tools/aidlc-worktree.ts` (`handleInfo` handler)
- Test: `tests/unit/t72-worktree-info.sh`
- Caller: `.omp/skills/aidlc/SKILL.md` (per-Bolt-loop halt-and-ask flow)
...
- Audit-format spec: `.omp/knowledge/aidlc-shared/audit-format.md` `WORKTREE_CREATED` row
