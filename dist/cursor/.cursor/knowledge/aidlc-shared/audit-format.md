...
## Event Registry (93 events, 22 categories)
...
| `HUMAN_TURN` | A supported prompt-submit, answered-widget, or review-UI Save seam was observed (the approval/interview gate requires one since the last gate resolution); omitted when the driver declares `AIDLC_UNATTENDED=1` | Timestamp; the review-UI seam adds Mode=browser, Source=review-ui, Submission=<answers file> | `hooks/aidlc-record-human-turn.ts` (UserPromptSubmit + PostToolUse AskUserQuestion) + the per-harness prompt-submit adapters; `tools/aidlc-review-ui.ts` on `POST /api/answers` |
...
| `REVIEW_UI_RESPONSES` | Agent feedback dispositions are attached when a revised browser gate re-opens | Timestamp, Stage, Revision, File, Remarks; optional Unit | `tools/aidlc-orchestrate.ts report` |
