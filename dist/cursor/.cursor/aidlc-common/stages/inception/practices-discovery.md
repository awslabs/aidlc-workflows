...
1. **aidlc-pipeline-deploy-agent** (lead) — Reads `.cursor/knowledge/aidlc-pipeline-deploy-agent/branching-strategies.md`. Scans git history (branch names, merge patterns, lifetime), CI config, deployment cadence. Returns: branching strategy match, deployment frequency, environment topology.
...
Log each question via `bun .cursor/tools/aidlc-log.ts decision` BEFORE presenting it. Log each answer via `bun .cursor/tools/aidlc-log.ts answer` after the user responds.
...
After writing the four artifacts, emit `PRACTICES_DISCOVERED` via `bun .cursor/tools/aidlc-state.ts practices-event --type discovered --field "Sources Scanned: <list>" --field "Drafts: team-practices.md, discovered-rules.md"` (the tool wraps the audit emission so events stay tool-owned per the audit-first invariant).

### Step 5: Affirmation Gate

Compliance with `stage-protocol.md` checklist:

1. `bun .cursor/tools/aidlc-state.ts gate-start practices-discovery` BEFORE the affirmation question.
2. `bun .cursor/tools/aidlc-log.ts decision` for the affirmation question.
3. A structured question presents `team-practices.md` and `discovered-rules.md` for review. Options:
   - **Approve** — promote affirmed content to `.cursor/rules/aidlc-team.md` and `.cursor/rules/aidlc-project.md` (Step 6).
...
4. `bun .cursor/tools/aidlc-log.ts answer` after the user answers.
5. `bun .cursor/tools/aidlc-orchestrate.ts report --stage practices-discovery --result approved --user-input "<exact label>"` (or `bun .cursor/tools/aidlc-state.ts reject practices-discovery --feedback "<text>"`) — auto-emits the gate-approved/gate-rejected audit events through the owning tools.
...
- Reads both drafts and both target files (`.cursor/rules/aidlc-team.md` and `.cursor/rules/aidlc-project.md`); fails closed before any write if any input is missing.
...
2. Update `Practices Affirmed Timestamp` in `<record>/aidlc-state.md` via `bun .cursor/tools/aidlc-state.ts set "Practices Affirmed Timestamp=NOW"` (the `NOW` literal expands to the current ISO 8601 timestamp; the field is part of the v7 state template).
...
- Review path: `<record>/inception/practices-discovery/` AND `.cursor/rules/aidlc-team.md` AND `.cursor/rules/aidlc-project.md`
...
- Prescriptive rule → `.cursor/rules/aidlc-phase-<phase>.md` (phase-scoped)
  or `.cursor/rules/aidlc-<org|team|project>.md` (cross-cutting)
