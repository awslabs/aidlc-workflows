...
- **Brownfield:** use available reverse-engineering artifacts and workspace
  ...
  ```bash
  bun .cursor/tools/aidlc-pr.ts detect --repo <owner/repo>
  ```
...
2. Read the affirmed `Integration mode` field. For `pr`, run:
   ...
   ```bash
   bun .cursor/tools/aidlc-state.ts set-integration-mode pr
   bun .cursor/tools/aidlc-utility.ts recompose --add pr-integration
   ```

   For `direct`, run:

   ```bash
   bun .cursor/tools/aidlc-state.ts set-integration-mode absent
   bun .cursor/tools/aidlc-utility.ts recompose --skip pr-integration
   ```
