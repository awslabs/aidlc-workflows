...
1. **Invoke reviewer sub-agent.** Before every dispatch, not only the first,
   ...
   bytes before any existing terminal appendix. After the request succeeds, if
   that artifact already carries a
