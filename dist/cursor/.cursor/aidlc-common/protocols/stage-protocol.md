...
- **Consolidated summary before generation**: After all questions have been
  ...
  `bun .cursor/tools/aidlc.ts engine log decision --stage <slug>
...
```bash
bun .cursor/tools/aidlc.ts engine state lookup phase-of SLUG          # → phase name
bun .cursor/tools/aidlc.ts engine state lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .cursor/tools/aidlc.ts engine state lookup agent-for SLUG          # → lead agent name
bun .cursor/tools/aidlc.ts engine state lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
`bun .cursor/tools/aidlc.ts engine gen scope-table` for the current
