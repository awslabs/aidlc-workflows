...
2. [ ] **Log questions via `aidlc-log.ts`** — before presenting a structured question: `bun .cursor/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .cursor/tools/aidlc-log.ts answer --stage <slug> --details "<exact choice>"`. (§3)
...
1. Optional before the human prompt: `bun .cursor/tools/aidlc-state.ts gate-start <slug>` — marks the stage `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. The stage is now on-hold waiting for the user; `/aidlc --status` will show "Awaiting your approval on <stage-name>". If this step is missed, the later `report --stage <slug> --result approved` opens the missing gate before approval, and `reject <slug>` likewise backfills it before the rejection (both backfilled rows carry `Recovered: true`).
...
3. Based on the user response:
   ...
   - **Request Changes** → `bun .cursor/tools/aidlc-state.ts reject <slug> --feedback "<text>"`. The tool emits `GATE_REJECTED` + `STAGE_REVISING`, marks `[?]` → `[R]`, increments Revision Count. If gate-start was skipped (stage still `[-]`), reject backfills the missing `STAGE_AWAITING_APPROVAL` first — mirroring the approve-side backfill. After re-running the stage work, call `bun .cursor/tools/aidlc-state.ts revise <slug>` to re-enter the gate (emits a fresh `STAGE_AWAITING_APPROVAL`, marks `[R]` → `[?]`). If you revise the artifact at an open gate without running this verb, the engine backfills the `GATE_REJECTED` + `STAGE_REVISING` pair (tagged `Recovered: true`) at the next approve so the revision is not lost, but run the verb yourself so the feedback text is recorded.
...
1. Advance state atomically via CLI tool (see "Silent bookkeeping writes" below):
   `bun .cursor/tools/aidlc-state.ts advance "<completed-slug>" "<next-slug>"`
...
1. BEFORE presenting the approval question: optionally `bun .cursor/tools/aidlc-state.ts gate-start <slug>` (emits `STAGE_AWAITING_APPROVAL` and makes status truthful while the prompt is open).
2. AFTER user response: `bun .cursor/tools/aidlc-orchestrate.ts report --stage <slug> --result approved --user-input "<choice>"` or `bun .cursor/tools/aidlc-state.ts reject <slug> --feedback "<text>"`. `report` emits any missing gate row, then `GATE_APPROVED` + `STAGE_COMPLETED`, and auto-advances to the next in-scope stage (or completes the workflow if this was the final stage). `reject` likewise emits any missing gate row, then `GATE_REJECTED` + `STAGE_REVISING`, and leaves the stage in `[R]`.
...
# Mark stage complete
bun .cursor/tools/aidlc-state.ts checkbox "SLUG=completed"
```

**Field updates** (aidlc-state.md) — the tool writes fields in `- **Field Name**: value` format:
```bash
bun .cursor/tools/aidlc-state.ts set "Current Stage=STAGE_NAME" "Lifecycle Phase=PHASE" "Status=In Progress" "Last Updated=NOW" "Active Agent=AGENT_NAME" "In Progress=STAGE_NAME"
```
...
```bash
bun .cursor/tools/aidlc-state.ts finalize "completed-slug"
```
...
```bash
bun .cursor/tools/aidlc-state.ts complete-workflow "completed-slug"
```
...
1. `.cursor/rules/` — organization and project guardrails (always)
...
| **Scope** | Controls which stages execute and at what depth. Nine built-in scopes, one file per scope under `.cursor/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, workshop. Custom scopes can be added without editing this file. |
...
| **Guardrail** | A learned behavioral rule (org-level or project-level) stored in `.cursor/rules/` |
