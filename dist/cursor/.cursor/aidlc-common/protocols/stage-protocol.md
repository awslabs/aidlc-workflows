...
2. After the learnings answer is logged: `bun .cursor/tools/aidlc-orchestrate.ts report --stage <slug> --result awaiting-approval` — the engine marks `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. `/aidlc --status` now truthfully shows the held gate.
...
compiled scope grid. Use `bun .cursor/tools/aidlc-utility.ts
