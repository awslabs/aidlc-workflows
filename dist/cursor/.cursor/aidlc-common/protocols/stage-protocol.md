...
5. **Persist (the tool writes + emits audit).** Build the selections file — `{ stage_slug, space, intent, selections[] }`, where `space`/`intent` are carried verbatim from step 2's surface output — and call:
   ...
   ```
   The tool, inside one `withAuditLock` transaction pinned to the selections file's own `space`/`intent` (decide-inside-lock, content-presence idempotency via a `<!-- cid:<intent-slug>:<stage-slug>:<content-hash> -->` marker — a hash of the learning's own text, not its positional candidate id, so a crashed run recovers without double-appending and two different learnings landing on the same candidate id across separate runs never collide):
