...
Load this module from `.cursor/aidlc-common/protocols/stage-protocol-html.md` when `directive.protocol_modules` lists `html`. Apply it to every HTML artifact in `directive.produces`; Markdown artifacts keep the ordinary stage protocol unchanged.
