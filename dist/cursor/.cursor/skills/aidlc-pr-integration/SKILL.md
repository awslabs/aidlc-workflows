...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .cursor/tools/aidlc-orchestrate.ts next --stage pr-integration --single
   ```
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .cursor/tools/aidlc-orchestrate.ts report --single --stage pr-integration --result completed
   ```
