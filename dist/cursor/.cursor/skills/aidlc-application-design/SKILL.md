...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .cursor/tools/aidlc-orchestrate.ts next --stage application-design --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .cursor/tools/aidlc-orchestrate.ts report --single --stage application-design --result completed
   ```
