...
```
Loop:
  1. directive = `bun .cursor/tools/aidlc-orchestrate.ts next <the text after /aidlc>`
...
`directive.mode` tells you HOW to run the body: `inline` (run it in this session, with the lead agent's persona framing loaded from its `.md` file under `.cursor/agents/`), or `subagent` (delegate it to the named subagent — `aidlc-developer-agent` for reverse-engineering and code-generation — which loads its own persona; do not inject it in the prompt).

### Harness notes (Cursor)

- **State sync rides the hooks where it can.** The shipped `.cursor/hooks.json` syncs the state file's stage status after shell commands (audit-tail mode) — but treat state sync as conductor-owned: the state tools do all bookkeeping (`aidlc-state.ts advance/approve` etc. via the engine's `report`), and the stage-protocol's TaskUpdate steps are satisfied by Cursor's plan/todo feature when available — keep a task list for visibility, but know that state-file sync rides on the tools, not on a todo hook.
- **Stage visibility**: no statusline is wired on this harness. Surface position with the Part 4 progress line after every gate, and `/aidlc --status` on demand.
- **Stop-hook mechanics**: Cursor's stop hook cannot hard-block a turn; the framework's forwarding-discipline nudge arrives as an auto-submitted follow-up message instead (the harness `followup_message` channel, capped by `loop_limit: 12` in `.cursor/hooks.json` — above the core hook's own interactive/autonomous ceilings of 2/8, so the core caps govern). Treat such a follow-up exactly like the Claude-side block reason: complete the forwarding loop (finish the `report`, or present the gate) rather than ending mid-workflow.
- **Reviewer read-scope (§12a) is prose-governed here.** Cursor's `preToolUse` payloads carry no subagent identity and hooks cannot be scoped to one subagent, so the deterministic reviewer-scope hook ships unwired on this harness. Still write (and delete) the dispatch record exactly as §12a says — it is the audit surface — and hold the reviewer to the read-scope bound in prose.
- **Headless caveat**: under `agent -p` (print mode) treat the loop above as the only forwarding discipline — never end a turn mid-workflow without either a gate question or a completed `report`.
...
When an intent is already active, `next` advances it (the engine is read-only and never births alongside a live intent). But the FIRST thing you do with each new `/aidlc` input is a knowledge judgment that belongs to you, not the engine: **does this input continue the active intent, describe a genuinely new, unrelated piece of work, or ask to re-shape the RUNNING workflow's plan?**
...
3. Continue into the normal birth: run `bun .cursor/tools/aidlc-orchestrate.ts next --scope <name>` and act on its birth print exactly as "Acting on a directive" describes (the `--label` essence and the duplicate-intent guard ride the same path as any birth).
...
The engine resolves scope-level stage routing internally (it reads the compiled scope grid the table below summarises). The summary table is kept here as human-readable data — not dispatch logic — and is regenerated, never hand-edited. (One carve-out: the dispatched composer agent APPENDS approved composed scopes to the runtime scope registry (`.cursor/scopes/aidlc-<name>.md` + a `scope-grid.json` entry) - that is the sanctioned write path for composed scopes, not a hand-edit; this summary table itself stays generated.) Source of truth: one file per scope under `.cursor/scopes/aidlc-<name>.md` plus each stage's `scopes:` frontmatter, transposed at `bun .cursor/tools/aidlc-graph.ts compile`; regenerate this table with `bun .cursor/tools/aidlc-utility.ts scope-table`.
...
- **11 domain experts**: Each stage leverages the appropriate agent persona; personas load inline from `.cursor/agents/aidlc-<role>-agent.md` for 30 of 32 stages.
...
- **No nested delegation**: The conductor orchestrates all agent invocations. Worker personas carry `disallowedTools: Task` plus an explicit do-not-delegate instruction, and the conductor never asks a delegate to delegate — even though this harness technically allows one level of subagent nesting.
