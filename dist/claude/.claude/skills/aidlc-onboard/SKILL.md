...
```bash
bun .claude/tools/aidlc-onboard.ts capture \
...
```bash
bun .claude/tools/aidlc-onboard.ts classify \
...
```bash
bun .claude/tools/aidlc-onboard.ts list
```
...
```bash
bun .claude/tools/aidlc-learnings.ts persist-rule \
  --scope <project|team> \
