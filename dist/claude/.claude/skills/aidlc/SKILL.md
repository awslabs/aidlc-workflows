...
| `ask` | Render `directive.question` via `AskUserQuestion` (this harness's binding for the protocol's structured questions — see `question-rendering.md` beside this file). For a cold-start scope-confirm ask (no workflow state yet), re-run `next` with the original task plus the selected route: `compose "<original task>"`, or `--scope <scope> <original task>`. Do **not** call `report` before an intent exists. For asks on an existing workflow, feed the answer back on `report` via `--user-input "<answer>"`. The engine never calls `AskUserQuestion` itself — it defers the human turn to you. |
