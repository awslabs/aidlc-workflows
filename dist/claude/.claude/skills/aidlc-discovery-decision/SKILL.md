---
name: aidlc-discovery-decision
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `discovery-decision` stage (ideation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage discovery-decision --single`:
  the engine emits one run-stage directive for discovery-decision and its gate, the
...
# AI-DLC Stage Runner — discovery-decision

Run the `discovery-decision` stage on its own. This is opt-in packaging over
`/aidlc --stage discovery-decision --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage discovery-decision --single
   ```

   The engine emits one `run-stage` directive for `discovery-decision` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage discovery-decision --result completed
   ```
