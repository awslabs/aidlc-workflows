...
| `SWARM_UNIT_CONVERGED` | A swarm Unit re-verified green (and untampered) at the `finalize` gate AND its merge-back landed (a converged unit in `merge_failures` gets no row until a finalize retry merges it) | Timestamp, Batch number, Unit name, Stage, Run floor (the stage's latest main-workflow `STAGE_STARTED` timestamp — consumers count a row only when both match the current attempt), optional Source Fingerprint and Source Commit (workspace-source binding; the immutable commit is the exact source object accepted by `aidlc-worktree merge`), or `Source Freshness Bypass: true` when that finalize explicitly ran under `AIDLC_SKIP_SOURCE_FRESHNESS=1` (a later merge must still carry the switch; absence remains reserved for genuine pre-binding migration rows) | `tools/aidlc-swarm.ts` |
