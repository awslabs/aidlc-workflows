...
2. [ ] **Log questions via `aidlc-log.ts`** — before presenting a structured question: `bun .pi/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .pi/tools/aidlc-log.ts answer --stage <slug> --details "<exact choice>"`. (§3)
...
- Call `bun .pi/tools/aidlc-orchestrate.ts report --stage <slug> --result approved --user-input "Approve" --test-run`. The report command opens the gate if it is still `[-]`, tags the emitted `GATE_APPROVED` event with `Test-Run: true`, and auto-advances to the next in-scope stage.
...
- Under `--test-run`: auto-select "Continue autonomously" — call `bun .pi/tools/aidlc-bolt.ts set-autonomy --mode autonomous` (the tool emits AUTONOMY_MODE_SET). No separate auto-event.
...
1. Optional before the human prompt: `bun .pi/tools/aidlc-state.ts gate-start <slug>` — marks the stage `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. The stage is now on-hold waiting for the user; `/aidlc --status` will show "Awaiting your approval on <stage-name>". If this step is missed, the later `report --stage <slug> --result approved` opens the missing gate before approval, and `reject <slug>` likewise backfills it before the rejection (both backfilled rows carry `Recovered: true`).
2. Present Parts 1-3 (announcement, summary, approval question).
3. Based on the user response:
   - **Approve** → `bun .pi/tools/aidlc-orchestrate.ts report --stage <slug> --result approved --user-input "<exact choice>"`. The engine emits any missing `STAGE_AWAITING_APPROVAL`, then `GATE_APPROVED` + `STAGE_COMPLETED`, and auto-advances to the next in-scope stage (or completes the workflow on the final stage). No separate `advance` call required.
   - **Request Changes** → `bun .pi/tools/aidlc-state.ts reject <slug> --feedback "<text>"`. The tool emits `GATE_REJECTED` + `STAGE_REVISING`, marks `[?]` → `[R]`, increments Revision Count. If gate-start was skipped (stage still `[-]`), reject backfills the missing `STAGE_AWAITING_APPROVAL` first — mirroring the approve-side backfill. After re-running the stage work, call `bun .pi/tools/aidlc-state.ts revise <slug>` to re-enter the gate (emits a fresh `STAGE_AWAITING_APPROVAL`, marks `[R]` → `[?]`).
...
- Record answers via `bun .pi/tools/aidlc-log.ts answer --stage <slug> --details "[N] questions auto-answered option A" --test-run` (the tool emits QUESTION_ANSWERED with `Test-Run: true`).
...
1. Advance state atomically via CLI tool (see "Silent bookkeeping writes" below):
   `bun .pi/tools/aidlc-state.ts advance "<completed-slug>" "<next-slug>"`
...
1. BEFORE presenting the approval question: optionally `bun .pi/tools/aidlc-state.ts gate-start <slug>` (emits `STAGE_AWAITING_APPROVAL` and makes status truthful while the prompt is open).
2. AFTER user response: `bun .pi/tools/aidlc-orchestrate.ts report --stage <slug> --result approved --user-input "<choice>"` or `bun .pi/tools/aidlc-state.ts reject <slug> --feedback "<text>"`. `report` emits any missing gate row, then `GATE_APPROVED` + `STAGE_COMPLETED`, and auto-advances to the next in-scope stage (or completes the workflow if this was the final stage). `reject` likewise emits any missing gate row, then `GATE_REJECTED` + `STAGE_REVISING`, and leaves the stage in `[R]`.

At each question interaction:
1. BEFORE presenting the question: `bun .pi/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .pi/tools/aidlc-log.ts answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
State and audit updates use the CLI tools in `.pi/tools/`. These tools handle atomic read-modify-write, timestamp generation, and audit formatting internally. Do NOT use Edit or Write for these updates — those tools show diffs that create visual noise.

**CWD drift warning**: If a stage runs `cd` in Bash (e.g., `cd todo-app/server && npm install`), subsequent `bun .pi/tools/...` calls using relative paths will fail with "Module not found". Always use absolute paths to the tools directory for tool calls (on Claude Code, `$CLAUDE_PROJECT_DIR/.claude/tools/`), or run `cd` commands in subshells: `(cd subdir && npm install)`.
...
# Mark stage complete
bun .pi/tools/aidlc-state.ts checkbox "SLUG=completed"
```

**Field updates** (aidlc-state.md) — the tool writes fields in `- **Field Name**: value` format:
```bash
bun .pi/tools/aidlc-state.ts set "Current Stage=STAGE_NAME" "Lifecycle Phase=PHASE" "Status=In Progress" "Last Updated=NOW" "Active Agent=AGENT_NAME" "In Progress=STAGE_NAME"
```
...
```bash
bun .pi/tools/aidlc-state.ts finalize "completed-slug"
```
...
```bash
bun .pi/tools/aidlc-state.ts complete-workflow "completed-slug"
```
...
```bash
bun .pi/tools/aidlc-state.ts lookup phase-of SLUG          # → phase name
bun .pi/tools/aidlc-state.ts lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .pi/tools/aidlc-state.ts lookup agent-for SLUG          # → lead agent name
bun .pi/tools/aidlc-state.ts lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
1. `.pi/rules/` — organization and project guardrails (always)
2. `.pi/knowledge/aidlc-shared/` — shared methodology principles
3. `.pi/knowledge/[agent-name]/` — agent-specific methodology
...
| **Scope** | Controls which stages execute and at what depth. Nine built-in scopes, one file per scope under `.pi/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, workshop. Custom scopes can be added without editing this file. |
...
| **Guardrail** | A learned behavioral rule (org-level or project-level) stored in `.pi/rules/` |
...
- `.pi/rules/aidlc-project-learnings.md` (default) or `.pi/rules/aidlc-team-learnings.md` — rolling dated entries, one click to widen a candidate from project to team. There is no org-learnings file and no widen-to-org path. These are a separate surface from the practices files (`aidlc-project.md` / `aidlc-team.md`) and their topical sections — never `## Corrections`, never a practices-discovery heading.
- `.pi/sensors/aidlc-<id>.md` — for verification checks. A project-tier manifest with a `matches:` capability glob, bound to the originating stage by appending its id to that stage's `sensors:` frontmatter list.
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .pi/tools/aidlc-learnings.ts surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file and call:
   ```bash
   bun .pi/tools/aidlc-learnings.ts persist --slug <stage-slug> --selections-json <path>
   ```
   ...
   - **Sensor** → scaffolds a project-tier `<project>/.pi/sensors/aidlc-<id>.md` manifest (with the user-supplied `matches:` glob) AND appends the new id to the originating stage's `sensors:` frontmatter list — both writes inside the same lock. Emits `SENSOR_PROPOSED`. The sensor binds and fires from the next workflow's compile.
...
└── Sensor (two-write install): scaffold a project-tier manifest at
    .pi/sensors/aidlc-<id>.md with a matches: glob, AND append its id to
...
```bash
bun .pi/tools/aidlc-state.ts reuse-artifact <stage-slug> \
