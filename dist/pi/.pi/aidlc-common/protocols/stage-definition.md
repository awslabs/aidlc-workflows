...
under `.pi/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
`.pi/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.pi/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.pi/tools/data/stage-graph.json`. Consumers continue to read the compiled
