...
Read the state template from `.pi/knowledge/aidlc-shared/state-template.md`.
...
- Total Stages: count EXECUTE stages only (not SKIP). Authoritative counts come from the compiled scope grid (`.pi/tools/data/scope-grid.json`, transposed from each stage's `scopes:` frontmatter; run `bun .pi/tools/aidlc-utility.ts scope-table` for the live table). Today's values:
  | Scope | EXECUTE / Total |
  |-------|-----------------|
