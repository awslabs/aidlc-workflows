...
1. **aidlc-pipeline-deploy-agent** (lead) — Reads `.pi/knowledge/aidlc-pipeline-deploy-agent/branching-strategies.md`. Scans git history (branch names, merge patterns, lifetime), CI config, deployment cadence. Returns: branching strategy match, deployment frequency, environment topology.
...
**Test-run mode**: per `stage-protocol.md`, skip the structured questions. For greenfield, use `aidlc-org.md` defaults verbatim. For brownfield, use evidence-only findings. Log via `bun .pi/tools/aidlc-log.ts answer --test-run` for each implied answer.

Log each question via `bun .pi/tools/aidlc-log.ts decision` BEFORE presenting it. Log each answer via `bun .pi/tools/aidlc-log.ts answer` after the user responds.
...
After writing the four artifacts, emit `PRACTICES_DISCOVERED` via `bun .pi/tools/aidlc-state.ts practices-event --type discovered --field "Sources Scanned: <list>" --field "Drafts: team-practices.md, discovered-rules.md"` (the tool wraps the audit emission so events stay tool-owned per the audit-first invariant).

### Step 5: Affirmation Gate

Compliance with `stage-protocol.md` checklist:

1. `bun .pi/tools/aidlc-state.ts gate-start practices-discovery` BEFORE the affirmation question.
2. `bun .pi/tools/aidlc-log.ts decision` for the affirmation question.
3. A structured question presents `team-practices.md` and `discovered-rules.md` for review. Options:
   - **Approve** — promote affirmed content to `.pi/rules/aidlc-team.md` and `.pi/rules/aidlc-project.md` (Step 6).
...
4. `bun .pi/tools/aidlc-log.ts answer` after the user answers.
5. `bun .pi/tools/aidlc-orchestrate.ts report --stage practices-discovery --result approved --user-input "<exact label>"` (or `bun .pi/tools/aidlc-state.ts reject practices-discovery --feedback "<text>"`) — auto-emits the gate-approved/gate-rejected audit events through the owning tools.
6. **Test-run mode**: `bun .pi/tools/aidlc-orchestrate.ts report --stage practices-discovery --result approved --user-input "Approve (test-run)" --test-run`.
...
```
bun .pi/tools/aidlc-state.ts practices-promote \
...
- Reads both drafts and both target files (`.pi/rules/aidlc-team.md` and `.pi/rules/aidlc-project.md`); fails closed before any write if any input is missing.
...
2. Update `Practices Affirmed Timestamp` in `aidlc-docs/aidlc-state.md` via `bun .pi/tools/aidlc-state.ts set "Practices Affirmed Timestamp=NOW"` (the `NOW` literal expands to the current ISO 8601 timestamp; the field is part of the v7 state template).
...
- Review path: `aidlc-docs/inception/practices-discovery/` AND `.pi/rules/aidlc-team.md` AND `.pi/rules/aidlc-project.md`
...
- Prescriptive rule → `.pi/rules/aidlc-phase-<phase>.md` (phase-scoped)
  or `.pi/rules/aidlc-<org|team|project>.md` (cross-cutting)
- Verification check → new manifest at `.pi/sensors/aidlc-<id>.md`
