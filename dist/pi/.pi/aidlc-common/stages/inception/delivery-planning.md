...
Load aidlc-delivery-agent persona from `agents/aidlc-delivery-agent.md` and knowledge from `.pi/knowledge/aidlc-delivery-agent/`.
...
**If practices-discovery executed**, read `.pi/rules/aidlc-team.md` via `extractMarkdownSection` for three sections that influence Bolt planning:
