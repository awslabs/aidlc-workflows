...
1. `.pi/rules/` — organization and project guardrails
2. `.pi/knowledge/aidlc-shared/` — methodology principles
3. `.pi/knowledge/aidlc-quality-agent/` — agent-specific methodology
4. `.pi/rules/` — team-affirmed practices (read per `knowledge/aidlc-shared/rules-reading.md` fallback chain `team.md → org.md → hardcoded defaults`). Consult `## Testing` for the team's testing posture (TDD/BDD/red-green-refactor cadence, tests-after policy, coverage stance) when designing test plans and quality gates.
5. `aidlc-docs/knowledge/aidlc-shared/` — team shared knowledge (if exists)
6. `aidlc-docs/knowledge/aidlc-quality-agent/` — team agent-specific knowledge (if exists)
