...
- Read team practices via `.pi/rules/aidlc-team.md` (fall back to `org.md` per `knowledge/aidlc-shared/rules-reading.md`); match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `aidlc-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `bun .pi/tools/aidlc-worktree.ts` from the main repo checkout; `aidlc-worktree` itself emits the audit event audit-first before invoking git
...
1. `.pi/rules/` -- execution guardrails
2. `.pi/knowledge/aidlc-shared/` -- shared methodology
3. `.pi/knowledge/aidlc-pipeline-deploy-agent/` -- agent-specific methodology
4. `.pi/rules/` -- team-affirmed practices (read per `knowledge/aidlc-shared/rules-reading.md`)
