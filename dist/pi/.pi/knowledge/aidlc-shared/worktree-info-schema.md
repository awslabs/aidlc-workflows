...
```
bun .pi/tools/aidlc-worktree.ts info --slug <kebab-slug>
```
...
- Implementation: `.pi/tools/aidlc-worktree.ts` (`handleInfo` handler)
- Test: `tests/unit/t72-worktree-info.sh`
- Caller: `.pi/skills/aidlc/SKILL.md` (per-Bolt-loop halt-and-ask flow)
...
- Audit-format spec: `.pi/knowledge/aidlc-shared/audit-format.md` `WORKTREE_CREATED` row
