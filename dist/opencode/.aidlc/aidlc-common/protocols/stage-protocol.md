...
- Load `stage-protocol-guide.md`; run `bun .aidlc/tools/aidlc-html.ts scaffold --guide <stage-dir>/<slug>-questions.md --out <stage-dir>/<slug>-questions-guide.html` (add `--depth minimal` at Minimal depth), fill the scaffold's prose and every `data-aidlc-recommend`, run the module's check, then say: "Your [N] questions are in the browser (`**Browser:** <review_ui.url>` or origin fallback) → Questions, each with an explainer and a recommendation. Read an explainer with trade-offs and answer in the browser; I'll continue the moment you save. Prefer the terminal? Say so and I'll walk you through here." On Claude Code: END TURN — the Stop hook holds the turn until the human clicks **Save**, then resumes you with the apply command; the human types nothing. A human who answers in the terminal instead has chosen **Guide me**: continue as Step 3a from there and do not switch back.
