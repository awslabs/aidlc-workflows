...
| `ask` | Render `directive.question` as a structured question per `question-rendering.md` (numbered prose options). For a cold-start scope-confirm ask (no workflow state yet), re-run `next` with the original task plus the selected route: `compose "<original task>"`, or `--scope <scope> <original task>`. Do **not** call `report` before an intent exists. For asks on an existing workflow, feed the answer back on `report` via `--user-input "<answer>"`. The engine never asks the user itself — it defers the human turn to you. |
