...
```bash
bun .kiro/tools/aidlc-onboard.ts capture \
...
```bash
bun .kiro/tools/aidlc-onboard.ts classify \
...
```bash
bun .kiro/tools/aidlc-onboard.ts list
```
...
```bash
bun .kiro/tools/aidlc-learnings.ts persist-rule \
  --scope <project|team> \
