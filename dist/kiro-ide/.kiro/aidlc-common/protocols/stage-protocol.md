...
`.kiro/aidlc-common/protocols/stage-protocol-html.md` exactly like
...
- Load `stage-protocol-guide.md`; run `bun .kiro/tools/aidlc-html.ts scaffold --guide <stage-dir>/<slug>-questions.md --out <stage-dir>/<slug>-questions-guide.html` (add `--depth minimal` at Minimal depth), fill the scaffold's prose and every `data-aidlc-recommend`, run the module's check, then say: "Your [N] questions are in the browser (`**Browser:** <review_ui.url>` or origin fallback) → Questions, each with an explainer and a recommendation. Read an explainer with trade-offs and answer in the browser; I'll continue the moment you save. Prefer the terminal? Say so and I'll walk you through here." On Claude Code: END TURN — the Stop hook holds the turn until the human clicks **Save**, then resumes you with the apply command; the human types nothing. A human who answers in the terminal instead has chosen **Guide me**: continue as Step 3a from there and do not switch back.
- On every other harness the Stop seam cannot hold a turn. There, before ending the turn, run `bun .kiro/tools/aidlc-log.ts answers-wait --stage <slug> --questions-file <path> [--unit <unit>]`: it blocks until the answers are saved (exit 0) or nine minutes pass (exit 3 — call it again). Ask for a shell-tool timeout of at least ten minutes when you invoke it.
- Once the answers have landed, run `bun .kiro/tools/aidlc-log.ts answers-apply --stage <slug> --questions-file <path> [--unit <unit>] [--single]` (pass `--single` on an isolated run, as you would to `decision`/`answer`). A browser round is its own confirmation - the human saw every answer with its recommendation and saved them as one act - so `answers-apply` records the consolidated-summary checkpoint itself (`SUMMARY_CONFIRMATION_RECORDED`, `Source: review-ui`) when every question is answered. Do NOT run Step 3a's `Looks correct / Request changes` question after a browser round: read the file, state the consolidated answers as a short summary (a statement, not a question), and generate the artifacts. A human who types **done** anyway is simply early; apply and continue the same way. If `answers-apply` reports `confirmed: false` (a question still open), the round is not finished - continue it in the browser.
