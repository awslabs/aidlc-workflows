...
- The Testing Contract block and instruction: run
  `bun .kiro/tools/aidlc-testing-posture.ts render` yourself and
...
```bash
bun .kiro/tools/aidlc-testing-posture.ts fingerprint --unit "<directive.unit>"
```
...
```bash
bun .kiro/tools/aidlc-log.ts decision --stage code-generation \
...
```bash
bun .kiro/tools/aidlc-log.ts answer --stage code-generation \
  --checkpoint plan-approval \
