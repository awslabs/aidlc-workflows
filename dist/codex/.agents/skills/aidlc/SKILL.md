...
| `ask` | Render `directive.question` as a structured question per `question-rendering.md` (request_user_input when available, numbered prose otherwise), then feed the human's answer back on the next `report` via `--user-input "<answer>"`. For "Resume from last checkpoint", call `report --result resumed --user-input "<answer>"`, then act on its run-then-continue `print`. The engine never asks the user itself — it defers the human turn to you. |
