---
name: aidlc-pr-integration
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `pr-integration` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage pr-integration --single`:
  the engine emits one run-stage directive for pr-integration and its gate, the
...
# AI-DLC Stage Runner — pr-integration

Run the `pr-integration` stage on its own. This is opt-in packaging over
`/aidlc --stage pr-integration --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts next --stage pr-integration --single
   ```

   The engine emits one `run-stage` directive for `pr-integration` (carrying the
...
2. Before acting on the directive, read
   `.codex/aidlc-common/protocols/stage-protocol.md`. Then read every
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .codex/tools/aidlc-orchestrate.ts report --single --stage pr-integration --result completed
   ```
