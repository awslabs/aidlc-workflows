...
the workflow explicitly: `bun .codex/tools/aidlc-orchestrate.ts park`.
...
1. Ask which delivery scope fits the committed build, with each option
   ...
   `bun .codex/tools/aidlc-utility.ts scope-change --scope <chosen>`.
2. Run `bun .codex/tools/aidlc-orchestrate.ts next` once: the engine
   ...
   `bun .codex/tools/aidlc-utility.ts set-status --stage <named stage>`.
3. Mark every ideation stage the new scope expects but this run already
   ...
   run: `bun .codex/tools/aidlc-state.ts skip <slug> --reason
