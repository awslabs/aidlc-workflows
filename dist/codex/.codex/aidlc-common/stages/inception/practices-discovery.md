...
```bash
bun .codex/tools/aidlc.ts engine state practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .codex/tools/aidlc.ts engine orchestrate report --stage
...
```bash
bun .codex/tools/aidlc.ts engine state practices-promote \
