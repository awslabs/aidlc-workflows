...
`bun .codex/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .codex/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo>
```
...
```
bun .codex/tools/aidlc-state.ts reuse-artifact reverse-engineering --decision keep --artifacts "<codekb-path output>" [--repo <repo>] [--single]
```
...
- If every repo is reused on an ordinary workflow run, report the stage as
  ...
  `bun .codex/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result skipped --reason "codekb reuse: all resolved stores CURRENT, human chose reuse"`.
...
Template in `.codex/knowledge/aidlc-developer-agent/re-artifacts.md`:
...
```
bun .codex/tools/aidlc-log.ts link --stage reverse-engineering --link aidlc-developer-agent --artifact "<developer scan handoff path>" [--repo <repo>] [--single]
```
...
its output verbatim:
   ...
   ```
   bun .codex/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo> --mint --paths <analyzed paths>
   ```
...
```
bun .codex/tools/aidlc.ts engine workspace codekb --repo <repo>
```
...
```
bun .codex/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo> --compare <record>/inception/reverse-engineering/scope-draft-<repo>.md
```
...
`bun .codex/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result <outcome>`.
