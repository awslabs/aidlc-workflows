...
1. **Detect.** Run `bun .codex/tools/aidlc-utility.ts detect --json`.
...
2. **Read the repertoire.** Read the stock scope definitions under the
   ...
   SKILL.md (or `.codex/tools/data/stage-graph.json`) for the stage
   list and phases.
...
4. **Validate.** Before the proposal is shown, run the deterministic grid
   check (`bun .codex/tools/aidlc-graph.ts validate-grid ...` once it
