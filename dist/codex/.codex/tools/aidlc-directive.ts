...
export interface IntentPickAskDirective extends AskDirectiveBase {
  ...
  select_command_template: string;
}
...
export function validateDirective(obj: unknown): ValidationResult {
  ...
  switch (kind) {
    ...
    case "ask": {
      ...
      checkOptionalString(o, "select_command_template", kind, errors);
      checkOptionalString(o, "stage", kind, errors);
      ...
      if (
        ...
        errors.push(
          `${kind}: ask_type must be one of scope-confirm | compose-offer | intent-pick | unit-paused | new-work-routing | unit-claim | legacy-plan-approval-recovery, got ${String(o.ask_type)}`,
        );
      }
      ...
      const rejectUnexpected = (
        ...
        for (const field of askPayloadFields) {
          if (field in o && !allowed.has(field)) {
      ...
      if (o.ask_type === "scope-confirm") {
        ...
        checkString(o, "select_command_template", kind, errors);
        rejectUnexpected(
          "intent-pick",
          new Set(["available_intents", "select_command_template"]),
        );
        ...
        rejectUnexpected(
          "unit-paused",
          new Set(["stage", "unit", "resume_command"]),
        );
        ...
        rejectUnexpected(
          "unit-claim",
          new Set(["claimable_units", "claimed_units", "waiting_units"]),
        );
...
if (import.meta.main) {
  ...
  const examples: Directive[] = [
    ...
    },
    {
      ...
      confirm_command:
        "bun <harness-dir>/tools/aidlc-orchestrate.ts next --scope bugfix -- 'fix the login bug'",
      compose_command:
        "bun <harness-dir>/tools/aidlc-orchestrate.ts next compose -- 'fix the login bug'",
      scope_command_template:
        "bun <harness-dir>/tools/aidlc-orchestrate.ts next --scope <scope> -- 'fix the login bug'",
    },
