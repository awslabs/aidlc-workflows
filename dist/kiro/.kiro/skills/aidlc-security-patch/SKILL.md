...
3. `bun .kiro/tools/aidlc-orchestrate.ts report --stage <directive.stage> --result <outcome> [--user-input "<text>"]` only after acting on a stage directive. The prompt-rendered resume menu is the sole non-stage report round-trip and uses `report --result resumed --user-input "<choice>"`.
