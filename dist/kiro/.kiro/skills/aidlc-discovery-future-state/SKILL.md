...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kiro/tools/aidlc-orchestrate.ts next --stage discovery-future-state --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kiro/tools/aidlc-orchestrate.ts report --single --stage discovery-future-state --result completed
   ```
