...
- **Brownfield:** use available reverse-engineering artifacts and workspace
  ...
  ```bash
  bun .kiro/tools/aidlc-pr.ts detect --repo <owner/repo>
  ```
...
2. Read the affirmed `Integration mode` field. For `pr`, run:
   ...
   ```bash
   bun .kiro/tools/aidlc-state.ts set-integration-mode pr
   bun .kiro/tools/aidlc-utility.ts recompose --add pr-integration
   ```

   For `direct`, run:

   ```bash
   bun .kiro/tools/aidlc-state.ts set-integration-mode absent
   bun .kiro/tools/aidlc-utility.ts recompose --skip pr-integration
   ```
