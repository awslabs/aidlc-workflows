...
```bash
bun .aidlc/tools/aidlc.ts engine state practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .aidlc/tools/aidlc.ts engine orchestrate report --stage
...
```bash
bun .aidlc/tools/aidlc.ts engine state practices-promote \
