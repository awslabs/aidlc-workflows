...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .github/tools/aidlc-orchestrate.ts next --stage performance-validation --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .github/tools/aidlc-orchestrate.ts report --single --stage performance-validation --result completed
   ```
