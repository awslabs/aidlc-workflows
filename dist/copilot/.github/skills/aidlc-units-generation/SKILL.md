...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .github/tools/aidlc-orchestrate.ts next --stage units-generation --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .github/tools/aidlc-orchestrate.ts report --single --stage units-generation --result completed
   ```
