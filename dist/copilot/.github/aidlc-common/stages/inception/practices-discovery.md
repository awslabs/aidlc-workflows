...
```bash
bun .github/tools/aidlc-state.ts practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .github/tools/aidlc-orchestrate.ts report --stage
...
```bash
bun .github/tools/aidlc-state.ts practices-promote \
...
- a new `.github/sensors/aidlc-<id>.md` manifest for a verification
