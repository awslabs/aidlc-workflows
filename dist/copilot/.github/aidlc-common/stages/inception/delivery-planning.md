...
Load aidlc-delivery-agent persona from `agents/aidlc-delivery-agent.md` and knowledge from `.github/knowledge/aidlc-delivery-agent/`.
...
`bun .github/tools/aidlc-orchestrate.ts report --stage delivery-planning --result <outcome>`.
...
`bun .github/tools/aidlc-state.ts set-construction-iteration unit-major`.
