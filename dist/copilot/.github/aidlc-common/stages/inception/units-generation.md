...
Load aidlc-delivery-agent persona from `agents/aidlc-delivery-agent.md` and knowledge from `.github/knowledge/aidlc-delivery-agent/` for feasibility validation and prioritization.
...
`bun .github/tools/aidlc-orchestrate.ts report --stage units-generation --result <outcome>`.
