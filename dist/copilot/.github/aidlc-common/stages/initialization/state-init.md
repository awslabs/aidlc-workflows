...
Read the state contract from `.github/knowledge/aidlc-shared/state-template.md`.
...
- Total Stages: count EXECUTE stages only (not SKIP). Authoritative counts come
  from the compiled scope grid (`.github/tools/data/scope-grid.json`),
  transposed from each stage's `scopes:` frontmatter. Run
  `bun .github/tools/aidlc-utility.ts scope-table` for the live scope
  counts and `bun .github/tools/aidlc-utility.ts stage-table` for the
