...
Load aidlc-architect-agent (lead) persona from `agents/aidlc-architect-agent.md` and knowledge from `.github/knowledge/aidlc-architect-agent/`. Load aidlc-developer-agent persona from `agents/aidlc-developer-agent.md` and knowledge from `.github/knowledge/aidlc-developer-agent/` for technical implementation input. Apply aidlc-architect-agent as the primary perspective with aidlc-developer-agent providing technical feasibility input.
...
`bun .github/tools/aidlc-orchestrate.ts report --stage functional-design --result <outcome>`.
