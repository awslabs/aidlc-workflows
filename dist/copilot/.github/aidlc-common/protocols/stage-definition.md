...
under `.github/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
`.github/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.github/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.github/tools/data/stage-graph.json`. Consumers continue to read the compiled
