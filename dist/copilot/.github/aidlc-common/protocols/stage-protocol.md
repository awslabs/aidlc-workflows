...
2. [ ] **Log non-gate questions via `aidlc-log.ts`** — before presenting a structured question that is not an approval gate: `bun .github/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .github/tools/aidlc-log.ts answer --stage <slug> --details "<exact choice>"`. Approval choices go only through `aidlc-orchestrate.ts report`. (§2, §3)
...
2. After the learnings answer is logged: `bun .github/tools/aidlc-orchestrate.ts report --stage <slug> --result awaiting-approval` — the engine marks `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. `/aidlc --status` now truthfully shows the held gate.
...
compiled scope grid. Use `bun .github/tools/aidlc-utility.ts
...
1. BEFORE presenting the approval question: `bun .github/tools/aidlc-orchestrate.ts report --stage <slug> --result awaiting-approval`.
...
1. BEFORE presenting the question: `bun .github/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .github/tools/aidlc-log.ts answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
State and audit updates use the CLI tools in `.github/tools/`. These tools handle atomic read-modify-write, timestamp generation, and audit formatting internally. Do NOT use Edit or Write for these updates — those tools show diffs that create visual noise.

**CWD drift warning**: If a stage runs `cd` in Bash (e.g., `cd todo-app/server && npm install`), subsequent `bun .github/tools/...` calls using relative paths will fail with "Module not found". Always use absolute paths to the tools directory for tool calls (on Claude Code, `$CLAUDE_PROJECT_DIR/.claude/tools/`), or run `cd` commands in subshells: `(cd subdir && npm install)`.
...
```bash
bun .github/tools/aidlc-orchestrate.ts report \
...
```bash
bun .github/tools/aidlc-state.ts lookup phase-of SLUG          # → phase name
bun .github/tools/aidlc-state.ts lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .github/tools/aidlc-state.ts lookup agent-for SLUG          # → lead agent name
bun .github/tools/aidlc-state.ts lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
2. `.github/knowledge/aidlc-shared/` — shared methodology principles
3. `.github/knowledge/[agent-name]/` — agent-specific methodology
...
.github/tools/aidlc-utility.ts scope-table` for the current
...
| **Scope** | Controls which stages execute and at what depth. Nine built-in scopes, one file per scope under `.github/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, workshop. Custom scopes can be added without editing this file. |
...
1. **Invoke reviewer sub-agent.** Delegate to the reviewer agent named in `directive.reviewer`. Pass:
   ...
   **Dispatch record (per-unit stages; enforcement-capable harnesses only).** This record is required only when the current harness registers reviewer-scope PreToolUse enforcement (Claude Code, Kiro CLI, Codex CLI, and opencode today). Immediately before invoking a per-unit reviewer (`directive.unit` present) on one of those harnesses, write `<record>/.aidlc-reviewer-dispatch.json`:
   ...
   `bun .github/tools/aidlc-log.ts review --stage "<directive.stage>" --reviewer "<directive.reviewer>" --iteration <n>`; add `--unit "<directive.unit>"` on a per-unit stage and `--single` on an isolated stage run.
...
- `.github/sensors/aidlc-<id>.md` — for verification checks. A project-tier manifest with a `matches:` capability glob, bound to the originating stage by appending its id to that stage's `sensors:` frontmatter list.
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .github/tools/aidlc-learnings.ts surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file and call:
   ```bash
   bun .github/tools/aidlc-learnings.ts persist --slug <stage-slug> --selections-json <path>
   ```
   ...
   - **Sensor** → scaffolds a project-tier `<project>/.github/sensors/aidlc-<id>.md` manifest (with the user-supplied `matches:` glob) AND appends the new id to the originating stage's `sensors:` frontmatter list — both writes inside the same lock. Emits `SENSOR_PROPOSED`. The sensor binds and fires from the next workflow's compile.
...
└── Sensor (two-write install): scaffold a project-tier manifest at
    .github/sensors/aidlc-<id>.md with a matches: glob, AND append its id to
...
```bash
bun .github/tools/aidlc-state.ts reuse-artifact <stage-slug> \
